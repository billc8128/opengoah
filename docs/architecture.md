...
Handoff remains an event-level control result rather than a current-state projection. It never replaces user-readable prose: the Agent writes the Assistant Message, while Handoff carries only outcome and evidence. Pi intercepts Handoff with `beforeToolCall`; Supervisor returns correctable invariant violations as Tool feedback and reserves hard termination for authority/fencing loss. A successful commit atomically marks the existing Message committed, records Handoff, acknowledges Mail, and closes the Turn without imposing a working-order policy.
