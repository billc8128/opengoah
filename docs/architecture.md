...
A Wake starts a Goal Turn but does not define its duration or amount of work. Goal Agents continue while safe, useful work is immediately executable; Work Record updates are checkpoints, and Schedule only records genuinely time-dependent future observation. Handoff remains terminal, but Agents call it only at a real control boundary: the current actionable frontier is exhausted, an external wait or real blocker prevents further work, or completion evidence is ready for review. Supervisor records and fences this decision; it does not impose minimum duration, tool counts, or semantic work quotas.
...
Turn routing is explicit without classifying Turns. Schema v29 stores canonical Agent role on Thread, reserves the `ceo` role for the `ceo` Agent, stores `user_message|wake` trigger on Turn, stores an optional immutable Goal commitment fence on Turn, and uses Transcript v2 content-addressed request components. Human replacement atomically commits the optional Root Goal, old Turn terminal, new Turn, and initial Message. A terminal Turn may retain `runnerPid` only as cleanup ownership; the next Runner waits until cleanup releases it, and restart recovery kills any retained process. Wake and Schedule keep a discriminated automatic target of `goal(agent, goalId)` or `specialist(agent, verifier|audit)`; Human input starts or steers a CEO Turn directly and never creates a Wake. Goal-targeted admission freezes the current `goalRevision`. Recovery, Verification, stop, replay, and Wake consumption read Thread role, Turn trigger, commitment, and automatic target rather than reconstructing intent from Agent names. Mail is Agent-only and stores one `MailRoute`: `goal`, `ceo_inbox`, or `specialist_inbox`, plus Agent-selected `high|normal|low` delivery priority; CEO/Human communication uses canonical Thread Messages only.
...
Mail routing and priority are envelope metadata, never business JSON. The sending Agent chooses `high|normal|low`; Supervisor only orders bounded delivery by priority and arrival, with `normal` as the default. Agent-to-Agent Mail requires `goalId`; the route must name a Goal currently owned by the recipient, and a committed Turn receives only Mail routed to that Goal. Active routed `high` and `normal` Mail may trigger that owner's next committed Turn, while `low` waits for another interaction. Routed Mail for a paused/blocked Goal remains unread without motion until the parent explicitly resumes the Goal. Verification/Audit results remain in the CEO inbox for the next direct interaction and do not create a CEO system Turn. Unknown recipients are rejected, and Agent Mail cannot address Human. Handoff contains only declarative outcome/evidence, while Mail and Schedule effects are explicit Ledger facts. Reassigning an inactive Goal changes ownership and records routed normal-priority Mail atomically, but returns no Wake.
