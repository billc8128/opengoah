...
Wake and Schedule no longer encode execution with optional fields. Schema v20 stores a discriminated `ExecutionTarget`: `human(ceo)`, `goal(agent, goalId)`, or `specialist(agent, verifier|audit)`. Mail similarly stores one `MailRoute`: `goal`, `human_inbox`, `human_request`, or `specialist_inbox`. SQLite CHECK constraints, Ledger admission, and Supervisor admission validate the same discriminants, so a Child unbound Wake or an unrouted Agent Mail cannot be constructed and deferred for later rejection.
