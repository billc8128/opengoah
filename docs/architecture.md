...
Handoff remains an event-level control result rather than a current-state projection. It never replaces user-readable prose: the Agent writes the Assistant Message, while Handoff carries only outcome and evidence. Pi intercepts Handoff with `beforeToolCall`; every other Runner adapter must provide equivalent feedback-and-continue behavior. Supervisor returns correctable invariant violations as Tool feedback and reserves hard termination for authority/fencing loss; once revoked, all later tools in that batch are blocked. A successful commit atomically marks the existing Message committed, records Handoff, acknowledges Mail, and closes the Turn without imposing a working-order policy. Validation tokens bind Turn/lease, Goal fence, Message, and Handoff—not a transient Work Record revision; commit injects the final current-Turn record revision.
