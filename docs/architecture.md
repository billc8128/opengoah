...
Handoff remains an event-level control result rather than a current-state projection. It never replaces user-readable prose: every Goal Turn atomically commits a readable Assistant Message followed by its structured Handoff. The message explains what happened; Handoff carries only outcome, evidence, and injected Goal/record revisions.
