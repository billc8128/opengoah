...
Full Runner setup snapshots its credential store and commits it only after Review; Back, Cancel, and setup failures restore the prior store. Profile updates to the global default and workspace config are rollback-safe. Scoped login/logout intentionally commit credentials immediately.
