...
For a general installation, run `goah init --provider <provider> --model <id>`, then use `goah doctor`. Run `goah "<objective>"` to start the resident Supervisor when necessary and enter the interactive CEO; `goah --continue` reconnects later. Secret values use `env:NAME` references and are resolved only inside the resident process. The runner executes locally under the directory containing `goah.config.json`; every Pi Agent receives Pi's native read/write/edit/bash tools, while Git behavior belongs in the selected coding skill. Control state defaults to `~/.goah/state/<project-hash>` and can be relocated with `GOAH_STATE_HOME`. The interactive CLI and lower-level mutation commands use the Supervisor-owned local control socket rather than opening SQLite as a second writer.

Pi tools deliberately retain upstream host-permission semantics. `cwd` resolves relative paths, but absolute paths may read or write anywhere allowed to the user that launched Goah, and Bash inherits that user's command and network access. This includes local Goah state and credential files readable by the same operating-system user. Goah does not show permission prompts or install an implicit sandbox. Deployments that need isolation should provide it outside the reference runner—for example with a custom Runner, process wrapper, container, VM, or restricted system account.
...
Schemas before 28 are intentionally not migrated during pre-1.0 development. Stop the resident daemon and recreate local Goah state before starting this build; `goah doctor` reports the incompatibility directly.
