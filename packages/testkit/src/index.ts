...
export interface FauxStep {
  ...
  effect?: (
    request: RunRequest,
    feedback: Exclude<HandoffValidationResult, { accepted: true }> | null,
  ) => void;
}
...
export class FauxPiDriver implements PiDriver {
  ...
  constructor(
    ...
    runnerSessions: FauxStep[][],
    readonly directory = process.cwd(),
  ) {
  ...
  async createRunnerSession(request: RunRequest): Promise<PiRunnerSession> {
    ...
    let validationFeedback: Exclude<HandoffValidationResult, { accepted: true }> | null = null;
    return {
      step: async () => {
        ...
        return {
          ...
          ...(step.response !== undefined ? { response: { content: step.response } } : {}),
...
export function fauxRunnerWorkerPath(): string {
  return fileURLToPath(new URL("./faux-runner-worker.js", import.meta.url));
}

export interface LedgerConformanceFactory {
  (clock: Clock): Ledger;
}
...
export function assertLedgerConformance(create: LedgerConformanceFactory): void {
  ...
  ledger.putGoal(
    {
      ...
      objective: "test",
      observationMethod,
      verificationMethod: observationMethod,
      owner: "a",
      phase: "active",
      revision: 0,
    },
  ...
  const completionEvidence = ledger.appendEvent({
    streamId: "conformance:goal",
  ...
  ledger.completeGoal(
    {
      goalId: "root",
      revision: 2,
      reason: "conformance observation passed",
      evidence: [completionEvidence.seq],
    },
    "human",
  );
  let reopened = false;
  ...
  const first = ledger.enqueueWake(
    {
      id: "z",
  ...
  ledger.enqueueWake(
    {
      id: "a",
      ...specialistAutomaticTarget("b", "audit"),
      triggerRef: "second",
      status: "queued",
      attempt: 0,
      enqueuedSeq: 0,
  ...
  if (
    ledger.turn(turn.id)?.status !== "completed" ||
    !ledger
      .readStream(`turn:${turn.id}`)
      .some(
        (item) =>
          item.type === "response.committed" &&
          (item.data as { messageItemId?: unknown }).messageItemId === "turn:a:answer",
      )
  )
  ...
  const informational = ledger.appendEvent({
    ...
    ts: clock.now().toISOString(),
    actor: "a",
    type: "transcript.conformance_info",
    data: {},
    ignorable: true,
  });
  ...
  const before = JSON.stringify({
    goals: ledger.goals(),
    wakes: ledger.wakes(),
    triggers: ledger.wakeTriggers("z"),
  });
