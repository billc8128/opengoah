...
export function assertLedgerConformance(create: LedgerConformanceFactory): void {
  ...
  if (claimed?.id !== "z") throw new Error("ledger conformance: wakes are not FIFO");
    ledger.putThread({id:"thread:a",agent:"a",role:"verifier",parentThreadId:null,createdAt:clock.now().toISOString(),updatedAt:clock.now().toISOString()},"supervisor");const turn={id:"turn:a",threadId:"thread:a",triggerKind:"wake" as const,goalId:null,goalRevision:null,status:"in_progress" as const,attempt:1,error:null,startedAt:clock.now().toISOString(),endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"lease",runnerPid:null};ledger.startTurnFromWake("z",turn,clock.now().toISOString());
  if(ledger.wake("z")?.turnId!==turn.id)throw new Error("ledger conformance: consumed Wake did not link its Turn");
    ...
    ledger.putTurnItem({id:"turn:a:answer",turnId:turn.id,ordinal:1,type:"assistant_message",status:"completed",data:{text:"verified"},createdAt:clock.now().toISOString(),completedAt:clock.now().toISOString()},"a");ledger.commitTurnResponse(turn.id,"turn:a:answer",clock.now().toISOString(),"supervisor");if(ledger.turn(turn.id)?.status!=="completed"||!ledger.readStream(`turn:${turn.id}`).some((item)=>item.type==="response.committed"&&(item.data as {messageItemId?:unknown}).messageItemId==="turn:a:answer"))throw new Error("ledger conformance: canonical Assistant response was not committed");
