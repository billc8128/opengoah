...
import { controlStream,goalRoute,goalStream,goalAutomaticTarget,humanInboxRoute,specialistInboxRoute,specialistAutomaticTarget,wakeStream,type AgentRole,type Clock,type EventInput,type GoalSnapshot,type JsonValue,type RunRequest,type Runner,type RunnerCandidateResult,type TurnSnapshot,type WakeSnapshot } from "goah-ledger-contract";
...
test("Human inbox Mail does not create an automatic CEO Wake",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});ledger.putMail({id:"human-mail",to:"ceo",from:"human",level:"decision",...humanInboxRoute(),body:{},readAt:null},"human");const supervisor=new Supervisor(ledger,fauxRunner([]),clock);assert.equal(await supervisor.tick(),null);assert.equal(ledger.wakes().length,0);assert.equal(ledger.unreadMail("ceo").length,1);ledger.close();});
...
test("official Pi agent core worker completes a structured handoff through the process boundary", async () => {
  ...
  const prepared = ledger.events().find((event) => event.type === "request.prepared")?.data as { systemPrompt?:string;tools?: Array<{ name?: string;description?:string }> };
