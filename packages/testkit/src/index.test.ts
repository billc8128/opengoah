...
test("recovery kills the recorded runner before another wake can use its local root", async () => {
  ...
  const handle = runner.prepare({ agent:"worker",execution, turn: { trigger: { kind: "wake", reasons:["test"] },activeGoal:null,goalCommitment:null }, context: {}, now: () => clock.now().toISOString(), emit: () => undefined });
