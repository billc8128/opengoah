...
test("official Pi gives an unbound system Mail Turn a Mail-specific prompt",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const now=clock.now().toISOString();ledger.putThread({id:"ceo-thread",agent:"ceo",parentThreadId:null,createdAt:now,updatedAt:now},"supervisor");ledger.putThread({id:"worker-thread",agent:"worker",parentThreadId:"ceo-thread",createdAt:now,updatedAt:now},"supervisor");ledger.putMail({id:"general-pi",to:"worker",from:"ceo",level:"decision",body:{message:"inspect"},readAt:null},"ceo");const runner=new ProcessRunner({command:process.execPath,args:[piWorkerPath()],env:{GOAH_PI_PROVIDER:"faux",GOAH_PI_MODEL:"faux-goah",GOAH_PI_FAUX_RESPONSE:"seen"}});const wake=await new Supervisor(ledger,runner,clock,{profiles:[{agent:"worker",role:"child",systemPrompt:"Keep the worker identity."}]}).tick();const prepared=ledger.readStream(`turn:${wake!.turnId}`).find((event)=>event.type==="request.prepared")?.data as {systemPrompt?:unknown};const prompt=String(prepared.systemPrompt??"");assert.match(prompt,/Keep the worker identity/);assert.match(prompt,/received durable Mail/);assert.doesNotMatch(prompt,/Own the assigned Child Goal|Respond normally to the Human/);ledger.close();});
