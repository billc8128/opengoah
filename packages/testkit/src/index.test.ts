...
import { controlStream,goalRoute,goalStream,goalAutomaticTarget,humanInboxRoute,specialistInboxRoute,specialistAutomaticTarget,wakeStream,type AgentRole,type Clock,type EventInput,type GoalSnapshot,type JsonValue,type RunRequest,type Runner,type TurnSnapshot,type WakeSnapshot } from "goah-ledger-contract";
...
test("a Human interaction interrupts active automatic CEO work", async () => {
  ...
  const runner: Runner = {
    ...
    prepare: () => {
      ...
      return { pid: null, begin: () => undefined, result: Promise.resolve({ outcome: "completed" as const, response: { content: "你好" } }), terminate: async () => undefined };
    },
...
test("a follow-up steers the active Human turn and both messages commit together", async () => {
  const clock = new SimulatedClock();
  const ledger = createMemoryLedger({ clock });
  const response = Promise.withResolvers<{ outcome: "completed"; response: { content: string } }>();
  let steered = "";
  const runner: Runner = {
    isolation: "process",
    prepare: () => ({
  ...
  response.resolve({ outcome: "completed", response: { content: "revised answer" } });
...
test("cleanup fallback never forwards an unpersisted invalid Runner pid",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});let fallbackPid:number|undefined;const runner:Runner={isolation:"process",prepare:()=>({pid:-1,begin:()=>undefined,result:Promise.resolve({outcome:"completed" as const,response:{content:"ok"}}),terminate:async()=>{throw new Error("cleanup failed")}}),terminateProcess:async(pid)=>{fallbackPid=pid}};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});const accepted=await supervisor.startHumanTurn("hello");await supervisor.waitForTurn(accepted.turnId);assert.equal(fallbackPid,undefined);assert.equal(ledger.turn(accepted.turnId)?.status,"failed");assert.equal(ledger.turn(accepted.turnId)?.runnerPid,null);ledger.close();});
