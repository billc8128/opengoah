...
import { ceoInboxRoute,controlStream,goalRoute,goalStream,goalAutomaticTarget,replayTranscript,specialistInboxRoute,specialistAutomaticTarget,TRANSCRIPT_FORMAT_VERSION,wakeStream,type AgentRole,type Clock,type EventInput,type GoalSnapshot,type JsonValue,type RunRequest,type Runner,type RunnerCandidateResult,type TurnSnapshot,type WakeSnapshot } from "goah-ledger-contract";
...
function queuedWake(id:string,agent="worker",triggerRef=`trigger:${id}`):WakeSnapshot{return{id,...specialistAutomaticTarget(agent,agent==="audit"?"audit":"verifier"),triggerRef,status:"queued",attempt:0,enqueuedSeq:0,claimedAt:null,consumedAt:null,turnId:null};}
function goalWake(id:string,agent:string,goalId:string,triggerRef=`trigger:${id}`):WakeSnapshot{return{id,...goalAutomaticTarget(agent,goalId),triggerRef,status:"queued",attempt:0,enqueuedSeq:0,claimedAt:null,consumedAt:null,turnId:null};}
...
function completedResult(request:RunRequest,text:string,id=`test:assistant:${++completedMessageSequence}`):RunnerCandidateResult{request.emit({type:"message.assistant.completed",data:{message:{id,role:"assistant",content:[{type:"text",text}]},commitState:"committed"}});return{outcome:"completed",finalMessageId:id};}
function completeStoredTurn(ledger:ReturnType<typeof createMemoryLedger>,turnId:string,now:string,actor="supervisor"):void{const id=`${turnId}:answer`;ledger.putTurnItem({id,turnId,ordinal:ledger.turnItems(turnId).length+1,type:"assistant_message",status:"completed",data:{text:"done"},createdAt:now,completedAt:now},actor);ledger.commitTurnResponse(turnId,id,now,"supervisor");}
...
test("an ordinary CEO Turn sees the active Root without acquiring a Goal commitment",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});ledger.putGoal({id:"root",parentId:null,objective:"durable work",observationMethod:"observe",verificationMethod:"verify",owner:"ceo",phase:"active",revision:0},"human");let visible:unknown;let commitment:unknown;const runner:Runner={isolation:"process",prepare:(request)=>{visible=request.turn.activeGoal;commitment=request.turn.goalCommitment;return{pid:null,begin:()=>undefined,result:Promise.resolve(completedResult(request,"你好")),terminate:async()=>undefined}},terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock);const accepted=await supervisor.startHumanTurn("你好");await supervisor.waitForTurn(accepted.turnId);assert.equal((visible as {id?:unknown})?.id,"root");assert.equal(commitment,null);assert.equal(ledger.workRecord("root")?.recordRevision,0);assert.equal(ledger.turnItems(accepted.turnId).some((item)=>item.type==="handoff"),false);ledger.close();});

test("an ordinary Turn rejects a finalMessageId without a readable Assistant Item",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const runner:Runner={isolation:"process",prepare:()=>({pid:null,begin:()=>undefined,result:Promise.resolve({outcome:"completed" as const,finalMessageId:"missing"}),terminate:async()=>undefined}),terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});const accepted=await supervisor.startHumanTurn("hello");await supervisor.waitForTurn(accepted.turnId);assert.equal(ledger.turn(accepted.turnId)?.status,"failed");assert.match(String((ledger.turn(accepted.turnId)?.error as {message?:unknown})?.message),/finalMessageId.*readable Assistant Item/);ledger.close();});

test("an uncommitted Turn rejects a provisional assistant trace",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const runner:Runner={isolation:"process",prepare:(request)=>({pid:null,begin:()=>request.emit({type:"message.assistant.completed",data:{message:{id:"hidden",role:"assistant",content:[{type:"text",text:"hidden"}]},commitState:"provisional"}}),result:Promise.resolve({outcome:"completed" as const,finalMessageId:"hidden"}),terminate:async()=>undefined}),terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});const accepted=await supervisor.startHumanTurn("hello");await supervisor.waitForTurn(accepted.turnId);assert.equal(ledger.turn(accepted.turnId)?.status,"failed");assert.equal(ledger.turnItems(accepted.turnId).some((item)=>item.type==="assistant_message"),false);assert.match(String((ledger.turn(accepted.turnId)?.error as {message?:unknown})?.message),/cannot emit a provisional/);ledger.close();});

test("an ordinary completed result must reference its committed Assistant Item",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const runner:Runner={isolation:"process",prepare:(request)=>({pid:null,begin:()=>request.emit({type:"message.assistant.completed",data:{message:{id:"trace",role:"assistant",content:[{type:"text",text:"trace response"}]},commitState:"committed"}}),result:Promise.resolve({outcome:"completed" as const,finalMessageId:"different"}),terminate:async()=>undefined}),terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});const accepted=await supervisor.startHumanTurn("hello");await supervisor.waitForTurn(accepted.turnId);assert.equal(ledger.turn(accepted.turnId)?.status,"failed");assert.deepEqual(ledger.turnItems(accepted.turnId).filter((item)=>item.type==="assistant_message").map((item)=>item.data),[{text:"trace response"}]);assert.match(String((ledger.turn(accepted.turnId)?.error as {message?:unknown})?.message),/finalMessageId/);ledger.close();});

test("assistant Item normalization remains canonical",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const runner:Runner={isolation:"process",prepare:(request)=>({pid:null,begin:()=>request.emit({type:"message.assistant.completed",data:{message:{id:"normalized",role:"assistant",content:[{type:"text",text:"  first\r\n"},{type:"text",text:"second  \n"}]},commitState:"committed"}}),result:Promise.resolve({outcome:"completed" as const,finalMessageId:"normalized"}),terminate:async()=>undefined}),terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock);const accepted=await supervisor.startHumanTurn("hello");await supervisor.waitForTurn(accepted.turnId);assert.equal(ledger.turn(accepted.turnId)?.status,"completed");assert.deepEqual(ledger.turnItems(accepted.turnId).filter((item)=>item.type==="assistant_message").map((item)=>item.data),[{text:"first\nsecond"}]);ledger.close();});

test("explicit Goal control starts a committed user Turn without an intermediate Wake",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});let commitment:unknown;const runner:Runner={isolation:"process",prepare:(request)=>{commitment=request.turn.goalCommitment;return{pid:null,begin:()=>undefined,result:Promise.resolve({outcome:"abnormal" as const,reason:"captured"}),terminate:async()=>undefined}},terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});const accepted=await supervisor.startHumanGoalTurn("ship the release","root");await supervisor.waitForTurn(accepted.turnId);assert.deepEqual(commitment,{goalId:"root",goalRevision:0});assert.equal(ledger.turn(accepted.turnId)?.triggerKind,"user_message");assert.equal(ledger.wakes().length,0);assert.equal(ledger.turn(accepted.turnId)?.status,"failed");ledger.close();});

test("failed committed user Turns participate in configured recovery",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const runner:Runner={isolation:"process",prepare:()=>({pid:null,begin:()=>undefined,result:Promise.resolve({outcome:"abnormal" as const,reason:"failed"}),terminate:async()=>undefined}),terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0},retryPolicy:{maxAttempts:2,baseDelayMs:1_000}});const accepted=await supervisor.startHumanGoalTurn("ship","root");await supervisor.waitForTurn(accepted.turnId);assert.equal(ledger.schedules().find((schedule)=>schedule.reason===`recovery:${accepted.turnId}`)?.goalId,"root");assert.deepEqual(deriveRecoveryViews(ledger).find((view)=>view.turnId===accepted.turnId),{turnId:accepted.turnId,agent:"ceo",state:"scheduled",actionable:false});ledger.close();});

test("restart recovery requeues an orphaned committed user Turn",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const now=clock.now().toISOString();ledger.putGoal({id:"root",parentId:null,objective:"ship",observationMethod:null,verificationMethod:null,owner:"ceo",phase:"active",revision:0},"human");const thread={id:"ceo",agent:"ceo",role:"ceo" as const,parentThreadId:null,createdAt:now,updatedAt:now};const turn={id:"orphan",threadId:"ceo",triggerKind:"user_message" as const,goalId:"root",goalRevision:0,status:"in_progress" as const,attempt:1,error:null,startedAt:now,endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"lease",runnerPid:null};ledger.admitHumanTurn({thread,turn,messageItem:{id:"orphan-message",turnId:turn.id,ordinal:1,type:"user_message",status:"completed",data:{text:"ship"},createdAt:now,completedAt:now},replaceTurnId:null});const runner:Runner={isolation:"process",prepare:()=>{throw new Error("must not prepare during recover")},terminateProcess:async()=>undefined};await new Supervisor(ledger,runner,clock).recover();assert.equal(ledger.turn("orphan")?.status,"failed");assert.equal(ledger.queuedWakeForAgent("ceo")?.goalId,"root");ledger.close();});
...
test("Supervisor startup rejects Agent profile role drift from the persisted Thread",()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const runner=fauxRunner([]);new Supervisor(ledger,runner,clock,{profiles:[{agent:"quality",role:"verifier"}]}).threadFor("quality");assert.throws(()=>new Supervisor(ledger,runner,clock,{profiles:[{agent:"quality",role:"child"}]}),/conflicts with persisted Thread role verifier/);assert.equal(ledger.threads().find((thread)=>thread.agent==="quality")?.role,"verifier");ledger.close();});

test("Supervisor startup rejects role drift for an active Goal owner before its Thread exists",()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});createWorkerGoal(ledger);ledger.enqueueWake(goalWake("wake","worker","root"),"supervisor");assert.equal(ledger.threads().some((thread)=>thread.agent==="worker"),false);assert.throws(()=>new Supervisor(ledger,fauxRunner([]),clock,{profiles:[{agent:"worker",role:"verifier"}]}),/role conflicts with persisted Child Goal root/);assert.equal(ledger.goal("root")?.phase,"active");assert.equal(ledger.wake("wake")?.status,"queued");ledger.close();});

test("a rejected Human Goal request does not interrupt the active Human Turn",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const pending=Promise.withResolvers<{outcome:"abnormal";reason:string}>();let terminated=false;const runner:Runner={isolation:"process",prepare:()=>({pid:null,begin:()=>undefined,result:pending.promise,terminate:async()=>{terminated=true;pending.resolve({outcome:"abnormal",reason:"cleanup"});}}),terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock);const active=await supervisor.startHumanTurn("working");await new Promise((resolve)=>setImmediate(resolve));supervisor.createRootGoal("won the race","root");await assert.rejects(()=>supervisor.startHumanGoalTurn("replacement","replacement"),/unfinished Root Goal/);assert.equal(terminated,false);assert.equal(ledger.turn(active.turnId)?.status,"in_progress");await supervisor.interruptTurn(active.turnId);await supervisor.waitForTurn(active.turnId);ledger.close();});
...
test("a Human interaction interrupts active automatic CEO work", async () => {
  ...
  const runner: Runner = {
    ...
    prepare: (request) => {
      ...
      return { pid: null, begin: () => undefined, result: Promise.resolve(completedResult(request,"你好")), terminate: async () => undefined };
    },
...
test("CEO send starts a direct user Turn without Human Mail or Wake",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const runner:Runner={isolation:"process",prepare:(request)=>({pid:null,begin:()=>undefined,result:Promise.resolve(completedResult(request,"reply")),terminate:async()=>undefined}),terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock);const sent=await supervisor.sendToCeo({message:"async hello"});while(ledger.turn(sent.turnId)?.status==="in_progress")await new Promise((resolve)=>setImmediate(resolve));assert.equal(ledger.turn(sent.turnId)?.status,"completed");assert.equal(ledger.wakes().length,0);assert.equal(ledger.mailbox().length,0);ledger.close();});
...
test("rejected steering starts a fresh Turn without duplicating a completed user message",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const firstResult=Promise.withResolvers<{outcome:"abnormal";reason:string}>();let prepared=0;const runner:Runner={isolation:"process",prepare:(request)=>{prepared+=1;if(prepared===1)return{pid:null,begin:()=>undefined,result:firstResult.promise,steer:async()=>{throw new Error("no longer accepting steering messages")},terminate:async()=>firstResult.resolve({outcome:"abnormal",reason:"superseded"})};return{pid:null,begin:()=>undefined,result:Promise.resolve(completedResult(request,"fresh answer")),terminate:async()=>undefined}},terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock);const first=await supervisor.startHumanTurn("first");await new Promise((resolve)=>setImmediate(resolve));const second=await supervisor.startHumanTurn("second");assert.equal(second.steered,false);assert.notEqual(second.turnId,first.turnId);while(ledger.turn(second.turnId)?.status==="in_progress")await new Promise((resolve)=>setTimeout(resolve,1));assert.deepEqual(ledger.turnItems(first.turnId).filter((item)=>item.type==="user_message").map((item)=>item.status),["completed","failed"]);assert.deepEqual(ledger.turnItems(second.turnId).filter((item)=>item.type==="user_message").map((item)=>[item.status,item.data]),[["completed",{text:"second"}]]);ledger.close();});

test("steering that loses the Turn completion race starts a fresh Turn",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const firstResult=Promise.withResolvers<RunnerCandidateResult>();const steerResult=Promise.withResolvers<void>();let prepared=0;let firstRequest!:RunRequest;const runner:Runner={isolation:"process",prepare:(request)=>{prepared+=1;if(prepared===1){firstRequest=request;return{pid:null,begin:()=>undefined,result:firstResult.promise,steer:()=>steerResult.promise,terminate:async()=>undefined};}return{pid:null,begin:()=>undefined,result:Promise.resolve(completedResult(request,"fresh answer")),terminate:async()=>undefined}},terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock);const first=await supervisor.startHumanTurn("first");await new Promise((resolve)=>setImmediate(resolve));const follow=supervisor.startHumanTurn("second");firstResult.resolve(completedResult(firstRequest,"stale answer"));while(ledger.turn(first.turnId)?.status==="in_progress")await new Promise((resolve)=>setTimeout(resolve,1));steerResult.resolve();const second=await follow;assert.equal(second.steered,false);assert.notEqual(second.turnId,first.turnId);while(ledger.turn(second.turnId)?.status==="in_progress")await new Promise((resolve)=>setTimeout(resolve,1));assert.equal(ledger.turn(second.turnId)?.status,"completed");assert.deepEqual(ledger.turnItems(second.turnId).filter((item)=>item.type==="user_message").map((item)=>item.data),[{text:"second"}]);ledger.close();});

test("completed Assistant messages materialize one durable reasoning Item",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const runner:Runner={isolation:"process",prepare:(request)=>({pid:null,begin:()=>request.emit({type:"message.assistant.completed",data:{message:{id:"done",role:"assistant",content:[{type:"thinking",thinking:"inspect state"},{type:"text",text:"done"}]},commitState:"committed"}}),result:Promise.resolve({outcome:"completed" as const,finalMessageId:"done"}),terminate:async()=>undefined}),terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock);const accepted=await supervisor.startHumanTurn("test");while(ledger.turn(accepted.turnId)?.status==="in_progress")await new Promise((resolve)=>setTimeout(resolve,1));const reasoning=ledger.turnItems(accepted.turnId).filter((item)=>item.type==="reasoning");assert.equal(reasoning.length,1);assert.deepEqual(reasoning[0]?.data,{text:"inspect state"});ledger.close();});

test("fifty thousand live deltas keep Ledger growth bounded",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const ready=Promise.withResolvers<void>();const finished=Promise.withResolvers<RunnerCandidateResult>();let request!:RunRequest;const runner:Runner={isolation:"process",prepare:(value)=>{request=value;return{pid:null,begin:()=>{request.emitLive!({type:"message.assistant.delta",data:{messageId:"stream",delta:{type:"start"}}});request.emitLive!({type:"message.assistant.delta",data:{messageId:"stream",delta:{type:"thinking_start",contentIndex:0}}});for(let index=0;index<50_000;index+=1)request.emitLive!({type:"message.assistant.delta",data:{messageId:"stream",delta:{type:"thinking_delta",contentIndex:0,delta:"x"}}});request.emitLive!({type:"message.assistant.delta",data:{messageId:"stream",delta:{type:"thinking_end",contentIndex:0}}});ready.resolve();},result:finished.promise,terminate:async()=>undefined};},terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock);const accepted=await supervisor.startHumanTurn("stream a long answer");await ready.promise;const live=supervisor.liveTurnSnapshot(accepted.turnId);assert.equal(live?.thinking.length,50_000);assert.equal(live?.thinkingActive,false);assert.equal(ledger.readStream(`turn:${accepted.turnId}`).some((event)=>event.type==="message.assistant.delta"),false);request.emit({type:"message.assistant.completed",data:{message:{id:"stream",role:"assistant",content:[{type:"thinking",thinking:"x".repeat(50_000)},{type:"text",text:"done"}]},commitState:"committed"}});finished.resolve({outcome:"completed",finalMessageId:"stream"});await supervisor.waitForTurn(accepted.turnId);assert.equal(supervisor.liveTurnSnapshot(accepted.turnId),null);assert.equal(ledger.turnItems(accepted.turnId).filter((item)=>item.type==="reasoning").length,1);assert.ok(ledger.readStream(`turn:${accepted.turnId}`).length<30);ledger.close();});

test("Runner cleanup failure is recorded without leaving the Turn active",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const runner:Runner={isolation:"process",prepare:(request)=>({pid:null,begin:()=>undefined,result:Promise.resolve(completedResult(request,"ok")),terminate:async()=>{throw new Error("cleanup failed")}}),terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock);const accepted=await supervisor.startHumanTurn("hello");while(ledger.turn(accepted.turnId)?.status==="in_progress")await new Promise((resolve)=>setTimeout(resolve,1));assert.equal(ledger.turn(accepted.turnId)?.status,"completed");assert.equal(ledger.readStream(`turn:${accepted.turnId}`).some((event)=>event.type==="runner.cleanup_failed"),true);ledger.close();});

test("cleanup fallback never forwards an unpersisted invalid Runner pid",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});let fallbackPid:number|undefined;const runner:Runner={isolation:"process",prepare:(request)=>({pid:-1,begin:()=>undefined,result:Promise.resolve(completedResult(request,"ok")),terminate:async()=>{throw new Error("cleanup failed")}}),terminateProcess:async(pid)=>{fallbackPid=pid}};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});const accepted=await supervisor.startHumanTurn("hello");await supervisor.waitForTurn(accepted.turnId);assert.equal(fallbackPid,undefined);assert.equal(ledger.turn(accepted.turnId)?.status,"failed");assert.equal(ledger.turn(accepted.turnId)?.runnerPid,null);ledger.close();});
...
test("a follow-up during Turn retry backoff joins the same Turn", async () => {
  ...
  const runner: Runner = { isolation:"process",prepare:(request)=>{ attempt+=1; if(attempt>1) retryContext=String((request.context as {text?:string}).text??""); return {pid:null,begin:()=>undefined,result:Promise.resolve(attempt===1?{outcome:"abnormal" as const,reason:"temporary"}:completedResult(request,"recovered")),terminate:async()=>undefined}; },terminateProcess:async()=>undefined };
...
test("a retry cannot complete by pointing finalMessageId at a prior attempt",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});let attempt=0;const runner:Runner={isolation:"process",prepare:(request)=>{attempt+=1;if(attempt===1)return{pid:null,begin:()=>request.emit({type:"message.assistant.completed",data:{message:{id:"old",role:"assistant",content:[{type:"text",text:"old attempt"}]},commitState:"committed"}}),result:Promise.resolve({outcome:"abnormal" as const,reason:"retry"}),terminate:async()=>undefined};return{pid:null,begin:()=>undefined,result:Promise.resolve({outcome:"completed" as const,finalMessageId:"old"}),terminate:async()=>undefined}},terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:2,baseDelayMs:0}});const accepted=await supervisor.startHumanTurn("test retry fence");await supervisor.waitForTurn(accepted.turnId);assert.equal(ledger.turn(accepted.turnId)?.status,"failed");assert.match(String((ledger.turn(accepted.turnId)?.error as {message?:unknown})?.message),/current attempt/);ledger.close();});

test("a user Turn that becomes committed rebuilds Goal context on Runner retry",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const seen:Array<{binding:unknown;execution:string|null;prompt:string;text:string}>=[];let attempt=0;const runner:Runner={isolation:"process",prepare:(request)=>{attempt+=1;const context=request.context as {systemPrompt?:unknown;text?:unknown};seen.push({binding:request.turn.goalCommitment,execution:request.execution.goalId,prompt:String(context.systemPrompt??""),text:String(context.text??"")});let resolve!:(value:import("goah-ledger-contract").RunnerCandidateResult)=>void;const result=new Promise<import("goah-ledger-contract").RunnerCandidateResult>((done)=>{resolve=done;});return{pid:null,begin:()=>{if(attempt===1)void request.rpc!("goal.create",{id:"retry-goal",objective:"finish the durable goal"}).then(()=>resolve({outcome:"abnormal",reason:"retry after commitment"}),error=>resolve({outcome:"abnormal",reason:String(error)}));else resolve({outcome:"abnormal",reason:"captured rebound context"});},result,terminate:async()=>undefined};},terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:2,baseDelayMs:0}});const accepted=await supervisor.startHumanTurn("finish this goal");while(ledger.turn(accepted.turnId)?.status==="in_progress")await new Promise((resolve)=>setImmediate(resolve));assert.equal(seen[0]?.execution,null);assert.equal(seen[1]?.execution,"retry-goal");assert.deepEqual(seen[1]?.binding,{goalId:"retry-goal",goalRevision:0});assert.match(seen[1]?.prompt??"",/sole operating interface/);assert.match(seen[1]?.text??"",/# Shared Work Record Index[\s\S]*retry-goal/);ledger.close();});

test("accepted Handoff remains valid when the Agent updates its Work Record afterward",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});createWorkerGoal(ledger);const evidence=ledger.appendEvent({...event("worker","observed",{}),ts:clock.now().toISOString()});const runner:Runner={isolation:"process",prepare:(request)=>{let resolve!:(value:import("goah-ledger-contract").RunnerCandidateResult)=>void;const result=new Promise<import("goah-ledger-contract").RunnerCandidateResult>((done)=>{resolve=done;});return{pid:null,begin:()=>{void(async()=>{const initial=await request.rpc!("work_record.read",{goalId:"root"}) as unknown as {recordRevision:number};await request.rpc!("work_record.update",{expectedRevision:initial.recordRevision,content:"# Current State\n\nValidated before final refinement.\n",reason:"first update",evidence:[evidence.seq]});const validation=await request.rpc!("goal.handoff.validate",{handoff:{outcome:"progress",evidence:[evidence.seq]},candidateMessageId:"final",candidateMessage:"Goal work completed."} as unknown as JsonValue) as unknown as import("goah-ledger-contract").HandoffValidationResult;if(!validation.accepted)throw new Error("validation failed");await request.rpc!("work_record.update",{expectedRevision:1,content:"# Current State\n\nRefined after Handoff validation.\n",reason:"final refinement",evidence:[evidence.seq]});request.emit({type:"message.assistant.completed",data:{message:{id:"final",role:"assistant",content:[{type:"text",text:"Goal work completed."}],stopReason:"toolUse"},commitState:"provisional"}});resolve({outcome:"completed",finalMessageId:"final",handoff:{validationAttemptId:validation.attemptId,validationToken:validation.token,handoff:{outcome:"progress",evidence:[evidence.seq]}}});})().catch((error)=>resolve({outcome:"abnormal",reason:String(error)}));},result,terminate:async()=>undefined};},terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});supervisor.planWake("worker",clock.now().toISOString(),"validate before refinement");const wake=await supervisor.tick();assert.equal(ledger.turn(wake!.turnId!)?.status,"completed");assert.equal((ledger.lastEvent("worker","handoff.recorded")?.data as {recordRevision?:unknown}).recordRevision,2);ledger.close();});

test("a committed result cannot point at a different Assistant Item than Handoff validation",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});createWorkerGoal(ledger);const evidence=ledger.latestEvent()!.seq;const runner:Runner={isolation:"process",prepare:(request)=>{let resolve!:(value:import("goah-ledger-contract").RunnerCandidateResult)=>void;const result=new Promise<import("goah-ledger-contract").RunnerCandidateResult>((done)=>{resolve=done;});return{pid:null,begin:()=>{void(async()=>{await request.rpc!("work_record.update",{expectedRevision:0,content:"# Current State\n\nworked\n",reason:"work",evidence:[evidence]});const validation=await request.rpc!("goal.handoff.validate",{handoff:{outcome:"progress",evidence:[evidence]},candidateMessageId:"validated",candidateMessage:"validated message"} as unknown as JsonValue) as unknown as import("goah-ledger-contract").HandoffValidationResult;if(!validation.accepted)throw new Error("validation failed");request.emit({type:"message.assistant.completed",data:{message:{id:"validated",role:"assistant",content:[{type:"text",text:"validated message"}]},commitState:"provisional"}});resolve({outcome:"completed",finalMessageId:"different",handoff:{validationAttemptId:validation.attemptId,validationToken:validation.token,handoff:{outcome:"progress",evidence:[evidence]}}});})().catch((error)=>resolve({outcome:"abnormal",reason:String(error)}));},result,terminate:async()=>undefined};},terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});supervisor.planWake("worker",clock.now().toISOString(),"mismatch");const wake=await supervisor.tick();assert.equal(ledger.turn(wake!.turnId!)?.status,"failed");assert.match(String((ledger.turn(wake!.turnId!)?.error as {message?:unknown})?.message),/finalMessageId/);assert.equal(ledger.lastEvent("worker","handoff.recorded"),null);ledger.close();});

test("a later rejected Handoff invalidates every older accepted token",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});createWorkerGoal(ledger);const evidence=ledger.appendEvent({...event("worker","observed",{}),ts:clock.now().toISOString()});let attempts:number[]=[];const runner:Runner={isolation:"process",prepare:(request)=>{let resolve!:(value:import("goah-ledger-contract").RunnerCandidateResult)=>void;const result=new Promise<import("goah-ledger-contract").RunnerCandidateResult>((done)=>{resolve=done;});return{pid:null,begin:()=>{void(async()=>{await request.rpc!("work_record.update",{expectedRevision:0,content:"# Current State\n\nready\n",reason:"ready",evidence:[evidence.seq]});const first=await request.rpc!("goal.handoff.validate",{handoff:{outcome:"progress",evidence:[evidence.seq]},candidateMessageId:"first",candidateMessage:"first accepted"} as unknown as JsonValue) as unknown as import("goah-ledger-contract").HandoffValidationResult;const second=await request.rpc!("goal.handoff.validate",{handoff:{outcome:"progress",evidence:[999999]},candidateMessageId:"second",candidateMessage:"second rejected"} as unknown as JsonValue) as unknown as import("goah-ledger-contract").HandoffValidationResult;attempts=[first.attemptId,second.attemptId];if(!first.accepted)throw new Error("first validation failed");request.emit({type:"message.assistant.completed",data:{message:{id:"first",role:"assistant",content:[{type:"text",text:"first accepted"}],stopReason:"toolUse"},commitState:"provisional"}});resolve({outcome:"completed",finalMessageId:"first",handoff:{validationAttemptId:first.attemptId,validationToken:first.token,handoff:{outcome:"progress",evidence:[evidence.seq]}}});})().catch((error)=>resolve({outcome:"abnormal",reason:String(error)}));},result,terminate:async()=>undefined};},terminateProcess:async()=>undefined};const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});supervisor.planWake("worker",clock.now().toISOString(),"invalidate old token");const wake=await supervisor.tick();assert.deepEqual(attempts,[1,2]);assert.equal(ledger.turn(wake!.turnId!)?.status,"failed");assert.equal(ledger.lastEvent("worker","handoff.recorded"),null);ledger.close();});
...
test("a nonrepairing Runner cannot hand off without updating its Work Record", async () => {
  ...
  const runner: Runner = {
    isolation: "process",
    prepare: (request) => {let resolve!:(value:import("goah-ledger-contract").RunnerCandidateResult)=>void;const result=new Promise<import("goah-ledger-contract").RunnerCandidateResult>((done)=>{resolve=done;});return{pid:null,begin:()=>{void request.rpc!("goal.handoff.validate",{handoff:{outcome:"progress",evidence:[1]},candidateMessageId:"missing-record",candidateMessage:"Goal progress recorded."}).then((value)=>resolve({outcome:"abnormal",reason:JSON.stringify(value)}));},result,terminate:async()=>undefined};},
    terminateProcess: async () => undefined,
  };
  const supervisor = new Supervisor(ledger, runner, clock);
  createWorkerGoal(ledger);
...
test("an unbound Human Turn cannot mutate Goal organization", async () => {
  ...
  assert.match(String((ledger.turn(accepted.turnId)?.error as { message?: string }).message), /requires a Goal commitment/);
  ledger.close();
});
...
test("automatic Goal rounds advance from the Work Record timeline", async () => {
  ...
  const commitments = ledger.eventsSince(0, ["run.admitted"]).map((event) => (event.data as { goalCommitment?: { goalId:string } }).goalCommitment?.goalId);
...
test("a queued Mail Wake adopts the latest Goal revision at admission",async()=>{
  ...
  let binding:unknown;
  const runner:Runner={isolation:"process",prepare:(request)=>{binding=request.turn.goalCommitment;return{pid:null,begin:()=>undefined,result:Promise.resolve({outcome:"abnormal" as const,reason:"observed latest revision"}),terminate:async()=>undefined}},terminateProcess:async()=>undefined};
  const wake=await new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}}).tick();
  assert.equal(wake?.status,"consumed");
...
test("recovery kills the recorded runner before another wake can use its local root", async () => {
  ...
  const execution=testTurn(ledger,"quality","running-turn",{role:"verifier",leaseUntil:new Date(clock.now().getTime()+100).toISOString(),leaseToken:"lease"});
  const handle = runner.prepare({ agent:"quality",execution, turn: { trigger: { kind: "wake", reasons:["test"] },activeGoal:null,goalCommitment:null }, context: {}, now: () => clock.now().toISOString(), emit: () => undefined });
  ledger.attachTurnProcess(execution.id,"lease",handle.pid!);
  handle.begin();
  await waitFor(() => existsSync(join(repo, "running.txt")));
  clock.advance(200);
  await new Supervisor(ledger, runner, clock,{profiles:[{agent:"quality",role:"verifier"}]}).recover();
...
test("restart recovery requeues an orphaned Specialist Turn from its persisted role",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});const turn=testTurn(ledger,"quality","specialist-orphan",{role:"verifier"});const supervisor=new Supervisor(ledger,fauxRunner([]),clock,{profiles:[{agent:"quality",role:"verifier"}]});await supervisor.recover();assert.equal(ledger.turn(turn.id)?.status,"failed");const wake=ledger.queuedWakeForAgent("quality");assert.equal(wake?.targetKind,"specialist");assert.equal(wake?.specialistRole,"verifier");assert.equal(wake?.triggerRef,`recovery:${turn.id}`);ledger.close();});
...
test("verification plane records findings and reports calibrated quality", async () => {
  ...
  const verifyTurn=testTurn(ledger,"worker","w",{goalId:"root",goalRevision:0});ledger.finishTurn(verifyTurn.id,"failed",{message:"verification target"},clock.now().toISOString(),"supervisor");
...
test("official Pi agent core worker completes a structured handoff through the process boundary", async () => {
  ...
  assert.equal((started?.data as { formatVersion?: number }).formatVersion, TRANSCRIPT_FORMAT_VERSION);
  assert.equal(ledger.events().some((event) => event.type === "request.prepared"), true);
  const rawPrepared=ledger.events().find((event)=>event.type==="request.prepared")!.data as Record<string,unknown>;
  assert.equal("systemPrompt" in rawPrepared||"activeContext" in rawPrepared||"messages" in rawPrepared||"tools" in rawPrepared,false);
  const components=ledger.events().filter((event)=>event.type==="request.component");
  assert.equal(components.filter((event)=>(event.data as {kind?:unknown}).kind==="system_prompt").length,1);
  assert.equal(components.filter((event)=>(event.data as {kind?:unknown}).kind==="active_context").length,1);
  assert.equal(components.filter((event)=>(event.data as {kind?:unknown}).kind==="toolset").length,1);
  const prepared = replayTranscript(ledger.readStream(`turn:${completed!.turnId}`)).lastRequest! as { systemPrompt:string;tools: Array<{ name?: string;description?:string }> };
  ...
  assert.match(prepared.systemPrompt??"",/Planning, partial progress, decomposition, or scheduling a future Wake are not by themselves reasons to stop/);
  assert.match(prepared.tools?.find((tool)=>tool.name==="handoff")?.description??"",/genuine control boundary/);
  assert.match(prepared.tools?.find((tool)=>tool.name==="schedule_wake")?.description??"",/Scheduling does not end the current Turn/);
...
test("tool-only Handoff reuses the prior readable canonical Assistant Item",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});createWorkerGoal(ledger);const runner=new ProcessRunner({command:process.execPath,args:[piWorkerPath()],env:{GOAH_PI_PROVIDER:"faux",GOAH_PI_MODEL:"faux-goah",GOAH_PI_FAUX_TOOL_ONLY_HANDOFF:"1"}});const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});supervisor.planWake("worker",clock.now().toISOString(),"separate prose and Handoff");const wake=await supervisor.tick();const turn=ledger.turn(wake!.turnId!)!;assert.equal(turn.status,"completed");const response=ledger.readStream(`turn:${turn.id}`).find((event)=>event.type==="response.committed")?.data as {messageItemId?:unknown;text?:unknown};const item=ledger.turnItems(turn.id).find((candidate)=>candidate.id===response.messageItemId);assert.equal((item?.data as {text?:unknown})?.text,"Work Record updated; the next wake is scheduled.");assert.equal(response.text,"Work Record updated; the next wake is scheduled.");assert.equal(ledger.lastEvent("worker","handoff.recorded")!==null,true);ledger.close();});
...
test("official Pi worker exposes organization tools only to the CEO profile", async () => {
  ...
  const prepared = replayTranscript(ledger.readStream(`turn:${turn.id}`)).lastRequest as { tools?: Array<{ name?: string }> };
...
test("Agent Mail cannot masquerade as a CEO or specialist inbox route",()=>{const ledger=createMemoryLedger();assert.throws(()=>ledger.putMail({id:"to-child",to:"worker",from:"ceo",level:"decision",...ceoInboxRoute(),body:{},readAt:null},"ceo"),/CEO inbox/);assert.throws(()=>ledger.putMail({id:"to-ceo",to:"ceo",from:"worker",level:"decision",...ceoInboxRoute(),body:{},readAt:null},"worker"),/CEO inbox/);ledger.close();});
...
test("Verifier retains the explicit uncommitted specialist execution path",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});ledger.enqueueWake(queuedWake("verify","verifier","verification:turn"),"supervisor");let trigger="";const runner:Runner={isolation:"process",prepare:(request)=>{trigger=request.turn.trigger.kind;return{pid:null,begin:()=>undefined,result:Promise.resolve(completedResult(request,"verified")),terminate:async()=>undefined}},terminateProcess:async()=>undefined};const wake=await new Supervisor(ledger,runner,clock,{profiles:[{agent:"verifier",role:"verifier"}]}).tick();assert.equal(wake?.status,"consumed");assert.equal(trigger,"wake");assert.equal(ledger.turn(wake!.turnId!)?.goalId,null);ledger.close();});

test("Specialist inbox Mail opens only the matching Specialist Turn",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});ledger.putMail({id:"verify-mail",to:"verifier",from:"supervisor",level:"decision",...specialistInboxRoute("verifier"),body:{request:"inspect"},readAt:null},"supervisor");let context="";const runner:Runner={isolation:"process",prepare:(request)=>{context=JSON.stringify(request.context);return{pid:null,begin:()=>undefined,result:Promise.resolve(completedResult(request,"done")),terminate:async()=>undefined}},terminateProcess:async()=>undefined};const wake=await new Supervisor(ledger,runner,clock,{profiles:[{agent:"verifier",role:"verifier"}]}).tick();assert.equal(wake?.targetKind,"specialist");assert.match(context,/inspect/);assert.equal(ledger.unreadMail("verifier").length,0);ledger.close();});

test("official Pi applies a custom Child prompt only inside its committed Turn",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});createWorkerGoal(ledger);const runner=new ProcessRunner({command:process.execPath,args:[piWorkerPath()],env:{GOAH_PI_PROVIDER:"faux",GOAH_PI_MODEL:"faux-goah"}});const supervisor=new Supervisor(ledger,runner,clock,{profiles:[{agent:"worker",role:"child",systemPrompt:"Keep the configured worker identity."}],turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});supervisor.planWake("worker",clock.now().toISOString(),"inspect");const wake=await supervisor.tick();const prompt=replayTranscript(ledger.readStream(`turn:${wake!.turnId}`)).lastRequest?.systemPrompt??"";assert.match(prompt,/Keep the configured worker identity/);assert.match(prompt,/Goal commitment starts a sustained work session/);assert.match(prompt,/another useful action can be completed/);assert.doesNotMatch(prompt,/context rather than an assignment|has no Goal commitment/);ledger.close();});

test("Child mail.send cannot address Human",async()=>{const clock=new SimulatedClock();const ledger=createMemoryLedger({clock});ledger.putGoal({id:"root",parentId:null,objective:"root",observationMethod:"o",verificationMethod:"v",owner:"ceo",phase:"active",revision:0},"human");ledger.putGoal({id:"child",parentId:"root",objective:"child",observationMethod:"o",verificationMethod:"v",owner:"worker",phase:"active",revision:0},"ceo");const runner=fauxRunner([{rpc:{method:"mail.send",params:{to:"human",goalId:"root",level:"decision",body:{message:"bypass"}}}},{handoff:{response:{content:"Goal work recorded."},handoff:{outcome:"progress",evidence:[1]}}}]);const supervisor=new Supervisor(ledger,runner,clock,{turnRetryPolicy:{maxAttempts:1,baseDelayMs:0}});ledger.enqueueWake(goalWake("child","worker","child"),"supervisor");const wake=await supervisor.tick();assert.equal(ledger.turn(wake!.turnId!)?.status,"failed");assert.match(String(ledger.turn(wake!.turnId!)?.error&&JSON.stringify(ledger.turn(wake!.turnId!)?.error)),/Agents only/);ledger.close();});
...
test("one root goal forms a two-agent organization and returns completion control to the human", async () => {
  ...
  const byTrigger = {
    ...
    "ceo|mail:": [
      { rpc: { method: "goal.complete", params: { goalId: "market", revision: 0, reason: "material handoff satisfies the observation method", evidence: ["$WORK_RECORD_SEQ:market"] } } },
      { rpc: { method: "goal.complete", params: { goalId: "operations", revision: 0, reason: "material handoff satisfies the observation method", evidence: ["$WORK_RECORD_SEQ:operations"] } } },
      { handoff: { response:{content:"Both Child Goals are complete. Please review the consolidated results and decide whether to complete the Root Goal."},handoff: { outcome:"waiting", evidence:[1] } } },
    ],
  };
  const byAgent = {
  ...
  const ceoThread=ledger.threads().find((thread)=>thread.agent==="ceo")!;const latestCeoMessage=ledger.turns(ceoThread.id).flatMap((turn)=>ledger.turnItems(turn.id)).findLast((item)=>item.type==="assistant_message");assert.match(String((latestCeoMessage?.data as {text?:unknown})?.text??""),/decide whether to complete the Root Goal/);
...
test("process verifier model runs on official Pi core and records findings", async () => {
  ...
  const verifyTurn=testTurn(ledger,"worker","verify-wake",{goalId:"root",goalRevision:0});ledger.finishTurn(verifyTurn.id,"failed",{message:"verification target"},clock.now().toISOString(),"supervisor");
...
function testTurn(ledger:ReturnType<typeof createMemoryLedger>,agent:string,id:string,options:Partial<TurnSnapshot>&{role?:AgentRole}={}):TurnSnapshot{const now=new Date().toISOString();const{role:requestedRole,...patch}=options;const role=requestedRole??(patch.goalId?(agent==="ceo"?"ceo":"child"):"verifier");let ceo=ledger.threads().find((candidate)=>candidate.agent==="ceo");if(agent!=="ceo"&&!ceo){ceo={id:"thread:ceo",agent:"ceo",role:"ceo",parentThreadId:null,createdAt:now,updatedAt:now};ledger.putThread(ceo,"supervisor");}let thread=ledger.threads().find((candidate)=>candidate.agent===agent);if(!thread){thread={id:`thread:${agent}`,agent,role,parentThreadId:agent==="ceo"?null:ceo!.id,createdAt:now,updatedAt:now};ledger.putThread(thread,"supervisor");}if(thread.role!==role)throw new Error(`test Thread role mismatch for ${agent}`);const target=patch.goalId?goalAutomaticTarget(agent,patch.goalId):specialistAutomaticTarget(agent,role as "verifier"|"audit");const wakeId=`test-wake:${id}`;ledger.enqueueWake({id:wakeId,...target,triggerRef:`test:${id}`,status:"queued",attempt:0,enqueuedSeq:0,claimedAt:null,consumedAt:null,turnId:null},"supervisor");const claimed=ledger.claimNextWake(now);if(claimed?.id!==wakeId)throw new Error("test failed to claim Turn Wake");const commitment=patch.goalId?{goalId:patch.goalId,goalRevision:patch.goalRevision??ledger.goal(patch.goalId)!.revision}:{goalId:null,goalRevision:null};const turn={id,threadId:thread.id,triggerKind:"wake" as const,status:"in_progress" as const,attempt:1,error:null,startedAt:now,endedAt:null,leaseUntil:new Date(Date.now()+60_000).toISOString(),leaseToken:"test-lease",runnerPid:null,...patch,...commitment} as TurnSnapshot;ledger.startTurnFromWake(wakeId,turn,now);return turn;}
