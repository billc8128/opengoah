...
await runProcessWorker(async (request, emit, rpc): Promise<RunnerCandidateResult> => {
  ...
  const byAgent = JSON.parse(process.env.GOAH_FAUX_STEPS_BY_AGENT ?? "{}") as Record<
    string,
    WorkerStep[]
  >;
  const byTrigger = JSON.parse(process.env.GOAH_FAUX_STEPS_BY_TRIGGER ?? "{}") as Record<
    string,
    WorkerStep[]
  >;
  const triggerSteps = Object.entries(byTrigger).find(([selector]) => {
    ...
    return (
      ...
      request.sourceWakeTriggers?.some((trigger) => trigger.triggerRef.startsWith(prefix))
    );
  })?.[1];
  const steps =
    triggerSteps ??
    byAgent[request.agent] ??
    (JSON.parse(process.env.GOAH_FAUX_STEPS ?? "[]") as WorkerStep[]);
  ...
  for (const step of steps) {
      ...
      emit({
        ...
        data: {
          message: {
            id: finalMessageId,
            role: "assistant",
            content: [{ type: "text", text: step.response }],
          },
    ...
    if (step.handoff) {
      ...
      if (goalCommitment && !recordUpdated) {
        ...
        await rpc("work_record.update", {
          expectedRevision: Number(record.recordRevision ?? 0),
          content: handoffRecord(draft.handoff, request.execution.id),
      ...
      const validation = (await rpc("goal.handoff.validate", {
        handoff: draft.handoff,
        candidateMessageId: messageItemId,
        candidateMessage: draft.response.content,
      ...
      emit({
        ...
        data: {
          message: {
            id: messageItemId,
            role: "assistant",
            content: [{ type: "text", text: draft.response.content }],
            stopReason: "toolUse",
          },
...
function resolveParams(value: JsonValue, context: JsonValue): JsonValue {
  ...
  const visit = (item: JsonValue): JsonValue => {
    ...
    if (typeof item === "string" && item.startsWith("$WORK_RECORD_SEQ:")) {
      const goalId = item.slice("$WORK_RECORD_SEQ:".length);
      const record = sharedWorkRecords(context).find((candidate) => candidate.goalId === goalId);
      if (!record) throw new Error(`missing Work Record in faux context: ${goalId}`);
      return record.lastEventSeq;
    }
...
function commitmentFrom(value: JsonValue): { goalId: string; goalRevision: number } | undefined {
  if (
    ...
    typeof value.goalCommitment !== "object" ||
    Array.isArray(value.goalCommitment)
  )
    return undefined;
  const binding = value.goalCommitment as Record<string, JsonValue>;
  return typeof binding.goalId === "string" && typeof binding.goalRevision === "number"
