...
export interface RequestComponent {
  hash: string;
  kind: RequestComponentKind;
  content: JsonValue;
}
export interface PreparedRequestReferences {
  provider: string;
  model: string;
...
export interface ReplayedTranscript {
  ...
  openToolCalls: Array<{ callId: string; name: string; arguments: JsonValue }>;
...
export function isTranscriptEvent(
  event: Pick<EventRecord, "type">,
): event is EventRecord & { type: TranscriptEventType } {
  return TRANSCRIPT_TYPES.has(event.type as TranscriptEventType);
}

export class TranscriptFormatUnsupportedError extends Error {
  constructor(
    readonly foundVersion: number,
    readonly supportedVersion = TRANSCRIPT_FORMAT_VERSION,
  ) {
    super(
      `transcript format ${foundVersion} is unsupported by this harness (supports ${supportedVersion}); upgrade the harness`,
    );
...
export function upgradeTranscriptEvents(source: readonly EventRecord[]): EventRecord[] {
  ...
  if (starts.length !== 1)
    throw new TranscriptCorruptionError(
      "turn transcript stream contains multiple transcript.started events",
    );
...
export function replayTranscript(events: readonly EventRecord[]): ReplayedTranscript {
  ...
  for (const event of upgraded) {
    ...
    if (event.type === "request.component") {
      ...
      if (requestComponentHash(component.kind, component.content) !== component.hash)
        throw new TranscriptCorruptionError(`request component hash mismatch: ${component.hash}`);
      const existing = components.get(component.hash);
      if (existing && JSON.stringify(existing) !== JSON.stringify(component))
        throw new TranscriptCorruptionError(`request component hash collision: ${component.hash}`);
      components.set(component.hash, component);
    } else if (event.type === "message.user" || event.type === "message.assistant.completed")
      messages.push(data.message as TranscriptMessage);
    else if (event.type === "tool.called") {
      const call = {
        callId: String(data.callId),
        name: String(data.name),
        arguments: (data.arguments ?? null) as JsonValue,
      };
      ...
      calls.delete(callId);
      messages.push({
        id: String(data.messageId ?? `tool:${callId}`),
        role: "tool",
        toolCallId: callId,
        content: (data.result ?? null) as JsonValue,
      });
    } else if (event.type === "context.compacted") {
      const replaced = new Set(
        Array.isArray(data.replacedMessageIds) ? data.replacedMessageIds.map(String) : [],
      );
      const kept = messages.filter((message) => !replaced.has(message.id));
      messages.splice(0, messages.length, ...kept);
      messages.unshift({
        id: String(data.summaryMessageId),
        role: "user",
        content: String(data.summary),
      });
    } else if (event.type === "request.prepared")
      lastRequest = materializeRequest(
        event.data as unknown as PreparedRequestReferences,
        components,
      );
...
function materializeRequest(
  request: PreparedRequestReferences,
  components: ReadonlyMap<string, RequestComponent>,
): RequestSnapshot {
  const content = (hash: string, kind: RequestComponentKind): JsonValue => {
    ...
    if (!component || component.kind !== kind)
      throw new TranscriptCorruptionError(`request references missing ${kind} component: ${hash}`);
  ...
  if (
    ...
    typeof activeContext !== "string" ||
    !Array.isArray(tools)
  )
    throw new TranscriptCorruptionError("request component has an invalid shape");
  return {
    provider: request.provider,
    model: request.model,
    systemPrompt,
    activeContext,
    messages: request.messageHashes.map((hash) => content(hash, "message")),
    tools,
...
export function interruptedTranscriptEvents(
  ...
  const repairs: EventInput[] = replayed.openToolCalls.map((call) => ({
    streamId,
    ts,
    actor,
    type: "tool.completed",
    data: {
      callId: call.callId,
      messageId: `repair:${call.callId}`,
      result: {
        outcome: "unknown",
        synthetic: true,
        reason: "runner interrupted before a durable result",
      },
    },
  }));
  repairs.push({
    ...
    data: { reason: "runner interrupted" },
  });
  return repairs;
}
...
function assertContiguous(events: readonly EventRecord[]): void {
  ...
  for (const event of events) {
    if (event.streamSeq !== expected)
      throw new TranscriptCorruptionError(
        `turn transcript stream gap: expected ${expected}, got ${event.streamSeq}`,
      );
    expected += 1;
  }
}
function isTranscriptNamespace(type: string): boolean {
  return ["transcript.", "request.", "turn.", "message.", "tool.", "context."].some((prefix) =>
