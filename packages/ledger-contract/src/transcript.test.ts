...
import { interruptedTranscriptEvents, replayTranscript, requestComponentHash, TRANSCRIPT_FORMAT_VERSION, TranscriptCorruptionError, TranscriptEventUnsupportedError, TranscriptFormatUnsupportedError, type EventRecord, type JsonValue, type RequestComponentKind } from "./index.js";
...
function component(streamSeq:number,kind:RequestComponentKind,content:JsonValue):EventRecord{const hash=requestComponentHash(kind,content);return event(streamSeq,"request.component",{hash,kind,content});}
...
test("Turn transcript replay derives messages from normalized facts and applies compaction without deleting history", () => {
  const events = [
    ...
    event(11, "request.prepared", { provider: "faux", model: "m", systemPromptHash:requestComponentHash("system_prompt","s"),activeContextHash:requestComponentHash("active_context","ctx"),messageHashes:[],toolsetHash:requestComponentHash("toolset",[]),modelConfig: {}, sourceSeqs: [2] }),
...
test("interrupted transcript repair preserves an unknown tool outcome", () => {
  const events = [event(1, "transcript.started", {formatVersion:TRANSCRIPT_FORMAT_VERSION}), event(2, "tool.called", { callId: "t1", name: "publish", arguments: {} })];
...
test("Turn transcript replay rejects a gap in the stream", () => {
  assert.throws(() => replayTranscript([event(1, "transcript.started", {formatVersion:TRANSCRIPT_FORMAT_VERSION}), event(3, "transcript.completed", {})]), /stream gap/);
});
...
test("request components are content-addressed and required for materialization", () => {
  ...
  const missing=event(2,"request.prepared",{provider:"faux",model:"m",systemPromptHash:"missing",activeContextHash:"missing",messageHashes:[],toolsetHash:"missing",modelConfig:{},sourceSeqs:[]});
  assert.throws(()=>replayTranscript([started,missing]),/missing system_prompt component/);
  const corrupt=event(2,"request.component",{hash:"wrong",kind:"system_prompt",content:"s"});
...
test("future formats and unknown required events fail closed while informational events may be skipped", () => {
  ...
  assert.throws(() => replayTranscript([event(1, "transcript.started", { formatVersion: TRANSCRIPT_FORMAT_VERSION - 1 })]), TranscriptCorruptionError);
  const started = event(1, "transcript.started", { formatVersion: TRANSCRIPT_FORMAT_VERSION });
