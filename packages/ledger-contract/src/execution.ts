...
export function goalAutomaticTarget(
  agent: string,
  goalId: string,
): Extract<AutomaticTarget, { targetKind: "goal" }> {
  return { targetKind: "goal", agent, goalId, specialistRole: null };
}
export function specialistAutomaticTarget(
  agent: string,
  role: SpecialistRole,
): Extract<AutomaticTarget, { targetKind: "specialist" }> {
  return { targetKind: "specialist", agent, goalId: null, specialistRole: role };
}
export function goalRoute(goalId: string): Extract<MailRoute, { routeKind: "goal" }> {
  return { routeKind: "goal", goalId, specialistRole: null };
}
export function ceoInboxRoute(): Extract<MailRoute, { routeKind: "ceo_inbox" }> {
  return { routeKind: "ceo_inbox", goalId: null, specialistRole: null };
}
export function specialistInboxRoute(
  role: SpecialistRole,
): Extract<MailRoute, { routeKind: "specialist_inbox" }> {
...
export function noGoalCommitment(): { goalId: null; goalRevision: null } {
  return { goalId: null, goalRevision: null };
}
export function goalCommitment(goalId: string, goalRevision: number): GoalCommitment {
...
export type GoalChangeOperation =
  "create" | "revise" | "pause" | "resume" | "block" | "complete" | "reassign";
...
export type ScheduleSnapshot = AutomaticTarget & {
  id: string;
  nextWakeAt: string;
  reason: string;
  setBy: string;
...
export interface RunnerLiveEvent {
  type: "message.assistant.delta";
  data: { messageId: string; delta: AssistantLiveDelta };
}
export interface TurnLiveSnapshot {
  revision: number;
  messageId: string;
  text: string;
  thinking: string;
...
export interface RunnerSetupProgress {
  current: number;
  total: number;
}
...
export interface RunnerConfigurator {
  ...
  ): Promise<RunnerSetupTransaction>;
...
export interface HumanTurnAdmissionRequest {
...
export interface HumanTurnAdmissionResult {
  turn: TurnSnapshot;
...
const wakeTransitions: Record<WakeStatus, readonly WakeStatus[]> = {
  ...
  claimed: ["queued", "consumed", "cancelled"],
  consumed: [],
  cancelled: [],
};
const goalTransitions: Record<GoalPhase, readonly GoalPhase[]> = {
...
export function assertGoalTransition(from: GoalPhase, to: GoalPhase): void {
...
export function assertHandoff(value: Handoff): void {
  ...
  if (
    !value.goalId.trim() ||
    !Number.isInteger(value.goalRevision) ||
    value.goalRevision < 0 ||
    !Number.isInteger(value.recordRevision) ||
    value.recordRevision < 1
  )
    throw new Error("invalid Goal handoff");
}
export function assertAgentHandoff(value: AgentHandoff): void {
  if (
    !["progress", "waiting", "blocked", "completion_proposed"].includes(value.outcome) ||
    !Array.isArray(value.evidence) ||
    value.evidence.length === 0
  )
    throw new Error("invalid Agent handoff");
}
export function assertTurnOutput(value: TurnOutput): void {
  if (
    !Number.isInteger(value.validationAttemptId) ||
    value.validationAttemptId <= 0 ||
    typeof value.validationToken !== "string" ||
    !value.validationToken.trim()
  )
