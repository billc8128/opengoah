...
- Added runner-agnostic Handoff validation issues/tokens and internal Runner control RPC; tokens omit transient Work Record revision, `TurnOutput` carries only the accepted token plus structured Handoff, and the duplicate `consumeWake` transition has been removed.
