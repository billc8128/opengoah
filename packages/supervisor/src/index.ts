...
export class Supervisor {
  ...
  async #startHumanTurn(message:string):Promise<{threadId:string;turnId:string;steered:boolean}>{
    ...
    const context: TurnContext = { source: { kind: "human" } };const deliveredMail=this.#contextMail("ceo");
  ...
  #preemptGoalTurn(goalId:string,reason:string):void{const turn=this.ledger.turns().find((candidate)=>candidate.status==="in_progress"&&candidate.goalId===goalId);if(!turn)return;this.ledger.finishTurn(turn.id,"interrupted",{message:reason},this.#now(),"supervisor");const handle=this.#handles.get(turn.id);if(handle)void this.#terminateHandle(turn.id,handle).finally(()=>this.#handles.delete(turn.id));}
  ...
  async #claimNextWake(): Promise<WakeSnapshot | null> {
    ...
    try {
      ...
      for (const mail of this.ledger.triggeringMail()) {
        ...
        const body=mail.body&&typeof mail.body==="object"&&!Array.isArray(mail.body)?mail.body as Record<string,JsonValue>:{};const goalId=typeof body.goalId==="string"?body.goalId:null;const goal=goalId?this.ledger.goal(goalId):null;const base=`mail:${mail.id}`;const related=this.ledger.wakeTriggersForAgent(mail.to).map((trigger)=>({trigger,attempt:mailDeliveryAttempt(trigger.triggerRef,base)})).filter((entry):entry is {trigger:WakeTriggerSnapshot;attempt:number}=>entry.attempt!==null);const pending=related.filter((entry)=>entry.trigger.status==="pending").sort((a,b)=>b.attempt-a.attempt)[0];const next=related.reduce((highest,entry)=>Math.max(highest,entry.attempt),-1)+1;const trigger=pending?.trigger.triggerRef??(next===0?base:`${base}@redelivery:${next}`);this.#enqueueTrigger(mail.to,trigger,goal&&goal.owner===mail.to&&goal.phase==="active"?{goalId:goal.id,goalRevision:goal.revision}:undefined);
      }
  ...
  async collectMetricNow(goalId: string): Promise<MetricEvaluation> {
    ...
    if(schedule.goalId){const goal=this.ledger.goal(schedule.goalId);if(!goal||goal.owner!==schedule.agent||goal.phase!=="active"||goal.revision!==schedule.goalRevision){this.ledger.supersedeSchedule(schedule.id,this.#now());return null;}}
    else if(!this.ledger.goalsForOwner(schedule.agent).some((goal)=>goal.phase==="active"||goal.phase==="blocked")){this.ledger.cancelSchedule(schedule.id,this.#now());return null;}
    const wake:WakeSnapshot={id:`wake:${schedule.id}`,agent:schedule.agent,triggerRef:`${schedule.id}@${schedule.nextWakeAt}`,status:"queued",attempt:0,enqueuedSeq:0,claimedAt:null,consumedAt:null,turnId:null,...(schedule.goalId?{goalId:schedule.goalId,goalRevision:schedule.goalRevision!}:{})};
    ...
    if (queued) {
      this.ledger.addWakeTrigger(queued.id,triggerRef,"supervisor");
      return queued;
    }
  ...
  #loadContext(wake: WakeSnapshot, turn: TurnContext,mail:MailSnapshot[],wakeTriggers:WakeTriggerSnapshot[]): JsonValue {
    ...
    const recoveryIds=[...new Set(wakeTriggers.flatMap((trigger)=>{const parsed=parseRecoveryTrigger(trigger.triggerRef);return parsed?[parsed.turnId]:[]}))];
    ...
    const active = composeActiveContext({ role, capabilities, systemPrompt: profile.systemPrompt ?? defaultRolePrompt(role), wake,wakeTriggers, goals, mail, lastHandoff: handoff, teamHandoffs, team: role === "ceo" ? this.teamList() : [], revisionWarnings, recoveryEvents, workingMemory });
  ...
  #goalRevisionWarning(goal: GoalSnapshot): string[] {
    ...
    const definitionChange=(event:EventRecord)=>event.type==="goal.changed"&&typeof event.data==="object"&&event.data!==null&&!Array.isArray(event.data)&&["create","revise"].includes(String((event.data as Record<string,JsonValue>).operation));const goalSeq=this.ledger.readStream(goalStream(goal.id)).filter(definitionChange).at(-1)?.seq??0;let ancestor=this.#goal(goal.parentId);let newest={goal:ancestor,seq:this.ledger.readStream(goalStream(ancestor.id)).filter(definitionChange).at(-1)?.seq??0};while(ancestor.parentId!==null){ancestor=this.#goal(ancestor.parentId);const seq=this.ledger.readStream(goalStream(ancestor.id)).filter(definitionChange).at(-1)?.seq??0;if(seq>newest.seq)newest={goal:ancestor,seq};}return newest.seq>goalSeq?[`Goal ${goal.id} predates ancestor ${newest.goal.id} revision ${newest.goal.revision}; revise its objective and methods before continuing work.`]:[];
  }
  ...
  #contextMail(agent:string):MailSnapshot[]{const selected:MailSnapshot[]=[];let used=0;for(const mail of this.ledger.unreadMail(agent)){const cost=Math.min(JSON.stringify(mail.body).length,2_000)+128;if(selected.length>=20||selected.length>0&&used+cost>this.#memoryTailChars)break;selected.push(mail);used+=cost;}return selected;}
