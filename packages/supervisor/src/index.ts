...
export class Supervisor {
  ...
  async #executeTurn(initial: TurnSnapshot, agent: string, turnContext: TurnContext, contextFactory: () => JsonValue, sourceWake: WakeSnapshot | null = null, deliveredMailIds: string[] = [], recordRevisionAtStart = -1, sourceWakeTriggers: WakeTriggerSnapshot[] = []): Promise<void> {
    while (this.ledger.turn(initial.id)?.status === "in_progress") {
      ...
      try {
        ...
        const response=normalizeAssistantText(result.response.content);if(!response)throw new Error("completed Turn requires a readable assistant response");
        if(turnContext.goalCommitment){if(!result.handoff)throw new Error("committed Turn requires Handoff");this.#commitTurnGoalWork(current.id,agent,turnContext,response,result.handoff,sourceWake?.id??null,deliveredMailIds,recordRevisionAtStart);return;}
        if(result.handoff)throw new Error("uncommitted Turn cannot finish with Handoff");const latestMessage=this.ledger.turnItems(current.id).findLast((item)=>item.type==="assistant_message"&&item.status==="completed");if(latestMessage){const traced=normalizeAssistantText(String((latestMessage.data as {text?:unknown}).text??""));if(traced!==response)throw new Error("completed Turn response does not match its latest assistant trace");if(this.#assistantCommitState(current.id,traced)!=="committed")throw new Error("uncommitted Turn requires a committed assistant trace");}else this.#appendTurnItem(current.id,"assistant_message",{text:response},agent);this.ledger.finishTurn(current.id,"completed",null,this.#now(),"supervisor",deliveredMailIds);return;
  ...
  #assistantCommitState(turnId:string,text:string):"committed"|"provisional"|null{const canonical=normalizeAssistantText(text);const event=this.ledger.readStream(`turn:${turnId}`).findLast((candidate)=>{if(candidate.type!=="message.assistant.completed"||!candidate.data||typeof candidate.data!=="object"||Array.isArray(candidate.data))return false;const message=(candidate.data as {message?:{content?:unknown}}).message;return messageTextContent(message?.content)===canonical;});if(!event||!event.data||typeof event.data!=="object"||Array.isArray(event.data))return null;const state=(event.data as {commitState?:unknown}).commitState;return state==="committed"||state==="provisional"?state:null;}
  ...
  #recordTurnTrace(turnId: string, leaseToken: string, type: string, data: JsonValue, actor = "ceo"): void {
    ...
    if (type === "message.assistant.completed") {const value=data as {message?:{content?:unknown}};const text=messageTextContent(value.message?.content);if(text)this.#appendTurnItem(turnId,"assistant_message",{text},actor);}
  ...
  #validateHandoffDraft(turnId:string,attemptId:number,agent:string,execution:TurnSnapshot,context:TurnContext,input:Record<string,JsonValue>):HandoffValidationResult{
    ...
    const message=normalizeAssistantText(request.candidateMessage)||normalizeAssistantText(String((existingMessage?.data as {text?:unknown}|undefined)?.text??""));
    if(!message)issues.push({code:"message_missing",message:"This committed Turn needs a readable assistant message before it can finish."});
