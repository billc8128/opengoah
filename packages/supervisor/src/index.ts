...
export class RunnerRouter implements Runner {
  ...
  prepare(request: Parameters<Runner["prepare"]>[0]): RunnerHandle {
    ...
    if (handle.pid!==null&&Number.isInteger(handle.pid)&&handle.pid>0) this.#byPid.set(handle.pid, runner);
    void handle.result.then(()=>{if(handle.pid!==null&&Number.isInteger(handle.pid)&&handle.pid>0)this.#byPid.delete(handle.pid);},()=>{if(handle.pid!==null&&Number.isInteger(handle.pid)&&handle.pid>0)this.#byPid.delete(handle.pid);});
    return handle;
  }
...
export class Supervisor {
  ...
  constructor(readonly ledger: Ledger, runner: Runner, readonly clock: Clock, options: SupervisorOptions = {}) {
    ...
    for(const thread of this.ledger.threads()){const configured=this.#profiles.get(thread.agent)?.role??(thread.agent==="ceo"?"ceo":"child");if(configured!==thread.role)throw new Error(`Agent ${thread.agent} role conflicts with persisted Thread role ${thread.role}`);}
    for(const goal of this.ledger.goals().filter((candidate)=>candidate.phase!=="complete")){if(!this.#validGoalOwner(goal.owner,goal))throw new Error(`Agent ${goal.owner} role conflicts with persisted ${goal.parentId===null?"Root":"Child"} Goal ${goal.id}`);}
  ...
  threadFor(agent = "ceo"): import("goah-ledger-contract").ThreadSnapshot {
    ...
    if (existing) {if(existing.role!==role)throw new Error(`Agent ${agent} role changed from ${existing.role} to ${role}`);return existing;}
    const now = this.#now(); const thread = { id: randomUUID(), agent, role, parentThreadId, createdAt: now, updatedAt: now };
  ...
  async startHumanGoalTurn(objective:string,id:string=randomUUID()):Promise<{threadId:string;turnId:string;steered:false;goal:GoalSnapshot}>{
    if(!objective.trim())throw new Error("root objective is required");const previous=this.#humanTail;let release!:()=>void;this.#humanTail=new Promise<void>((resolve)=>{release=resolve;});await previous;try{const goal:GoalSnapshot={id,parentId:null,objective,observationMethod:null,verificationMethod:null,owner:"ceo",phase:"active",revision:0};const accepted=await this.#startHumanTurn(`/goal ${objective}`,goal);return{...accepted,steered:false,goal:this.#goal(id)};}finally{release();}
  }
  async #startHumanTurn(message:string,newRoot:GoalSnapshot|null=null):Promise<{threadId:string;turnId:string;steered:boolean}>{
    if (!message.trim()) throw new Error("message is required");
    const existingThread=this.ledger.threads().find((candidate)=>candidate.agent==="ceo");const now=this.#now();const thread=existingThread?{...existingThread,updatedAt:now}:{id:randomUUID(),agent:"ceo",role:"ceo" as const,parentThreadId:null,createdAt:now,updatedAt:now};const active=existingThread?this.ledger.activeTurn(existingThread.id):null;
    if (active) {
      if(active.triggerKind==="user_message"&&!newRoot){const handle=this.#handles.get(active.id);if(handle?.steer){const item=this.#appendTurnItem(active.id,"user_message",{text:message},"human",randomUUID(),"in_progress");try{await handle.steer(message);const stored=this.ledger.turnItems(active.id).find((candidate)=>candidate.id===item.id);if(this.ledger.turn(active.id)?.status==="in_progress"&&stored?.status==="in_progress"){this.ledger.putTurnItem({...stored,status:"completed",completedAt:this.#now()},"human");return{threadId:thread.id,turnId:active.id,steered:true};}}catch{const stored=this.ledger.turnItems(active.id).find((candidate)=>candidate.id===item.id);if(this.ledger.turn(active.id)?.status==="in_progress"&&stored?.status==="in_progress")this.ledger.putTurnItem({...stored,status:"failed",completedAt:this.#now()},"human");}}else if(!handle){this.#appendTurnItem(active.id,"user_message",{text:message},"human");return{threadId:thread.id,turnId:active.id,steered:true};}}
    }
    const replacement=this.ledger.activeTurn(thread.id);const turnStartedAt=this.#now();const turn:TurnSnapshot={id:randomUUID(),threadId:thread.id,triggerKind:"user_message",...(newRoot?goalCommitment(newRoot.id,newRoot.revision):noGoalCommitment()),status:"in_progress",attempt:1,error:null,startedAt:turnStartedAt,endedAt:null,leaseUntil:new Date(this.clock.now().getTime()+this.#leaseMs).toISOString(),leaseToken:randomUUID(),runnerPid:null,runnerProfileId:this.#runnerProfileId("ceo")};const messageItem:TurnItemSnapshot={id:randomUUID(),turnId:turn.id,ordinal:1,type:"user_message",status:"completed",data:{text:message},createdAt:turnStartedAt,completedAt:turnStartedAt};
    const admitted=this.ledger.admitHumanTurn({thread:{...thread,updatedAt:turnStartedAt},turn,messageItem,replaceTurnId:replacement?.id??null,...(newRoot?{rootGoal:newRoot}:{})});const interruptedGoalId=replacement?.triggerKind==="wake"?replacement.goalId:null;if(interruptedGoalId){const goal=this.ledger.goal(interruptedGoalId);if(goal?.phase==="active")this.#enqueueTrigger(goal.owner,`human-interrupted:${replacement!.id}`,{goalId:goal.id});}
    const context:TurnContext={trigger:{kind:"user_message"},activeGoal:newRoot??this.currentRoot(),goalCommitment:newRoot?{goalId:newRoot.id,goalRevision:newRoot.revision}:null};const deliveredMail=this.#contextMail("ceo",newRoot?.id??null);const recordRevisionAtStart=newRoot?this.ledger.workRecord(newRoot.id)?.recordRevision??-1:-1;
    await this.#activateAdmittedHumanTurn(admitted.turn,admitted.replacedTurn,context,deliveredMail,recordRevisionAtStart);return{threadId:thread.id,turnId:turn.id,steered:false};
  }
  ...
  async #executeTurn(initial: TurnSnapshot, agent: string, turnContext: TurnContext, contextFactory: () => JsonValue, sourceWake: WakeSnapshot | null = null, deliveredMailIds: string[] = [], recordRevisionAtStart = -1, sourceWakeTriggers: WakeTriggerSnapshot[] = []): Promise<void> {
    while (this.ledger.turn(initial.id)?.status === "in_progress") {
      ...
      try {
        ...
        if(turnContext.goalCommitment){if(!result.handoff)throw new Error("committed Turn requires Handoff");this.#commitTurnGoalWork(current.id,agent,turnContext,responseItem,result.handoff,sourceWake?.id??null,deliveredMailIds,recordRevisionAtStart);return;}
        if(result.handoff)throw new Error("uncommitted Turn cannot finish with Handoff");if(this.#assistantCommitState(current.id,responseItem.id)!=="committed")throw new Error("uncommitted Turn requires a committed assistant trace");this.ledger.commitTurnResponse(current.id,responseItem.id,this.#now(),"supervisor",deliveredMailIds);return;
      } catch(error){if(renewal)clearInterval(renewal);if(handle&&processClean)processClean=await this.#terminateHandle(initial.id,handle);this.#handles.delete(initial.id);let current=this.ledger.turn(initial.id);if(processClean&&current?.status==="in_progress"&&current.runnerPid!==null){this.ledger.releaseTurnProcess(initial.id,"supervisor");current=this.ledger.turn(initial.id);}if(current?.status==="in_progress"){this.#failTurn(current.id,error instanceof Error?error.message:String(error));this.#scheduleTurnRecovery(sourceWake,sourceWakeTriggers,current.id,agent);}return;}
    }
  }
  #trackExecution(initial:TurnSnapshot,agent:string,turnContext:TurnContext,contextFactory:()=>JsonValue,sourceWake:WakeSnapshot|null=null,deliveredMailIds:string[]=[],recordRevisionAtStart=-1,sourceWakeTriggers:WakeTriggerSnapshot[]=[]):Promise<void>{const running=this.#executeTurn(initial,agent,turnContext,contextFactory,sourceWake,deliveredMailIds,recordRevisionAtStart,sourceWakeTriggers);this.#executions.set(initial.id,running);this.#agentExecutions.set(agent,running);void running.finally(()=>{if(this.#executions.get(initial.id)===running)this.#executions.delete(initial.id);if(this.#agentExecutions.get(agent)===running)this.#agentExecutions.delete(agent);this.#requestComponents.delete(initial.id);});return running;}
  async #activateAdmittedHumanTurn(turn:TurnSnapshot,replaced:TurnSnapshot|null,context:TurnContext,mail:MailSnapshot[],recordRevisionAtStart:number):Promise<void>{if(replaced){this.#clearHandoffState(replaced.id);if(!await this.#cleanupTerminalTurn(replaced)){this.#failTurn(turn.id,"previous Runner cleanup did not complete");this.#scheduleTurnRecovery(null,[],turn.id,"ceo");return;}}else await this.#awaitAgentExecution("ceo");for(const fence of this.ledger.turns(turn.threadId).filter((candidate)=>candidate.status!=="in_progress"&&candidate.runnerPid!==null)){if(!await this.#cleanupTerminalTurn(fence)){this.#failTurn(turn.id,"previous Runner cleanup did not complete");this.#scheduleTurnRecovery(null,[],turn.id,"ceo");return;}}const current=this.ledger.turn(turn.id);if(current?.status!=="in_progress")return;const activated={...current,leaseUntil:new Date(this.clock.now().getTime()+this.#leaseMs).toISOString()};this.ledger.putTurn(activated,"supervisor");this.#trackExecution(activated,"ceo",context,()=>this.#humanContext(turn.id,"ceo",mail),null,mail.map((item)=>item.id),recordRevisionAtStart);}
  async #cleanupTerminalTurn(turn:TurnSnapshot):Promise<boolean>{const handle=this.#handles.get(turn.id);let cleaned=true;try{if(handle)cleaned=await this.#terminateHandle(turn.id,handle);else if(turn.runnerPid)await this.runner.terminateProcess(turn.runnerPid,turn.runnerProfileId);}catch(error){this.#recordCleanupFailure(turn.id,error);cleaned=false;}finally{this.#handles.delete(turn.id);}if(!cleaned)return false;await this.#executions.get(turn.id);const stored=this.ledger.turn(turn.id);if(stored&&stored.status!=="in_progress"&&stored.runnerPid!==null){try{this.ledger.releaseTurnProcess(turn.id,"supervisor");}catch(error){this.#recordCleanupFailure(turn.id,error);return false;}}return true;}
  ...
  #humanContext(turnId: string, agent: string,mail:MailSnapshot[]): JsonValue {
    ...
    if(turn.goalId!==null&&turn.goalRevision!==null){
      const goal=this.#goal(turn.goalId);const context:TurnContext={trigger:{kind:"user_message"},activeGoal:goal,goalCommitment:{goalId:turn.goalId,goalRevision:turn.goalRevision}};
      const base=this.#loadContext({...goalAutomaticTarget(agent,turn.goalId),attempt:turn.attempt},context,mail,[]) as Record<string,JsonValue>;
    ...
    const activeGoal=this.currentRoot();const context:TurnContext={trigger:{kind:"user_message"},activeGoal,goalCommitment:null};
    ...
    return {text:[...(recent.length?[`# Recent conversation\n\n${recent.join("\n")}`]:[]),...goalText,...(incoming.length?[`# Incoming\n\n${incoming.join("\n")}`]:[]),`# Current Turn\n\n${current}`].join("\n\n"),sourceSeqs,activeGoal,capabilities:profile.capabilities??defaultCapabilities(profile.role),systemPrompt:profile.systemPrompt??defaultTurnPrompt(profile.role,agent,context),...(runnerProfile?{runnerProfile}:{})} as unknown as JsonValue;
  }
  ...
  #assistantMessageEvent(turnId:string,messageItemId:string):EventRecord|undefined{return this.ledger.readStream(`turn:${turnId}`).findLast((candidate)=>{if(candidate.type!=="message.assistant.completed"||!candidate.data||typeof candidate.data!=="object"||Array.isArray(candidate.data))return false;const message=(candidate.data as {message?:{id?:unknown}}).message;return message?.id===messageItemId;});}
  ...
  #scheduleTurnRecovery(sourceWake:WakeSnapshot|null,triggers:WakeTriggerSnapshot[],turnId:string,agent:string):void{const turn=this.ledger.turn(turnId);if(!turn)return;const role=this.#role(agent);const goal=turn.goalId?this.ledger.goal(turn.goalId):null;if(turn.goalId&&(!goal||goal.phase!=="active"||goal.owner!==agent)||!turn.goalId&&(turn.triggerKind!=="wake"||role!=="verifier"&&role!=="audit"))return;const prior=Math.max(0,...triggers.map((trigger)=>parseRecoveryTrigger(trigger.triggerRef)?.attempt??0));const next=prior+1;if(next<this.#retryPolicy.maxAttempts){const delay=this.#retryPolicy.baseDelayMs*2**prior;const target=goal?goalAutomaticTarget(agent,goal.id):specialistAutomaticTarget(agent,role as "verifier"|"audit");this.ledger.putSchedule({id:recoveryScheduleId(turnId,next),...target,nextWakeAt:new Date(this.clock.now().getTime()+delay).toISOString(),reason:recoveryRef(turnId),setBy:"supervisor",status:"pending",resolvedAt:null},"supervisor",sourceWake?.id);}else if(role==="child"&&goal?.parentId){const parent=this.ledger.goal(goal.parentId);if(parent?.phase==="active")this.#enqueueTrigger(parent.owner,childRetryExhaustedRef(turnId),{goalId:parent.id});}}
  ...
  #recordTurnTrace(turnId: string, leaseToken: string, type: string, data: JsonValue, actor = "ceo"): void {
    let componentToRecord:{components:Map<string,string>;hash:string;encoded:string}|null=null;
    ...
    if(type==="request.component"){const hash=data&&typeof data==="object"&&!Array.isArray(data)?String((data as {hash?:unknown}).hash??""):"";if(!hash)throw new Error("request component is missing a hash");let components=this.#requestComponents.get(turnId);if(!components){components=new Map(this.ledger.readStream(`turn:${turnId}`).filter((event)=>event.type==="request.component").map((event)=>[String((event.data as {hash?:unknown}).hash??""),JSON.stringify(event.data)]));this.#requestComponents.set(turnId,components);}const encoded=JSON.stringify(data);const existing=components.get(hash);if(existing){if(existing!==encoded)throw new Error(`request component hash collision: ${hash}`);return;}componentToRecord={components,hash,encoded};}
    ...
    if(componentToRecord)componentToRecord.components.set(componentToRecord.hash,componentToRecord.encoded);
  ...
  async reassignGoal(request: ReassignmentRequest, actor = "ceo", wakeId?: string): Promise<ReassignmentResult> {const goal=this.#goal(request.goalId);if(!this.#validGoalOwner(request.newOwner,{...goal,owner:request.newOwner}))throw new Error("Child Goal owner must use a Child Agent profile");const thread=this.ledger.threads().find((candidate)=>candidate.agent===goal.owner);const active=thread?this.ledger.activeTurn(thread.id):null;const gate=active?Promise.withResolvers<void>():null;if(gate)this.#goalExecutionBarriers.set(goal.id,gate.promise);try{const result=this.ledger.commitReassignment(request,actor,wakeId);if(active?.goalId===goal.id&&this.ledger.turn(active.id)?.status==="in_progress")await this.#interruptTurn(active.id,"Goal reassigned to another Agent","supervisor");return result;}finally{gate?.resolve();if(gate&&this.#goalExecutionBarriers.get(goal.id)===gate.promise)this.#goalExecutionBarriers.delete(goal.id);} }
  ...
  planWake(agent: string, at: string, reason: string, setBy = agent,target?:{goalId:string}): WakeSnapshot | null {
    ...
    const executionTarget=binding?goalAutomaticTarget(agent,binding.goalId):specialistAutomaticTarget(agent,role as "verifier"|"audit");const schedule: ScheduleSnapshot = { id: `schedule:${agent}${binding?`:${binding.goalId}`:""}:${randomUUID()}`, ...executionTarget, nextWakeAt, reason, setBy,status:"pending",resolvedAt:null };
    this.ledger.putSchedule(schedule, setBy);
    return nextWakeAt <= this.#now() ? this.#enqueueSchedule(schedule) : null;
  }

  async stopAgentWake(agent:string,goalId?:string):Promise<WakeSnapshot|null>{const role=this.#role(agent);if(goalId){const goal=this.#goal(goalId);if(!this.#validGoalOwner(agent,goal))throw new Error("Goal stop target is not owned by the Agent");}const target=role==="verifier"||role==="audit"?null:goalId?{goalId}:this.#singleActiveGoalTarget(agent);if(role!=="verifier"&&role!=="audit"&&!target)throw new Error("Goal stop requires an explicit Goal target");const wake=this.ledger.wakes().find((item)=>item.agent===agent&&(item.status==="queued"||item.status==="claimed")&&(target?item.targetKind==="goal"&&item.goalId===target.goalId:item.targetKind==="specialist"));if(wake)this.ledger.cancelWake(wake.id,this.#now());const thread=this.ledger.threads().find((item)=>item.agent===agent);const turn=thread?this.ledger.activeTurn(thread.id):null;if(turn&&(target?turn.goalId===target.goalId:turn.goalId===null&&turn.triggerKind==="wake"))await this.interruptTurn(turn.id);return wake?this.#wake(wake.id):null;}
  ...
  async recover(): Promise<void> {
    ...
    for(const turn of this.ledger.turns().filter((candidate)=>candidate.status!=="in_progress"&&candidate.runnerPid!==null)){await this.runner.terminateProcess(turn.runnerPid!,turn.runnerProfileId);this.ledger.releaseTurnProcess(turn.id,"supervisor");}
    for (const turn of this.ledger.turns().filter((candidate) => candidate.status === "in_progress"&&!this.#handles.has(candidate.id))) { if (turn.runnerPid){await this.runner.terminateProcess(turn.runnerPid,turn.runnerProfileId);this.ledger.releaseTurnProcess(turn.id,"supervisor");} this.#failTurn(turn.id,"orphaned Runner ownership recovered after Supervisor restart"); const thread=this.ledger.thread(turn.threadId);if(!thread)continue;if(turn.goalId){const goal=this.ledger.goal(turn.goalId);if(goal?.phase==="active"&&goal.owner===thread.agent)this.#enqueueTrigger(thread.agent,recoveryRef(turn.id),{goalId:goal.id});}else if(turn.triggerKind==="wake"&&(thread.role==="verifier"||thread.role==="audit"))this.#enqueueTrigger(thread.agent,recoveryRef(turn.id)); }
  }
  async tick(): Promise<WakeSnapshot | null> {
    ...
    try {
      ...
      if(currentWake.targetKind==="goal")await this.#goalExecutionBarriers.get(currentWake.goalId);
      ...
      if(currentWake.targetKind==="goal"){
        ...
        if(!goal||goal.owner!==currentWake.agent||goal.phase!=="active"||!this.#validGoalOwner(currentWake.agent,goal)){this.ledger.cancelWake(currentWake.id,this.#now());return this.#wake(currentWake.id);}
      }
      ...
      if(!this.#validExecution(currentWake.agent,turnContext)){this.ledger.cancelWake(currentWake.id,this.#now());return this.#wake(currentWake.id);}
      const thread=this.threadFor(currentWake.agent);const now=this.#now();const leaseToken=randomUUID();
      const commitment=turnContext.goalCommitment??noGoalCommitment();
      const execution:TurnSnapshot={id:randomUUID(),threadId:thread.id,triggerKind:"wake",...commitment,status:"in_progress",attempt:1,error:null,startedAt:now,endedAt:null,leaseUntil:new Date(this.clock.now().getTime()+this.#leaseMs).toISOString(),leaseToken,runnerPid:null,runnerProfileId:this.#runnerProfileId(currentWake.agent)};
      this.ledger.putThread({ ...thread,updatedAt:this.#now() },"supervisor");
      this.ledger.startTurnFromWake(currentWake.id,execution,now);createdTurnId=execution.id;const deliveredMail=this.#contextMail(currentWake.agent,currentWake.targetKind==="goal"?currentWake.goalId:null);const deliveredMailIds=deliveredMail.map((mail)=>mail.id);this.ledger.appendEvent({streamId:`turn:${execution.id}`,ts:now,actor:"supervisor",type:"run.admitted",data:{...turnContext,wakeTriggers} as unknown as JsonValue,ignorable:true});const revision=turnContext.goalCommitment?this.ledger.workRecord(turnContext.goalCommitment.goalId)?.recordRevision??-1:-1;await this.#trackExecution(execution,currentWake.agent,turnContext,()=>this.#goalContext(currentWake,turnContext,execution.id,deliveredMail,wakeTriggers),currentWake,deliveredMailIds,revision,wakeTriggers);
  ...
  async runAvailable(concurrency = 4, maxWakes = 100): Promise<WakeSnapshot[]> {
    ...
    if(schedule.targetKind==="goal"){const goal=this.ledger.goal(schedule.goalId);if(!goal||goal.owner!==schedule.agent||goal.phase!=="active"||!this.#validGoalOwner(schedule.agent,goal)){this.ledger.supersedeSchedule(schedule.id,this.#now());return null;}}
    else if(!["verifier","audit"].includes(this.#role(schedule.agent))){this.ledger.cancelSchedule(schedule.id,this.#now());return null;}
    const wake:WakeSnapshot={id:`wake:${schedule.id}`,...(schedule.targetKind==="goal"?goalAutomaticTarget(schedule.agent,schedule.goalId):specialistAutomaticTarget(schedule.agent,schedule.specialistRole)),triggerRef:`${schedule.id}@${schedule.nextWakeAt}`,status:"queued",attempt:0,enqueuedSeq:0,claimedAt:null,consumedAt:null,turnId:null};
    ...
    if(target){const goal=this.ledger.goal(target.goalId);if(!goal||goal.owner!==agent||goal.phase!=="active"||!this.#validGoalOwner(agent,goal))return null;binding=goalAutomaticTarget(agent,target.goalId);}
    else {const role=this.#role(agent);if(role==="verifier"||role==="audit")binding=specialistAutomaticTarget(agent,role);else return null;}
    const sameBinding=(wake:WakeSnapshot)=>wake.targetKind===binding.targetKind&&wake.goalId===binding.goalId&&wake.specialistRole===binding.specialistRole;
    const exact = this.ledger.wakeByTrigger(agent, triggerRef);
    if (exact) {if(!sameBinding(exact))throw new Error("Wake trigger was reused with a different automatic target");return exact;}
  ...
  #role(agent:string):AgentRole{return this.ledger.threads().find((thread)=>thread.agent===agent)?.role??this.#profiles.get(agent)?.role??(agent==="ceo"?"ceo":"child");}
  ...
  #validExecution(agent:string,turn:TurnContext):boolean{const role=this.#role(agent);if(turn.goalCommitment){const goal=this.ledger.goal(turn.goalCommitment.goalId);return Boolean(goal&&this.#validGoalOwner(agent,goal)&&(role==="ceo"||role==="child"));}if(turn.trigger.kind==="user_message")return agent==="ceo"&&role==="ceo";return role==="verifier"||role==="audit";}
  #commitTurnGoalWork(turnId:string,agent:string,turn:TurnContext,responseItem:TurnItemSnapshot,raw:TurnOutput,sourceWakeId:string|null,mailIds:string[],revisionAtStart:number):void{const binding=turn.goalCommitment!;const goal=this.#goal(binding.goalId);if(goal.revision!==binding.goalRevision)throw new Error("Goal revision changed during the Turn");const record=this.ledger.workRecord(goal.id);if(!record||record.recordRevision<=revisionAtStart||record.updatedInTurn!==turnId||record.goalRevision!==goal.revision)throw new Error("committed Turn must update its Work Record before handoff");assertTurnOutput(raw);const validation=this.#handoffValidations.get(raw.validationToken);const execution=this.ledger.turn(turnId);const response=normalizeAssistantText(String((responseItem.data as {text?:unknown}).text??""));if(!validation||!execution||validation.turnId!==turnId||validation.attemptId!==raw.validationAttemptId||this.#handoffAttemptSeq.get(turnId)!==raw.validationAttemptId||validation.agent!==agent||validation.attempt!==execution.attempt||validation.leaseToken!==execution.leaseToken||validation.goalId!==goal.id||validation.goalRevision!==goal.revision||validation.messageItemId!==responseItem.id||validation.message!==response||validation.handoff.outcome!==raw.handoff.outcome||!isSameNumbers(validation.handoff.evidence,raw.handoff.evidence))throw new Error("Handoff validation token is stale or does not match this Turn result");const output=this.#committedTurnOutput(raw,binding);assertHandoff(output.handoff);const now=this.#now();const item:TurnItemSnapshot={id:randomUUID(),turnId,ordinal:this.ledger.turnItems(turnId).length+1,type:"handoff",status:"completed",data:output.handoff as unknown as JsonValue,createdAt:now,completedAt:now};this.ledger.commitHandoff({agent,turnId,sourceWakeId,mailIds,ts:now,output,responseItemId:responseItem.id,item});this.#clearHandoffState(turnId);}

  #committedTurnOutput(output: TurnOutput, binding: NonNullable<TurnContext["goalCommitment"]>): CommittedTurnOutput {
  ...
  #turnContext(wake:WakeSnapshot,triggers=this.ledger.wakeTriggers(wake.id).filter((trigger)=>trigger.status==="pending")):TurnContext{
    ...
    const goal=this.#goal(wake.goalId);return{trigger,activeGoal:goal,goalCommitment:{goalId:goal.id,goalRevision:goal.revision}};
  }

  #loadContext(wake: AutomaticTarget&{attempt:number}, turn: TurnContext,mail:MailSnapshot[],wakeTriggers:WakeTriggerSnapshot[]): JsonValue {
    const profile:AgentProfile = this.#profiles.get(wake.agent) ?? { agent: wake.agent, role: "child" };
    ...
    return { ...active, text: `${active.text}\n\n${workText}`, activeGoal:turn.activeGoal,goalCommitment:turn.goalCommitment,sourceSeqs: [...new Set([...active.sourceSeqs,...this.#mailSourceSeqs(mail), ...records.map((record) => record.lastEventSeq)])].sort((a, b) => a - b), workRecord: currentRecord, sharedWorkRecords: records, ...(runnerProfile ? { runnerProfile } : {}) } as unknown as JsonValue;
  }
  ...
  #contextMail(agent:string,goalId:string|null):MailSnapshot[]{const selected:MailSnapshot[]=[];let used=0;const role=this.#role(agent);const matches=(mail:MailSnapshot)=>goalId?mail.routeKind==="goal"&&mail.goalId===goalId:role==="ceo"?mail.routeKind==="ceo_inbox":mail.routeKind==="specialist_inbox"&&mail.specialistRole===role;for(const mail of this.ledger.unreadMail(agent).filter(matches)){const cost=Math.min(JSON.stringify(mail.body).length,2_000)+128;if(selected.length>=20||selected.length>0&&used+cost>this.#memoryTailChars)break;selected.push(mail);used+=cost;}return selected;}
  ...
  async #agentRpcForTurn(turnId: string, agent: string, context: TurnContext, method: RunnerRpcMethod, params: JsonValue, sourceWakeId?:string): Promise<JsonValue> {
    ...
    if(context.goalCommitment){const goal=this.ledger.goal(context.goalCommitment.goalId);const thread=this.ledger.thread(execution.threadId);if(!goal||goal.phase!=="active"||goal.revision!==context.goalCommitment.goalRevision||goal.owner!==agent||execution.goalId!==goal.id||execution.goalRevision!==goal.revision||thread?.agent!==agent)throw new Error("stale committed Turn RPC rejected");}
    const input = asRecord(params);
    if(method==="goal.handoff.validate"){const candidate=String(input.candidateMessage??"");const candidateMessageId=String(input.candidateMessageId??"");this.ledger.appendEvent({streamId:`turn:${turnId}`,ts:this.#now(),actor:agent,type:"rpc.goal.handoff.validate",data:{attemptId:handoffAttemptId!,handoff:input.handoff??null,candidateMessageId,candidateMessagePresent:Boolean(candidate.trim()),candidateMessageChars:candidate.length},ignorable:true});return this.#validateHandoffDraft(turnId,handoffAttemptId!,agent,execution,context,input) as unknown as JsonValue;}
    ...
    if (!context.goalCommitment && goalBoundCapability(method)) throw new Error(`${method} requires a Goal commitment`);
    ...
    if (method === "goal.get") {const visibleId=context.goalCommitment?.goalId??context.activeGoal?.id;return (visibleId?this.ledger.goal(visibleId):null) as unknown as JsonValue;}
    if (method === "goal.create") {
      if (context.trigger.kind !== "user_message" || context.goalCommitment || agent !== "ceo") throw new Error("Root Goal creation requires an uncommitted direct CEO Turn"); const goal = this.createRootGoal(String(input.objective), typeof input.id === "string" ? input.id : undefined,turnId); context.activeGoal=goal;context.goalCommitment = { goalId: goal.id, goalRevision: goal.revision }; this.ledger.commitTurnToGoal(turnId,context.goalCommitment,"supervisor"); return { goal, goalCommitment: context.goalCommitment } as unknown as JsonValue;
    }
    if (method === "goal.work") { if(context.trigger.kind!=="user_message"||context.goalCommitment)throw new Error("work_on_goal requires an uncommitted direct CEO Turn");const goal = this.#goal(String(input.goalId)); if (goal.phase !== "active" || !this.#validGoalOwner(agent,goal)) throw new Error("work_on_goal requires the active owned Root Goal"); context.activeGoal=goal;context.goalCommitment = { goalId: goal.id, goalRevision: goal.revision }; this.ledger.commitTurnToGoal(turnId,context.goalCommitment,"supervisor"); return { goal, goalCommitment: context.goalCommitment } as unknown as JsonValue; }
    if (method === "work_record.list") return this.ledger.workRecords() as unknown as JsonValue;
    if (method === "work_record.read") return this.ledger.workRecord(String(input.goalId ?? context.goalCommitment?.goalId ?? context.activeGoal?.id ?? "")) as unknown as JsonValue;
    if (method === "work_record.history") return this.ledger.workRecordHistory(String(input.goalId ?? context.goalCommitment?.goalId ?? context.activeGoal?.id ?? "")) as unknown as JsonValue;
    if (method === "work_record.diff") return this.ledger.workRecordDiff(String(input.goalId ?? context.goalCommitment?.goalId ?? context.activeGoal?.id ?? ""), Number(input.fromRevision), Number(input.toRevision)) as unknown as JsonValue;
    ...
    if (method === "work_record.update") { const binding = context.goalCommitment!; return this.ledger.updateWorkRecord({ goalId: binding.goalId, goalRevision: binding.goalRevision, expectedRevision: Number(input.expectedRevision), content: String(input.content), reason: String(input.reason), evidence: numberArray(input.evidence), turnId, ...(sourceWakeId?{sourceWakeId}:{}) }, agent) as unknown as JsonValue; }
    if (method === "goal.delegate") {const binding=context.goalCommitment!;if(String(input.parentGoalId)!==binding.goalId)throw new Error("delegation parent must be the Goal committed to this Turn");return this.delegate({ id:String(input.id),parentGoalId:binding.goalId,expectedParentRevision:Number(input.expectedParentRevision),childGoal:asChildGoal(input.childGoal),brief:(input.brief??null) as JsonValue,reason:String(input.reason),evidence:numberArray(input.evidence),sourceTurnId:turnId },agent,sourceWakeId) as unknown as JsonValue;}
    if (method === "goal.reassign") {const goal=this.#goal(String(input.goalId));if(goal.parentId!==context.goalCommitment!.goalId)throw new Error("reassignment target must be a child of the Goal committed to this Turn");return await this.reassignGoal({ id:String(input.id),goalId:goal.id,expectedGoalRevision:Number(input.expectedGoalRevision),newOwner:String(input.newOwner),brief:(input.brief??null) as JsonValue,reason:String(input.reason),evidence:numberArray(input.evidence),sourceTurnId:turnId },agent,sourceWakeId) as unknown as JsonValue;}
    ...
    if (method === "goal.pause" || method === "goal.resume") { const goal=this.#goal(String(input.goalId)); const directRoot=!context.goalCommitment&&context.trigger.kind==="user_message"&&goal.parentId===null; if(!context.goalCommitment&&!directRoot) throw new Error(`${method} requires a Goal commitment`); const next=this.transitionGoal(goal.id,method==="goal.pause"?"paused":"active",directRoot?"human":agent,!directRoot,turnId); if(method==="goal.resume"&&directRoot){context.activeGoal=next;context.goalCommitment={goalId:next.id,goalRevision:next.revision};this.ledger.commitTurnToGoal(turnId,context.goalCommitment,"supervisor");return {goal:next,goalCommitment:context.goalCommitment} as unknown as JsonValue;} return next as unknown as JsonValue; }
    if (method === "goal.complete") { const goal=this.#goal(String(input.goalId)); const directRoot=!context.goalCommitment&&context.trigger.kind==="user_message"&&goal.parentId===null; if(!context.goalCommitment&&!directRoot) throw new Error("goal.complete requires a Goal commitment or direct Human Root authority"); return this.completeGoal({goalId:goal.id,revision:Number(input.revision),reason:String(input.reason),evidence:numberArray(input.evidence),sourceTurnId:turnId},directRoot?"human":agent,sourceWakeId) as unknown as JsonValue; }
    if (method === "mail.send") {const to=String(input.to);if(!this.#knownAgent(to))throw new Error(`unknown Agent recipient: ${to}; Mail addresses Agents only`);const goalId=String(input.goalId??"").trim();if(!goalId)throw new Error("Agent Mail requires a Goal route");const goal=this.#goal(goalId);if(!this.#validGoalOwner(to,goal))throw new Error("Agent Mail Goal route does not match the recipient role and ownership");const mail: MailSnapshot = { id:randomUUID(),to,from:agent,level:String(input.level) as MailSnapshot["level"],...goalRoute(goalId),body:(input.body??null) as JsonValue,readAt:null }; this.ledger.putMail(mail,agent,sourceWakeId); return mail as unknown as JsonValue; }
    if (method === "schedule.set") return (this.planWake(agent,String(input.at),String(input.reason),agent,context.goalCommitment??undefined)??{scheduled:true}) as unknown as JsonValue;
  ...
  #validateHandoffDraft(turnId:string,attemptId:number,agent:string,execution:TurnSnapshot,context:TurnContext,input:Record<string,JsonValue>):HandoffValidationResult{
    if(!context.goalCommitment||execution.goalId===null)return{accepted:false,fatal:true,attemptId,issues:[{code:"turn_not_committed",message:"This Turn no longer has a Goal commitment."}]};
    const request: HandoffValidationRequest={handoff:(input.handoff??null) as unknown as HandoffValidationRequest["handoff"],candidateMessageId:String(input.candidateMessageId??"").trim(),candidateMessage:String(input.candidateMessage??"")};
    ...
    const message=normalizeAssistantText(request.candidateMessage);const existingMessage=request.candidateMessageId?this.ledger.turnItems(turnId).find((item)=>item.id===request.candidateMessageId):undefined;
    if(!request.candidateMessageId||!message)issues.push({code:"message_missing",message:"This committed Turn needs a readable assistant message Item before it can finish."});
    else if(existingMessage&&(existingMessage.type!=="assistant_message"||existingMessage.status!=="completed"||normalizeAssistantText(String((existingMessage.data as {text?:unknown}).text??""))!==message))issues.push({code:"message_mismatch",message:"Handoff message identity does not match its assistant Item."});
    const goal=this.ledger.goal(context.goalCommitment.goalId);if(!goal||goal.phase!=="active"||goal.owner!==agent||goal.revision!==context.goalCommitment.goalRevision)return{accepted:false,fatal:true,attemptId,issues:[{code:"goal_fence_changed",message:"Goal ownership, phase, or revision changed; this Turn can no longer finish against the old commitment."}]};
    ...
    const token=randomUUID();this.#handoffValidations.set(token,{turnId,attemptId,agent,attempt:execution.attempt,leaseToken:execution.leaseToken!,goalId:goal.id,goalRevision:goal.revision,messageItemId:request.candidateMessageId,message,handoff:{outcome:request.handoff.outcome,evidence:[...request.handoff.evidence]}});return{accepted:true,fatal:false,attemptId,token,goalId:goal.id,goalRevision:goal.revision,messageItemId:request.candidateMessageId};
  }
...
function goalBoundCapability(method: AgentCapability): boolean { return ["goal.delegate", "goal.reassign", "goal.revise", "goal.put", "work_record.update", "schedule.set"].includes(method); }
...
function defaultCapabilities(role: AgentRole): AgentCapability[] {
  if (role === "ceo") return ["ledger.search", "mail.send", "schedule.set", "team.list", "goal.get", "goal.create", "goal.work", "goal.delegate", "goal.reassign", "goal.revise", "goal.pause", "goal.resume", "goal.complete", "work_record.list", "work_record.read", "work_record.history", "work_record.diff", "work_record.search", "work_record.update"];
