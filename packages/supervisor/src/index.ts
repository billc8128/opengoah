...
export class Supervisor {
  ...
  async runAvailable(concurrency = 4, maxWakes = 100): Promise<WakeSnapshot[]> {
    ...
    const mailDelivery=triggerRef.startsWith("mail:")&&this.#knownAgent(agent);const runtimeFallback=agent==="ceo"&&triggerRef.startsWith("child-");
    if (!ownsLiveGoal && !mailDelivery&&!runtimeFallback) return null;
  ...
  async #agentRpcForTurn(turnId: string, agent: string, context: TurnContext, method: AgentCapability, params: JsonValue, sourceWakeId?:string): Promise<JsonValue> {
    ...
    if (method === "mail.send") {const to=String(input.to);if(to==="human")throw new Error("Human communication requires human.request");if(!this.#knownAgent(to))throw new Error(`unknown Agent recipient: ${to}`);const goalId=typeof input.goalId==="string"?input.goalId:undefined;const mail: MailSnapshot = { id:randomUUID(),to,from:agent,level:String(input.level) as MailSnapshot["level"],...(goalId?{goalId}:{}),body:(input.body??null) as JsonValue,readAt:null }; this.ledger.putMail(mail,agent,sourceWakeId); return mail as unknown as JsonValue; }
