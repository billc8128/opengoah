...
export class Supervisor {
  ...
  async #executeTurn(initial: TurnSnapshot, agent: string, turnContext: TurnContext, contextFactory: () => JsonValue, sourceWake: WakeSnapshot | null = null, deliveredMailIds: string[] = [], recordRevisionAtStart = -1, sourceWakeTriggers: WakeTriggerSnapshot[] = []): Promise<void> {
    while (this.ledger.turn(initial.id)?.status === "in_progress") {
      ...
      try {
        ...
        if(turnContext.goalCommitment){if(!result.handoff)throw new Error("committed Turn requires Handoff");this.#commitTurnGoalWork(current.id,agent,turnContext,result.handoff,sourceWake?.id??null,deliveredMailIds,recordRevisionAtStart);return;}
        if(result.handoff)throw new Error("uncommitted Turn cannot finish with Handoff");if(!this.ledger.turnItems(current.id).some((item)=>item.type==="assistant_message"&&(item.data as {text?:unknown}).text===result.response.content))this.#appendTurnItem(current.id,"assistant_message",{text:result.response.content},agent);this.ledger.finishTurn(current.id,"completed",null,this.#now(),"supervisor",deliveredMailIds);return;
  ...
  #recordTurnTrace(turnId: string, leaseToken: string, type: string, data: JsonValue, actor = "ceo"): void {
    ...
    if (type === "message.assistant.completed") {const value=data&&typeof data==="object"&&!Array.isArray(data)?data as {message?:{content?:unknown};commitState?:unknown}:{};if(!["committed","provisional"].includes(String(value.commitState)))throw new Error("Runner assistant completion is missing normalized commit state");const text=messageTextContent(value.message?.content);if(text)this.#appendTurnItem(turnId,"assistant_message",{text},actor);}
