...
export class Supervisor {
  ...
  async startHumanGoalTurn(objective:string,id:string=randomUUID()):Promise<{threadId:string;turnId:string;steered:false;goal:GoalSnapshot}>{
    if(!objective.trim())throw new Error("root objective is required");const previous=this.#humanTail;let release!:()=>void;this.#humanTail=new Promise<void>((resolve)=>{release=resolve;});await previous;try{const goal=this.createRootGoal(objective,id);const accepted=await this.#startHumanTurn(`/goal ${objective}`,goal);return{...accepted,steered:false,goal};}finally{release();}
  }
  async #startHumanTurn(message:string,initialGoal:GoalSnapshot|null=null):Promise<{threadId:string;turnId:string;steered:boolean}>{
    if (!message.trim()) throw new Error("message is required");
    const thread = this.threadFor("ceo"); const active = this.ledger.activeTurn(thread.id);
    if (active) {
      if (active.triggerKind === "wake") { const goalId = active.goalId; await this.interruptTurn(active.id); if (goalId){const goal=this.ledger.goal(goalId);if(goal?.phase==="active")this.#enqueueTrigger(goal.owner,`human-interrupted:${active.id}`,{goalId:goal.id});} }
      else { if(initialGoal){await this.interruptTurn(active.id);}else{const handle=this.#handles.get(active.id);if(handle&&!handle.steer)await this.interruptTurn(active.id);else if(handle?.steer){const item=this.#appendTurnItem(active.id,"user_message",{text:message},"human",randomUUID(),"in_progress");try{await handle.steer(message);const stored=this.ledger.turnItems(active.id).find((candidate)=>candidate.id===item.id);if(this.ledger.turn(active.id)?.status==="in_progress"&&stored?.status==="in_progress"){this.ledger.putTurnItem({...stored,status:"completed",completedAt:this.#now()},"human");return{threadId:thread.id,turnId:active.id,steered:true};}}catch{const stored=this.ledger.turnItems(active.id).find((candidate)=>candidate.id===item.id);if(this.ledger.turn(active.id)?.status==="in_progress"&&stored?.status==="in_progress")this.ledger.putTurnItem({...stored,status:"failed",completedAt:this.#now()},"human");if(this.ledger.turn(active.id)?.status==="in_progress")await this.interruptTurn(active.id);}}else{this.#appendTurnItem(active.id,"user_message",{text:message},"human");return{threadId:thread.id,turnId:active.id,steered:true};}} }
    }
    await this.#awaitAgentExecution("ceo");
    if(this.ledger.activeTurn(thread.id))return this.#startHumanTurn(message,initialGoal);
    const now = this.#now(); this.ledger.putThread({ ...thread,updatedAt:now },"supervisor"); const leaseToken = randomUUID();
    const turn: TurnSnapshot = { id: randomUUID(), threadId: thread.id, triggerKind: "user_message", ...(initialGoal?goalCommitment(initialGoal.id,initialGoal.revision):noGoalCommitment()), status: "in_progress", attempt: 1, error: null, startedAt: now, endedAt: null, leaseUntil: new Date(this.clock.now().getTime() + this.#leaseMs).toISOString(), leaseToken, runnerPid: null,runnerProfileId:this.#runnerProfileId("ceo") };
    this.ledger.putTurn(turn, "human"); this.#appendTurnItem(turn.id, "user_message", { text: message }, "human");
    const context: TurnContext = { trigger: { kind: "user_message" },activeGoal:initialGoal??this.currentRoot(),goalCommitment:initialGoal?{goalId:initialGoal.id,goalRevision:initialGoal.revision}:null };const deliveredMail=this.#contextMail("ceo",initialGoal?.id??null);const recordRevisionAtStart=initialGoal?this.ledger.workRecord(initialGoal.id)?.recordRevision??-1:-1;
    void this.#trackExecution(turn,"ceo",context,()=>this.#humanContext(turn.id,"ceo",deliveredMail),null,deliveredMail.map((mail)=>mail.id),recordRevisionAtStart); return { threadId: thread.id, turnId: turn.id, steered: false };
  }
  ...
  async #executeTurn(initial: TurnSnapshot, agent: string, turnContext: TurnContext, contextFactory: () => JsonValue, sourceWake: WakeSnapshot | null = null, deliveredMailIds: string[] = [], recordRevisionAtStart = -1, sourceWakeTriggers: WakeTriggerSnapshot[] = []): Promise<void> {
    while (this.ledger.turn(initial.id)?.status === "in_progress") {
      ...
      try {
        ...
        if(!result.response.content.trim())throw new Error("completed Turn requires a readable assistant response");
        if(turnContext.goalCommitment){if(!result.handoff)throw new Error("committed Turn requires Handoff");this.#commitTurnGoalWork(current.id,agent,turnContext,result.response.content,result.handoff,sourceWake?.id??null,deliveredMailIds,recordRevisionAtStart);return;}
        if(result.handoff)throw new Error("uncommitted Turn cannot finish with Handoff");if(this.#assistantCommitState(current.id,result.response.content)==="provisional")throw new Error("uncommitted Turn cannot complete with a provisional assistant response");if(!this.ledger.turnItems(current.id).some((item)=>item.type==="assistant_message"&&(item.data as {text?:unknown}).text===result.response.content))this.#appendTurnItem(current.id,"assistant_message",{text:result.response.content},agent);this.ledger.finishTurn(current.id,"completed",null,this.#now(),"supervisor",deliveredMailIds);return;
  ...
  #assistantCommitState(turnId:string,text:string):"committed"|"provisional"|null{const event=this.ledger.readStream(`turn:${turnId}`).findLast((candidate)=>{if(candidate.type!=="message.assistant.completed"||!candidate.data||typeof candidate.data!=="object"||Array.isArray(candidate.data))return false;const message=(candidate.data as {message?:{content?:unknown}}).message;return messageTextContent(message?.content)===text;});if(!event||!event.data||typeof event.data!=="object"||Array.isArray(event.data))return null;const state=(event.data as {commitState?:unknown}).commitState;return state==="committed"||state==="provisional"?state:null;}
  ...
  #preemptGoalTurn(goalId:string,reason:string):void{const turn=this.ledger.turns().find((candidate)=>candidate.status==="in_progress"&&candidate.goalId===goalId);if(!turn)return;this.#clearHandoffState(turn.id);this.ledger.finishTurn(turn.id,"interrupted",{message:reason},this.#now(),"supervisor");const handle=this.#handles.get(turn.id);if(handle)void this.#terminateHandle(turn.id,handle).finally(()=>this.#handles.delete(turn.id));}
  ...
  #scheduleTurnRecovery(sourceWake:WakeSnapshot|null,triggers:WakeTriggerSnapshot[],turnId:string,agent:string):void{if(!sourceWake)return;const turn=this.ledger.turn(turnId);if(!turn||turn.triggerKind!=="wake")return;const role=this.#role(agent);const goal=turn.goalId?this.ledger.goal(turn.goalId):null;if(turn.goalId&&(!goal||goal.phase!=="active"||goal.owner!==agent)||!turn.goalId&&role!=="verifier"&&role!=="audit")return;const prior=Math.max(0,...triggers.map((trigger)=>parseRecoveryTrigger(trigger.triggerRef)?.attempt??0));const next=prior+1;if(next<this.#retryPolicy.maxAttempts){const delay=this.#retryPolicy.baseDelayMs*2**prior;const target=goal?goalAutomaticTarget(agent,goal.id):specialistAutomaticTarget(agent,role as "verifier"|"audit");this.ledger.putSchedule({id:recoveryScheduleId(turnId,next),...target,nextWakeAt:new Date(this.clock.now().getTime()+delay).toISOString(),reason:recoveryRef(turnId),setBy:"supervisor",status:"pending",resolvedAt:null},"supervisor",sourceWake.id);}else if(role==="child"&&goal?.parentId){const parent=this.ledger.goal(goal.parentId);if(parent?.phase==="active")this.#enqueueTrigger(parent.owner,childRetryExhaustedRef(turnId),{goalId:parent.id});}}
  ...
  async recover(): Promise<void> {
    ...
    for (const turn of this.ledger.turns().filter((candidate) => candidate.status === "in_progress"&&!this.#handles.has(candidate.id))) { if (turn.runnerPid) await this.runner.terminateProcess(turn.runnerPid,turn.runnerProfileId); this.#failTurn(turn.id,"orphaned Runner ownership recovered after Supervisor restart"); const thread=this.ledger.thread(turn.threadId);if(!thread||turn.triggerKind!=="wake")continue;if(turn.goalId){const goal=this.ledger.goal(turn.goalId);if(goal?.phase==="active"&&goal.owner===thread.agent)this.#enqueueTrigger(thread.agent,recoveryRef(turn.id),{goalId:goal.id});}else if(thread.role==="verifier"||thread.role==="audit")this.#enqueueTrigger(thread.agent,recoveryRef(turn.id)); }
  }
