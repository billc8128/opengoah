...
export class Supervisor {
  ...
  async #startHumanTurn(message:string):Promise<{threadId:string;turnId:string;steered:boolean}>{
    ...
    const context: TurnContext = { source: { kind: "human" } };const deliveredMail=this.#contextMail("ceo",null);
  ...
  async runAvailable(concurrency = 4, maxWakes = 100): Promise<WakeSnapshot[]> {
    ...
    else {const mailId=mailIdFromTriggerRef(triggerRef);const mail=mailId?this.ledger.mailbox().find((candidate)=>candidate.id===mailId):null;const role=this.#role(agent);if(!(agent==="ceo"&&role==="ceo"&&mail?.from==="human"&&!mail.goalId)&&role!=="verifier"&&role!=="audit")return null;}
  ...
  #contextMail(agent:string,goalId:string|null):MailSnapshot[]{const selected:MailSnapshot[]=[];let used=0;for(const mail of this.ledger.unreadMail(agent).filter((candidate)=>(candidate.goalId??null)===goalId)){const cost=Math.min(JSON.stringify(mail.body).length,2_000)+128;if(selected.length>=20||selected.length>0&&used+cost>this.#memoryTailChars)break;selected.push(mail);used+=cost;}return selected;}
