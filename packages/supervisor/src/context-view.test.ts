...
import { goalRoute,goalTarget,type EventRecord } from "goah-ledger-contract";
...
test("recovery context selects semantic failure facts instead of raw transcript traffic", () => {
  ...
  const view = composeActiveContext({
    role: "child", capabilities: ["ledger.search"], systemPrompt: "worker", wake: { id: "retry", ...goalTarget("worker","goal"),triggerRef: "retry:failed", status: "consumed", attempt: 1, enqueuedSeq: 1, claimedAt:"2030-01-01T00:00:00.000Z",consumedAt:"2030-01-01T00:00:00.000Z",turnId:"turn" },wakeTriggers:[],
...
test("active context renders working memory with evidence sequences", () => {
  ...
  const view = composeActiveContext({
    role: "child", capabilities: ["memory.append"], systemPrompt: "worker", wake: { id: "w2", ...goalTarget("worker","goal"),triggerRef: "schedule:worker", status: "consumed", attempt: 1, enqueuedSeq: 1, claimedAt:"2030-01-01T00:00:00.000Z",consumedAt:"2030-01-01T00:00:00.000Z",turnId:"turn" },wakeTriggers:[],
...
test("verification Mail is delivered through the bounded Incoming section",()=>{const view=composeActiveContext({role:"child",capabilities:["ledger.search"],systemPrompt:"worker",wake:{id:"w",...goalTarget("worker","goal"),triggerRef:"review",status:"consumed",attempt:1,enqueuedSeq:1,claimedAt:"2030-01-01T00:00:00.000Z",consumedAt:"2030-01-01T00:00:00.000Z",turnId:"turn"},wakeTriggers:[],goals:[],mail:[{id:"verification",to:"worker",from:"verifier",level:"decision",...goalRoute("goal"),body:{type:"verification_result",findings:[{body:{issue:"unsupported claim"}}]},readAt:null}],lastHandoff:null,teamHandoffs:[],team:[],recoveryEvents:[]});assert.match(view.text,/# Incoming[\s\S]*unsupported claim/);});
