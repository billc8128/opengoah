...
test("recovery context selects semantic failure facts instead of raw transcript traffic", () => {
  const events = [
    ...
    ...Array.from({ length: 200 }, (_, index) =>
      event(index + 2, "message.assistant.delta", { delta: "x".repeat(100) }),
    ),
    ...
    event(207, "tool.completed", {
      callId: "publish",
      result: { outcome: "unknown", synthetic: true },
    }),
  ...
  assert.deepEqual(
    ...
    ["tool.called", "tool.completed", "transcript.interrupted"],
  );
  const view = composeActiveContext({
    role: "child",
    capabilities: ["ledger.search"],
    systemPrompt: "worker",
    wake: {
      id: "retry",
      ...goalAutomaticTarget("worker", "goal"),
      triggerRef: "retry:failed",
      status: "consumed",
      attempt: 1,
      enqueuedSeq: 1,
...
test("working memory keeps the newest notes inside the budget without compacting the stream", () => {
  ...
  assert.deepEqual(
    selectWorkingMemory(notes, 100).map((item) => item.streamSeq),
    [2, 3],
  );
  assert.deepEqual(
    selectWorkingMemory(notes, 10).map((item) => item.streamSeq),
    [3],
  );
...
test("active context renders working memory with evidence sequences", () => {
  const note = event(7, "memory.appended", {
    note: "integration tests fake-fail when the clock is mocked; approach A rejected: metric freshness",
  });
  const view = composeActiveContext({
    role: "child",
    capabilities: ["memory.append"],
    systemPrompt: "worker",
    wake: {
      id: "w2",
      ...goalAutomaticTarget("worker", "goal"),
      triggerRef: "schedule:worker",
      status: "consumed",
      attempt: 1,
      enqueuedSeq: 1,
      claimedAt: "2030-01-01T00:00:00.000Z",
...
test("verification Mail is delivered through the bounded Incoming section", () => {
  const view = composeActiveContext({
    role: "child",
    capabilities: ["ledger.search"],
    systemPrompt: "worker",
  ...
  assert.match(view.text, /# Incoming[\s\S]*unsupported claim/);
});
