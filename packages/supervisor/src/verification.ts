...
export interface VerifierProcessSpec {
  command: string;
  args: string[];
...
export class ProcessVerifierModel implements VerifierModel {
  ...
  async #call(operation: string, input: unknown): Promise<VerificationResult> {
    ...
    const append = (channel: "stdout" | "stderr", chunk: Buffer) => {
      ...
      if (channel === "stdout") stdout += text.slice(0, remaining);
    ...
    if (code !== 0)
      throw new Error(
        timedOut ? "verifier process timed out" : stderr.trim() || `verifier exited ${code}`,
      );
...
export class VerificationPlane {
  ...
  async verifyTurn(turnId: string): Promise<VerificationResult> {
    ...
    if (turn.status === "in_progress") throw new Error("cannot verify an in-progress Turn");
    const trace = this.ledger.readStream(`turn:${turnId}`);
    const handoff =
      this.ledger.turnItems(turnId).findLast((item) => item.type === "handoff")?.data ?? null;
    ...
    let route:
      | ReturnType<typeof goalRoute>
      | ReturnType<typeof ceoInboxRoute>
      | ReturnType<typeof specialistInboxRoute>;
    if (turn.goalId) {
  ...
  async auditGlobal(sinceSeq = 0): Promise<VerificationResult> {
    const facts = this.ledger
      ...
      .filter((event) => !["handoff.recorded", "runner.note"].includes(event.type));
    ...
    this.ledger.putMail(
      {
        ...
        body: {
          type: "audit_result",
          sinceSeq,
          findings: result.findings as unknown as JsonValue,
          tokensUsed: result.tokensUsed,
        },
        readAt: null,
      },
  ...
  #mailId(kind: string, source: string, recipient: string, result: VerificationResult): string {
...
export function evaluateVerification(
  ...
  predictedIds: string[],
): { precision: number; riskWeightedRecall: number } {
...
export function calibrateVerificationThreshold(
  labels: VerificationLabel[],
  predictions: ScoredPrediction[],
  minimumPrecision: number,
): number {
  const candidates = [...new Set(predictions.map((prediction) => prediction.score))].sort(
    (a, b) => a - b,
  );
  let selected = 1;
  let bestRecall = -1;
  for (const threshold of candidates) {
    const metrics = evaluateVerification(
      labels,
      predictions
