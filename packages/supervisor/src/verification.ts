...
export class VerificationPlane {
  ...
  async verifyTurn(turnId: string): Promise<VerificationResult> {
    ...
    const level=result.findings.length?"decision" as const:"fyi" as const;const body={type:"verification_result",turnId,agent:thread.agent,findings:result.findings,tokensUsed:result.tokensUsed} as unknown as JsonValue;let route:ReturnType<typeof goalRoute>|ReturnType<typeof humanInboxRoute>|ReturnType<typeof specialistInboxRoute>;if(turn.bindingKind==="goal"){const goal=this.ledger.goal(turn.goalId);if(!goal||goal.owner!==thread.agent)throw new Error("verified Goal Turn no longer matches its owner");route=goalRoute(turn.goalId);}else if(turn.bindingKind==="human"){if(thread.agent!=="ceo")throw new Error("verified Human Turn is not owned by CEO");route=humanInboxRoute();}else{if(this.supervisor.agentRole(thread.agent)!==turn.specialistRole)throw new Error("verified Specialist Turn does not match its Agent profile");route=specialistInboxRoute(turn.specialistRole);}const mail:MailSnapshot={id:this.#mailId("verification",turnId,thread.agent,result),to:thread.agent,from:"verifier",level,...route,body,readAt:null};this.ledger.putMail(mail,"verifier");
    return result;
  }
