...
test("RunnerRouter selects by opaque Runner Profile without knowing provider or model", async () => {
  ...
  const request = { agent:wake.agent,sourceWake:wake,execution:{ id:"turn",threadId:"thread",source:"system",goalId:null,goalRevision:null,status:"in_progress",attempt:1,error:null,startedAt:"2026-08-18T00:00:00.000Z",endedAt:null,leaseUntil:null,leaseToken:null,runnerPid:null }, turn: { source: { kind: "system", reason: "test" } }, context: { runnerProfile: { id: "worker", runner: "anything", config: { opaque: true } } }, now: () => "", emit: () => undefined } satisfies RunRequest;
