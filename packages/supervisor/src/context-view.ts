    ...
    events
      .filter(
        (event) =>
          ...
          field(field(event.data, "result"), "outcome") === "unknown",
      )
      .map((event) => String(field(event.data, "callId"))),
  );
...
export function composeActiveContext(input: ActiveContextInput): ActiveContextView {
  ...
  const sections: Array<[string, string[]]> = [
    [
      ...
      input.goals.map(
        (goal) =>
          `- [${goal.id}] ${goal.objective} (owner: ${goal.owner}, phase: ${goal.phase}, revision: ${goal.revision})`,
      ),
    ],
    [
      "Observation methods",
    ...
    [
      ...
      [
        ...input.wakeTriggers
          ...
          .map((trigger) => `- [${trigger.source}] ${trigger.triggerRef}`),
        `- Attempt: ${input.wake.attempt}`,
      ],
    ],
    [
      "Working memory",
      (input.workingMemory ?? []).map(
        (event) => `- ${String(field(event.data, "note") ?? "")} [event:${event.seq}]`,
      ),
    ],
    [
      "Last outcome",
      typeof handoff?.outcome === "string"
        ? [`- ${handoff.outcome}${input.lastHandoff ? ` [event:${input.lastHandoff.seq}]` : ""}`]
        : [],
    ],
    [
      "Incoming",
    ...
    [
      ...
      input.team.map(
        (member) =>
          `- ${member.agent}: motion=${member.motion}; outcome=${member.lastOutcome ?? "none"}; goals=${member.goalIds.join(",") || "none"}; next=${member.nextWakeAt ?? "none"}; handoff=${member.lastHandoffSeq ?? "none"}`,
      ),
    ],
    [
      "Team handoffs",
      input.teamHandoffs.map(
        (event) => `- ${event.actor}: ${render(event.data)} [event:${event.seq}]`,
      ),
    ],
    [
      "Recovery",
      input.recoveryEvents.map(
        (event) => `- ${event.type}: ${render(event.data)} [event:${event.seq}]`,
      ),
    ],
  ];
  const text = sections
  ...
  return {
    ...
    text,
    sourceSeqs: [...sourceSeqs].sort((a, b) => a - b),
  };
}

function render(value: JsonValue, maxChars = Number.POSITIVE_INFINITY): string {
