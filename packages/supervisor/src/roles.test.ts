...
test("default Turn prompts separate role from Goal commitment", () => {
  ...
  const human = defaultTurnPrompt("ceo", "ceo", {
    trigger: { kind: "user_message" },
    activeGoal,
    goalCommitment: null,
  });
  const ceoGoal = defaultTurnPrompt("ceo", "ceo", {
    trigger: { kind: "wake", reasons: ["goal"] },
    activeGoal,
    goalCommitment: { goalId: "root", goalRevision: 0 },
  });
  const goal = defaultTurnPrompt("child", "worker", {
    trigger: { kind: "wake", reasons: ["goal"] },
    activeGoal: childGoal,
    goalCommitment: { goalId: "child", goalRevision: 0 },
  });
  const verifier = defaultTurnPrompt("verifier", "verifier", {
    trigger: { kind: "wake", reasons: ["mail:verify"] },
    activeGoal: null,
    goalCommitment: null,
  });
  ...
  assert.throws(
    () =>
      defaultTurnPrompt("child", "worker", {
        trigger: { kind: "wake", reasons: ["mail:m1"] },
        activeGoal: childGoal,
        goalCommitment: null,
      }),
    /require a Goal commitment/,
  );
  assert.throws(
    () =>
      defaultTurnPrompt("verifier", "verifier", {
        trigger: { kind: "wake", reasons: ["goal"] },
        activeGoal: childGoal,
        goalCommitment: { goalId: "child", goalRevision: 0 },
      }),
    /cannot commit to a Goal/,
  );
