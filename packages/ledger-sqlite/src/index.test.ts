...
test("inactive Goal reassignment wakes the new owner through Mail without scheduling Goal work",()=>{for(const phase of ["paused","blocked"] as const){const ledger=new SqliteLedger(":memory:",{clock:new FixedClock()});ledger.putGoal({id:"root",parentId:null,objective:"operate",observationMethod:null,verificationMethod:null,owner:"ceo",phase:"active",revision:0},"human");ledger.putGoal({id:"target",parentId:"root",objective:"target",observationMethod:"observe",verificationMethod:"verify",owner:"old",phase:"active",revision:0},"ceo");ledger.putGoal({...ledger.goal("target")!,phase,revision:1},"ceo");const evidence=ledger.appendEvent(event("ceo","observed",{}));const result=ledger.commitReassignment({id:`move-${phase}`,goalId:"target",expectedGoalRevision:1,newOwner:"new",brief:{},reason:"move inactive target",evidence:[evidence.seq]},"ceo");assert.equal(result.goal.phase,phase);assert.equal(result.wake.goalId,undefined);assert.equal(result.wake.triggerRef,`mail:reassignment-new-mail:move-${phase}`);assert.equal(ledger.wakeTriggers(result.wake.id)[0]?.source,"system");ledger.close();}});
