...
test("pre-v21 development schemas are rejected explicitly", () => {
  for(const version of [1,6,9,10,11,15,16,17,18,19,20]){const path=join(mkdtempSync(join(tmpdir(),`goah-retired-${version}-`)),"ledger.sqlite");const raw=new DatabaseSync(path);raw.exec(`PRAGMA user_version=${version}`);raw.close();assert.throws(()=>new SqliteLedger(path,{clock:new FixedClock()}),/predates runtime schema 21/);}
});
...
test("Work Records are versioned Goal documents backed by replayable events", () => {
  ...
  const turn=activeTurn(ledger,"ceo","turn-1","human");ledger.putTurn({...turn,bindingKind:"goal",goalId:"root",goalRevision:0,specialistRole:null},"supervisor");
...
test("reassignment changes ownership, notifies both sides, and wakes only the new owner", () => {
  ...
  const turn=activeTurn(ledger,"new","new-turn","human");ledger.putTurn({...turn,bindingKind:"goal",goalId:"launch",goalRevision:1,specialistRole:null},"supervisor");
...
function activeTurn(ledger:SqliteLedger,agent:string,id:string,source:"human"|"goal"|"system"="system"):import("goah-ledger-contract").TurnSnapshot{let thread=ledger.threads().find((candidate)=>candidate.agent===agent);if(!thread){const now="2030-01-01T00:00:00.000Z";thread={id:`thread:${agent}`,agent,parentThreadId:null,createdAt:now,updatedAt:now};ledger.putThread(thread,"supervisor");}const existing=ledger.turn(id);if(existing)return existing;const binding=source==="human"?{bindingKind:"human" as const,goalId:null,goalRevision:null,specialistRole:null}:source==="system"?{bindingKind:"specialist" as const,goalId:null,goalRevision:null,specialistRole:"verifier" as const}:{bindingKind:"human" as const,goalId:null,goalRevision:null,specialistRole:null};const turn:import("goah-ledger-contract").TurnSnapshot={id,threadId:thread.id,source,...binding,status:"in_progress",attempt:1,error:null,startedAt:"2030-01-01T00:00:00.000Z",endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"lease",runnerPid:null};ledger.putTurn(turn,"supervisor");return turn;}
function goalTurn(ledger:SqliteLedger,agent:string,turnId:string,goalId=`goal:${turnId}`):import("goah-ledger-contract").TurnSnapshot{ledger.putGoal({id:goalId,parentId:null,objective:"work",observationMethod:"observe",verificationMethod:"verify",owner:agent,phase:"active",revision:0},"human");let thread=ledger.threads().find((candidate)=>candidate.agent===agent);if(!thread){thread={id:`thread:${agent}`,agent,parentThreadId:null,createdAt:"2030-01-01T00:00:00.000Z",updatedAt:"2030-01-01T00:00:00.000Z"};ledger.putThread(thread,"supervisor");}const turn:import("goah-ledger-contract").TurnSnapshot={id:turnId,threadId:thread.id,source:"goal",bindingKind:"goal",goalId,goalRevision:0,specialistRole:null,status:"in_progress",attempt:1,error:null,startedAt:"2030-01-01T00:00:00.000Z",endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"lease",runnerPid:null};ledger.putTurn(turn,"supervisor");const evidence=ledger.appendEvent(event(agent,"observation",{ok:true}));ledger.updateWorkRecord({goalId,expectedRevision:0,goalRevision:0,content:"# Current State\n\nObserved current work.\n",reason:"record current work",evidence:[evidence.seq],turnId},agent);return ledger.turn(turnId)!;}
