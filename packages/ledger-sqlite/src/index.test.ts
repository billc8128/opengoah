...
import { controlStream,goalRoute,humanInboxRoute,goalStream,goalExecutionBinding,replayTranscript,specialistInboxRoute,specialistBinding,wakeStream,type Clock,type EventInput,type GoalSnapshot,type JsonValue,type ScheduleSnapshot,type WakeSnapshot } from "goah-ledger-contract";
...
function wake(id:string,agent="agent-1",goalId?:string):WakeSnapshot{return{id,...(goalId?goalExecutionBinding(agent,goalId):specialistBinding(agent,"verifier")),triggerRef:`trigger:${id}`,status:"queued",attempt:0,enqueuedSeq:0,claimedAt:null,consumedAt:null,turnId:null};}
function schedule(id:string,agent="a",goal?:{goalId:string}):ScheduleSnapshot{return{id,...(goal?goalExecutionBinding(agent,goal.goalId):specialistBinding(agent,"verifier")),nextWakeAt:"2030-01-01T00:00:00.000Z",reason:"test",setBy:agent,status:"pending",resolvedAt:null};}
...
test("Thread Turn and Item projections form one resumable execution history", () => {
  ...
  ledger.putTurn({ id: "turn", threadId: "thread", source: "human", bindingKind:"human",goalId: null, goalRevision: null,specialistRole:null, status: "in_progress", attempt:1,error: null, startedAt: now, endedAt: null, leaseUntil: "2030-01-01T00:10:00.000Z", leaseToken: "lease", runnerPid: null }, "human");
...
test("pre-v22 development schemas are rejected explicitly", () => {
  for(const version of [1,6,9,10,11,15,16,17,18,19,20,21]){const path=join(mkdtempSync(join(tmpdir(),`goah-retired-${version}-`)),"ledger.sqlite");const raw=new DatabaseSync(path);raw.exec(`PRAGMA user_version=${version}`);raw.close();assert.throws(()=>new SqliteLedger(path,{clock:new FixedClock()}),/predates runtime schema 22/);}
});
...
test("a claimed Wake cannot start after a Human Turn acquires priority",()=>{const ledger=new SqliteLedger(":memory:",{clock:new FixedClock()});ledger.putThread({id:"ceo-thread",agent:"ceo",parentThreadId:null,createdAt:new FixedClock().value,updatedAt:new FixedClock().value},"supervisor");ledger.putThread({id:"worker-thread",agent:"worker",parentThreadId:"ceo-thread",createdAt:new FixedClock().value,updatedAt:new FixedClock().value},"supervisor");ledger.enqueueWake(wake("w","worker"),"supervisor");ledger.claimNextWake(new FixedClock().value);ledger.putTurn({id:"human",threadId:"ceo-thread",source:"human",bindingKind:"human",goalId:null,goalRevision:null,specialistRole:null,status:"in_progress",attempt:1,error:null,startedAt:new FixedClock().value,endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"human",runnerPid:null},"human");const automatic={id:"automatic",threadId:"worker-thread",source:"system" as const,bindingKind:"specialist" as const,goalId:null,goalRevision:null,specialistRole:"verifier" as const,status:"in_progress" as const,attempt:1,error:null,startedAt:new FixedClock().value,endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"auto",runnerPid:null};assert.throws(()=>ledger.startTurnFromWake("w",automatic,new FixedClock().value),/Human Turn/);assert.equal(ledger.wake("w")?.status,"claimed");assert.equal(ledger.turn("automatic"),null);ledger.close();});

test("Turn admission rejects non-CEO Human Turns and stale Goal revisions",()=>{const ledger=new SqliteLedger(":memory:",{clock:new FixedClock()});const now=new FixedClock().value;ledger.putThread({id:"worker",agent:"worker",parentThreadId:null,createdAt:now,updatedAt:now},"supervisor");assert.throws(()=>ledger.putTurn({id:"forged-human",threadId:"worker",source:"human",bindingKind:"human",goalId:null,goalRevision:null,specialistRole:null,status:"in_progress",attempt:1,error:null,startedAt:now,endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"lease",runnerPid:null},"human"),/claimed Wake/);ledger.putGoal({id:"goal",parentId:null,objective:"v0",observationMethod:null,verificationMethod:null,owner:"worker",phase:"active",revision:0},"human");ledger.putGoal({...ledger.goal("goal")!,objective:"v1",revision:1},"human");ledger.enqueueWake(wake("goal-wake","worker","goal"),"supervisor");assert.equal(ledger.claimNextWake(now)?.id,"goal-wake");const stale={id:"stale",threadId:"worker",source:"goal" as const,bindingKind:"goal" as const,goalId:"goal",goalRevision:0,specialistRole:null,status:"in_progress" as const,attempt:1,error:null,startedAt:now,endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"stale",runnerPid:null};assert.throws(()=>ledger.startTurnFromWake("goal-wake",stale,now),/current active Goal revision/);assert.equal(ledger.turn("stale"),null);ledger.close();});
...
test("Goal completion requires a readable response and commits it atomically before Handoff",()=>{const ledger=new SqliteLedger(":memory:",{clock:new FixedClock()});const turn=goalTurn(ledger,"agent-1","readable","g");const commit=handoffCommit(ledger,turn.id,null);assert.throws(()=>ledger.commitHandoff({...commit,output:{...commit.output,response:{content:"   "}},responseItem:{...commit.responseItem,data:{text:"   "}}}),/readable response/);assert.deepEqual(ledger.turnItems(turn.id).map((item)=>item.type),[]);ledger.commitHandoff(commit);assert.deepEqual(ledger.turnItems(turn.id).map((item)=>item.type),["assistant_message","handoff"]);assert.deepEqual(ledger.readStream(`turn:${turn.id}`).filter((event)=>event.type==="message.assistant.completed"||event.type==="handoff.recorded").map((event)=>event.type),["message.assistant.completed","handoff.recorded"]);assert.equal(ledger.turn(turn.id)?.status,"completed");ledger.close();});

test("Handoff cannot reuse a Turn Item identity",()=>{const ledger=new SqliteLedger(":memory:",{clock:new FixedClock()});const old=activeTurn(ledger,"agent-1","old");ledger.putTurnItem({id:"shared-item",turnId:old.id,ordinal:1,type:"assistant_message",status:"completed",data:{text:"old"},createdAt:new FixedClock().value,completedAt:new FixedClock().value},"agent-1");ledger.finishTurn(old.id,"completed",null,new FixedClock().value,"supervisor");const turn=goalTurn(ledger,"agent-1","goal-identities","g");const commit=handoffCommit(ledger,turn.id,null);assert.throws(()=>ledger.commitHandoff({...commit,item:{...commit.item,id:"shared-item"}}),/identity/);assert.equal(ledger.turn(turn.id)?.status,"in_progress");ledger.close();});

test("reading Mail preserves a coalesced Schedule trigger on the same Wake",()=>{const ledger=new SqliteLedger(":memory:",{clock:new FixedClock()});ledger.putGoal({id:"g",parentId:null,objective:"work",observationMethod:null,verificationMethod:null,owner:"ceo",phase:"active",revision:0},"human");ledger.putMail({id:"m",to:"ceo",from:"verifier",level:"decision",...goalRoute("g"),body:{},readAt:null},"verifier");ledger.enqueueWake({...wake("multi","ceo","g"),triggerRef:"mail:m"},"supervisor");ledger.putSchedule(schedule("s","ceo",{goalId:"g"}),"ceo");ledger.consumeSchedule("s",{...wake("scheduled","ceo","g"),triggerRef:`s@${new FixedClock().value}`},new FixedClock().value);const human=activeTurn(ledger,"ceo","human","human");ledger.finishTurn(human.id,"completed",null,new FixedClock().value,"supervisor",["m"]);assert.equal(ledger.mailbox()[0]?.readAt!==null,true);assert.equal(ledger.schedules()[0]?.status,"consumed");assert.equal(ledger.wake("multi")?.status,"queued");ledger.close();});
...
test("replay rejects a structurally valid but unauthorized Turn binding transition",()=>{const ledger=new SqliteLedger(":memory:",{clock:new FixedClock()});const turn=activeTurn(ledger,"ceo","human","human");ledger.putGoal({id:"foreign",parentId:null,objective:"foreign",observationMethod:null,verificationMethod:null,owner:"worker",phase:"active",revision:0},"human");const poisoned={...turn,bindingKind:"goal",goalId:"foreign",goalRevision:0,specialistRole:null};ledger.db.prepare("INSERT INTO events(stream_id,stream_seq,ts,actor,type,data,projection_name) VALUES (?,?,?,?,?,json(?),?)").run(`turn:${turn.id}`,2,new FixedClock().value,"supervisor","turn.in_progress",JSON.stringify({snapshot:poisoned}),"turns");assert.throws(()=>ledger.rebuildProjections(),/invalid Human-to-Goal binding/);assert.equal(ledger.turn(turn.id)?.bindingKind,"human");ledger.close();});
...
function ensureThread(ledger:SqliteLedger,agent:string):import("goah-ledger-contract").ThreadSnapshot{let thread=ledger.threads().find((candidate)=>candidate.agent===agent);if(!thread){const now="2030-01-01T00:00:00.000Z";thread={id:`thread:${agent}`,agent,parentThreadId:null,createdAt:now,updatedAt:now};ledger.putThread(thread,"supervisor");}return thread;}
function automaticTurn(ledger:SqliteLedger,agent:string,id:string):import("goah-ledger-contract").TurnSnapshot{const thread=ensureThread(ledger,agent);return{id,threadId:thread.id,source:"system",bindingKind:"specialist",goalId:null,goalRevision:null,specialistRole:"verifier",status:"in_progress",attempt:1,error:null,startedAt:"2030-01-01T00:00:00.000Z",endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"lease",runnerPid:null};}
function activeTurn(ledger:SqliteLedger,agent:string,id:string,source:"human"|"system"="system"):import("goah-ledger-contract").TurnSnapshot{const existing=ledger.turn(id);if(existing)return existing;const thread=ensureThread(ledger,agent);if(source==="human"){const turn:import("goah-ledger-contract").TurnSnapshot={id,threadId:thread.id,source:"human",bindingKind:"human",goalId:null,goalRevision:null,specialistRole:null,status:"in_progress",attempt:1,error:null,startedAt:"2030-01-01T00:00:00.000Z",endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"lease",runnerPid:null};ledger.putTurn(turn,"human");return turn;}const claimed=ledger.wakes().find((candidate)=>candidate.agent===agent&&candidate.status==="claimed"&&candidate.bindingKind==="specialist")??(()=>{const wakeId=`wake:${id}`;ledger.enqueueWake(wake(wakeId,agent),"supervisor");return ledger.claimNextWake("2030-01-01T00:00:00.000Z");})();if(!claimed||claimed.agent!==agent)throw new Error("test failed to claim Specialist Wake");const turn=automaticTurn(ledger,agent,id);ledger.startTurnFromWake(claimed.id,turn,"2030-01-01T00:00:00.000Z");return turn;}
function startGoalTurn(ledger:SqliteLedger,agent:string,turnId:string,goalId:string):import("goah-ledger-contract").TurnSnapshot{const goal=ledger.goal(goalId)!;const thread=ensureThread(ledger,agent);let pending=ledger.wakes().find((candidate)=>candidate.agent===agent&&candidate.status==="queued"&&candidate.bindingKind==="goal"&&candidate.goalId===goalId);if(!pending){const wakeId=`wake:${turnId}`;ledger.enqueueWake(wake(wakeId,agent,goalId),"supervisor");pending=ledger.wake(wakeId)!;}const claimed=ledger.claimNextWake("2030-01-01T00:00:00.000Z");if(claimed?.id!==pending.id)throw new Error("test failed to claim Goal Wake");const turn:import("goah-ledger-contract").TurnSnapshot={id:turnId,threadId:thread.id,source:"goal",bindingKind:"goal",goalId,goalRevision:goal.revision,specialistRole:null,status:"in_progress",attempt:1,error:null,startedAt:"2030-01-01T00:00:00.000Z",endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"lease",runnerPid:null};ledger.startTurnFromWake(claimed.id,turn,"2030-01-01T00:00:00.000Z");return turn;}
function goalTurn(ledger:SqliteLedger,agent:string,turnId:string,goalId=`goal:${turnId}`):import("goah-ledger-contract").TurnSnapshot{ledger.putGoal({id:goalId,parentId:null,objective:"work",observationMethod:"observe",verificationMethod:"verify",owner:agent,phase:"active",revision:0},"human");const turn=startGoalTurn(ledger,agent,turnId,goalId);const evidence=ledger.appendEvent(event(agent,"observation",{ok:true}));ledger.updateWorkRecord({goalId,expectedRevision:0,goalRevision:0,content:"# Current State\n\nObserved current work.\n",reason:"record current work",evidence:[evidence.seq],turnId},agent);return ledger.turn(turnId)!;}
function handoffCommit(ledger:SqliteLedger,turnId:string,sourceWakeId:string|null,mailIds:string[]=[]):import("goah-ledger-contract").HandoffCommit{const ts="2030-01-01T00:00:02.000Z";const turn=ledger.turn(turnId)!;const record=ledger.workRecord(turn.goalId!)!;const response={content:"Goal progress recorded."};const handoff={goalId:turn.goalId!,goalRevision:turn.goalRevision!,recordRevision:record.recordRevision,outcome:"progress" as const,evidence:record.evidence};const ordinal=ledger.turnItems(turnId).length+1;return{agent:ledger.thread(turn.threadId)!.agent,turnId,sourceWakeId,mailIds,ts,output:{response,handoff},responseItem:{id:`response:${turnId}`,turnId,ordinal,type:"assistant_message",status:"completed",data:{text:response.content},createdAt:ts,completedAt:ts},item:{id:`handoff:${turnId}`,turnId,ordinal:ordinal+1,type:"handoff",status:"completed",data:handoff,createdAt:ts,completedAt:ts}};}
