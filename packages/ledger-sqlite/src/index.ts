...
const createWakes = `CREATE TABLE IF NOT EXISTS wakes (
  id TEXT PRIMARY KEY,
  target_kind TEXT NOT NULL CHECK(target_kind IN ('human','goal','specialist')),
  agent TEXT NOT NULL,
  ...
  CHECK((target_kind='human' AND agent='ceo' AND goal_id IS NULL AND specialist_role IS NULL) OR (target_kind='goal' AND goal_id IS NOT NULL AND specialist_role IS NULL) OR (target_kind='specialist' AND goal_id IS NULL AND specialist_role IS NOT NULL))
) STRICT;`;
...
export class SqliteLedger implements Ledger {
  ...
  commitReassignment(request: ReassignmentRequest, actor: string, wakeId?: string): ReassignmentResult {
    ...
    return this.#transaction(() => {
      ...
      const wakeBase: WakeSnapshot | null = goal.phase === "active" ? { id: `reassignment-wake:${request.id}`, ...goalTarget(goal.owner,goal.id), triggerRef: `reassignment:${request.id}`, status: "queued", attempt: 0, enqueuedSeq: 0, claimedAt: null, consumedAt: null, turnId: null } : null;
  ...
  #normalizeSchedule(value:ScheduleSnapshot,actor:string,binding?:{goalId:string|null}):ScheduleSnapshot{this.#assertExecutionTarget(value,false);if(actor!=="supervisor"&&actor!==value.agent)throw new Error("schedule may only be set by its agent or supervisor");if(value.setBy!==actor)throw new Error("schedule setBy does not match actor");const parsed=Date.parse(value.nextWakeAt);if(!value.id.trim()||!value.agent.trim()||!value.reason.trim()||!Number.isFinite(parsed))throw new Error("schedule requires an id, agent, valid time, and reason");const normalized={...value,nextWakeAt:new Date(parsed).toISOString()} as ScheduleSnapshot;if(normalized.status!=="pending"||normalized.resolvedAt!==null)throw new Error("new schedule must be pending");if(binding&&normalized.goalId!==binding.goalId)throw new Error("handoff schedule does not match the Turn Goal binding");if(normalized.targetKind==="goal"){const goal=this.#getGoal(normalized.goalId);if(!goal||goal.owner!==normalized.agent||goal.phase!=="active")throw new Error("schedule Goal target is inactive or owned by another Agent");}return normalized;}
  #assertExecutionTarget(value:{targetKind:string;agent:string;goalId:string|null;specialistRole:string|null},allowHuman:boolean):void{if(value.targetKind==="human"){if(!allowHuman||value.agent!=="ceo"||value.goalId!==null||value.specialistRole!==null)throw new Error("invalid Human execution target");return;}if(value.targetKind==="goal"){if(!value.goalId||value.specialistRole!==null)throw new Error("invalid Goal execution target");return;}if(value.targetKind!=="specialist"||value.goalId!==null||!value.specialistRole||!["verifier","audit"].includes(value.specialistRole))throw new Error("invalid Specialist execution target");}
