...
export class SqliteLedger implements Ledger {
  ...
  putThread(thread: ThreadSnapshot, actor: string): EventRecord {
    ...
    if (current && (current.agent !== thread.agent || current.role !== thread.role || current.parentThreadId !== thread.parentThreadId || current.createdAt !== thread.createdAt)) throw new Error("thread identity cannot change");
  ...
  putTurn(turn: TurnSnapshot, actor: string): EventRecord {
    ...
    } else {if(turn.status!=="in_progress"||turn.attempt!==1)throw new Error("new turn must start in progress at attempt one");if(actor!=="human"||thread.role!=="ceo"||turn.triggerKind!=="user_message")throw new Error("new automatic Turns must start from a claimed Wake");if(turn.goalId){const goal=this.#getGoal(turn.goalId);if(!goal||goal.parentId!==null||goal.owner!==thread.agent||goal.phase!=="active"||goal.revision!==turn.goalRevision)throw new Error("direct CEO Turn commitment must target the current active Root Goal");}}
  ...
  #assertReplayTurn(turn:TurnSnapshot,event:EventRecord):void{this.#assertTurnShape(turn);const thread=this.thread(turn.threadId);if(!thread)throw new Error(`turn event ${event.seq} has no Thread`);const current=this.turn(turn.id);if(!current){if(event.type!=="turn.started"||turn.status!=="in_progress"||turn.attempt!==1)throw new Error(`turn event ${event.seq} has an invalid initial state`);if(event.actor==="human"){if(turn.triggerKind!=="user_message"||thread.role!=="ceo")throw new Error(`turn event ${event.seq} has invalid user ownership`);if(turn.goalId){const goal=this.#getGoal(turn.goalId);if(!goal||goal.parentId!==null||goal.owner!==thread.agent||goal.phase!=="active"||goal.revision!==turn.goalRevision)throw new Error(`turn event ${event.seq} has an invalid direct Goal commitment`);}}else{if(event.actor!=="supervisor"||!this.wakes().some((wake)=>wake.status==="claimed"&&wake.agent===thread.agent&&sameWakeTurnTarget(wake,turn,thread)))throw new Error(`turn event ${event.seq} bypasses Wake admission`);this.#assertCurrentAutomaticTurn(turn,thread,event.seq);}return;}if(current.threadId!==turn.threadId||current.triggerKind!==turn.triggerKind||current.startedAt!==turn.startedAt||current.runnerProfileId!==turn.runnerProfileId)throw new Error(`turn event ${event.seq} changes Turn identity`);if(current.status!=="in_progress")throw new Error(`turn event ${event.seq} changes a terminal Turn`);const commitsGoal=event.type==="turn.goal_committed"&&event.actor==="supervisor"&&current.goalId===null&&turn.goalId!==null;if(!commitsGoal&&(current.goalId!==turn.goalId||current.goalRevision!==turn.goalRevision))throw new Error(`turn event ${event.seq} changes Goal commitment`);if(commitsGoal){const goal=this.#getGoal(turn.goalId!);if(!goal||goal.owner!==thread.agent||goal.phase!=="active"||goal.revision!==turn.goalRevision||thread.role==="ceo"&&goal.parentId!==null||thread.role==="child"&&goal.parentId===null||thread.role==="verifier"||thread.role==="audit")throw new Error(`turn event ${event.seq} has an invalid Goal commitment`);}if(turn.status==="in_progress"){if(!commitsGoal&&event.type!=="turn.in_progress"||turn.attempt!==current.attempt&&turn.attempt!==current.attempt+1)throw new Error(`turn event ${event.seq} has an invalid retry transition`);}else{if(event.type!==`turn.${turn.status}`||turn.attempt!==current.attempt)throw new Error(`turn event ${event.seq} has an invalid terminal transition`);if(turn.status==="completed"&&turn.goalId!==null)this.#assertReplayGoalCompletion(turn,event);}}
