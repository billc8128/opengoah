...
export class SqliteLedger implements Ledger {
  ...
  putTurn(turn: TurnSnapshot, actor: string): EventRecord {
    ...
    if (current) {
      ...
      const bindsGoal=actor==="supervisor"&&current.bindingKind==="human"&&turn.bindingKind==="goal"&&current.source==="human";if(current.bindingKind!==turn.bindingKind&&!bindsGoal||current.bindingKind===turn.bindingKind&&(current.goalId!==turn.goalId||current.goalRevision!==turn.goalRevision||current.specialistRole!==turn.specialistRole))throw new Error("turn execution binding cannot change");if(bindsGoal){const goal=this.#getGoal(turn.goalId);if(thread.agent!=="ceo"||!goal||goal.parentId!==null||goal.owner!=="ceo"||goal.phase!=="active"||goal.revision!==turn.goalRevision)throw new Error("Human Turn may bind only the current active CEO Root Goal");}
    ...
    } else {if(turn.status!=="in_progress"||turn.attempt!==1)throw new Error("new turn must start in progress at attempt one");if(actor!=="human"||thread.agent!=="ceo"||turn.bindingKind!=="human")throw new Error("new automatic Turns must start from a claimed Wake");}
  ...
  updateWorkRecord(request: WorkRecordUpdateRequest, actor: string): WorkRecordSnapshot {
    ...
    const turn = this.turn(request.turnId); if (!turn || turn.status !== "in_progress" || turn.bindingKind!=="goal" || turn.goalId !== goal.id || turn.goalRevision !== request.goalRevision) throw new Error("work record update does not match an active Goal-bound Turn");
  ...
  commitReassignment(request: ReassignmentRequest, actor: string, wakeId?: string): ReassignmentResult {
    ...
    return this.#transaction(() => {
      ...
      const wakeBase: WakeSnapshot | null = goal.phase === "active" ? { id: `reassignment-wake:${request.id}`, ...goalExecutionBinding(goal.owner,goal.id), triggerRef: `reassignment:${request.id}`, status: "queued", attempt: 0, enqueuedSeq: 0, claimedAt: null, consumedAt: null, turnId: null } : null;
  ...
  startTurnFromWake(id:string,turn:TurnSnapshot,now:string):WakeSnapshot{const wake=this.#requiredWake(id);if(wake.status!=="claimed")throw new Error("only a claimed Wake may start a Turn");if(this.turn(turn.id))throw new Error("Wake Turn already exists");this.#assertTurnBinding(turn);if(turn.status!=="in_progress"||turn.attempt!==1||!turn.leaseUntil||!turn.leaseToken)throw new Error("Wake must start an owned Turn at attempt one");const thread=this.thread(turn.threadId);if(!thread||thread.agent!==wake.agent)throw new Error("Wake Turn agent does not match");if(!sameWakeTurnBinding(wake,turn))throw new Error("Wake Turn execution binding does not match");return this.#transaction(()=>{if(this.db.prepare("SELECT 1 FROM turns WHERE source='human' AND status='in_progress' LIMIT 1").get())throw new Error("automatic Wake cannot start while a Human Turn is in progress");this.#assertCurrentAutomaticTurn(turn,thread.agent);this.#recordProjection("turns",turn,"supervisor","turn.started",undefined,now,undefined,`turn:${turn.id}`);this.#resolveWakeTriggers(id,now);const next:WakeSnapshot={...wake,status:"consumed",consumedAt:now,turnId:turn.id};this.#recordProjection("wakes",next,"supervisor","wake.consumed",id,now);return next;});}
  ...
  #assertGoalProvenance(snapshot:GoalSnapshot,actor:string,authority:GoalChangeAuthority,sourceTurnId?:string,sourceWakeId?:string):void{if(sourceWakeId&&!sourceTurnId)throw new Error("Goal change source Wake requires its source Turn");const turn=sourceTurnId?this.turn(sourceTurnId):null;if(sourceTurnId&&!turn)throw new Error("Goal change source Turn does not exist");if(turn){if(turn.status!=="in_progress")throw new Error("Goal change source Turn is not active");const thread=this.thread(turn.threadId);if(!thread)throw new Error("Goal change source Turn has no Thread");if(authority.kind==="human"){const legalBinding=turn.bindingKind==="human"||turn.bindingKind==="goal"&&turn.goalId===snapshot.id;if(turn.source!=="human"||thread.agent!=="ceo"||!legalBinding)throw new Error("Goal change source Turn does not carry Human authority");}else if(authority.kind==="parent_goal"){if(thread.agent!==actor||turn.bindingKind!=="goal"||turn.goalId!==authority.goalId||turn.goalRevision!==authority.goalRevision)throw new Error("Goal change source Turn is not bound to the authorizing parent Goal");}else if(thread.agent!==actor)throw new Error("Goal change source Turn actor does not match");}if(sourceWakeId){const wake=this.wake(sourceWakeId);if(!wake)throw new Error("Goal change source Wake does not exist");if(wake.status!=="consumed"||sourceTurnId&&wake.turnId!==sourceTurnId)throw new Error("Goal change source Wake does not match its Turn");if(authority.kind==="parent_goal"&&(wake.bindingKind!=="goal"||wake.agent!==actor||wake.goalId!==authority.goalId))throw new Error("Goal change source Wake is not bound to the authorizing parent Goal");}}
  ...
  #admitWake(input:WakeSnapshot,actor:string,allowExisting:boolean):{event:EventRecord;created:boolean;wake:WakeSnapshot}{
    ...
    if(duplicate){
      ...
      if(!allowExisting||!sameBinding)throw new Error("Wake trigger was reused with a different execution binding");
    ...
    if(input.bindingKind==="goal"){const goal=this.#getGoal(input.goalId);if(!goal||goal.owner!==input.agent||goal.phase!=="active")throw new Error("Wake Goal binding is inactive or owned by another Agent");}
  ...
  #normalizeSchedule(value:ScheduleSnapshot,actor:string,binding?:{goalId:string|null}):ScheduleSnapshot{this.#assertExecutionBinding(value,false);if(actor!=="supervisor"&&actor!==value.agent)throw new Error("schedule may only be set by its agent or supervisor");if(value.setBy!==actor)throw new Error("schedule setBy does not match actor");const parsed=Date.parse(value.nextWakeAt);if(!value.id.trim()||!value.agent.trim()||!value.reason.trim()||!Number.isFinite(parsed))throw new Error("schedule requires an id, agent, valid time, and reason");const normalized={...value,nextWakeAt:new Date(parsed).toISOString()} as ScheduleSnapshot;if(normalized.status!=="pending"||normalized.resolvedAt!==null)throw new Error("new schedule must be pending");if(binding&&normalized.goalId!==binding.goalId)throw new Error("handoff schedule does not match the Turn Goal binding");if(normalized.bindingKind==="goal"){const goal=this.#getGoal(normalized.goalId);if(!goal||goal.owner!==normalized.agent||goal.phase!=="active")throw new Error("schedule Goal target is inactive or owned by another Agent");}return normalized;}
  #assertExecutionBinding(value:{bindingKind:string;agent:string;goalId:string|null;specialistRole:string|null},allowHuman:boolean):void{if(value.bindingKind==="human"){if(!allowHuman||value.agent!=="ceo"||value.goalId!==null||value.specialistRole!==null)throw new Error("invalid Human execution binding");return;}if(value.bindingKind==="goal"){if(!value.goalId||value.specialistRole!==null)throw new Error("invalid Goal execution binding");return;}if(value.bindingKind!=="specialist"||value.goalId!==null||!value.specialistRole||!["verifier","audit"].includes(value.specialistRole))throw new Error("invalid Specialist execution binding");}
  #assertTurnBinding(value:{source:string;bindingKind:string;goalId:string|null;goalRevision:number|null;specialistRole:string|null}):void{if(value.bindingKind==="human"){if(value.source!=="human"||value.goalId!==null||value.goalRevision!==null||value.specialistRole!==null)throw new Error("invalid Human Turn binding");return;}if(value.bindingKind==="goal"){if(!["human","goal"].includes(value.source)||!value.goalId||value.goalRevision===null||value.specialistRole!==null)throw new Error("invalid Goal Turn binding");return;}if(value.bindingKind!=="specialist"||value.source!=="system"||value.goalId!==null||value.goalRevision!==null||!value.specialistRole||!["verifier","audit"].includes(value.specialistRole))throw new Error("invalid Specialist Turn binding");}
  ...
  #assertReplayTurn(turn:TurnSnapshot,event:EventRecord):void{this.#assertTurnBinding(turn);const thread=this.thread(turn.threadId);if(!thread)throw new Error(`turn event ${event.seq} has no Thread`);const current=this.turn(turn.id);if(!current){if(event.type!=="turn.started"||turn.status!=="in_progress"||turn.attempt!==1)throw new Error(`turn event ${event.seq} has an invalid initial state`);if(event.actor==="human"){if(turn.bindingKind!=="human"||thread.agent!=="ceo")throw new Error(`turn event ${event.seq} has invalid Human ownership`);}else{if(event.actor!=="supervisor"||!this.wakes().some((wake)=>wake.status==="claimed"&&wake.agent===thread.agent&&sameWakeTurnBinding(wake,turn)))throw new Error(`turn event ${event.seq} bypasses Wake admission`);this.#assertCurrentAutomaticTurn(turn,thread.agent,event.seq);}return;}if(current.threadId!==turn.threadId||current.source!==turn.source||current.startedAt!==turn.startedAt||current.runnerProfileId!==turn.runnerProfileId)throw new Error(`turn event ${event.seq} changes Turn identity`);if(current.status!=="in_progress")throw new Error(`turn event ${event.seq} changes a terminal Turn`);const bindsGoal=event.actor==="supervisor"&&current.bindingKind==="human"&&turn.bindingKind==="goal"&&current.source==="human";if(current.bindingKind!==turn.bindingKind&&!bindsGoal||current.bindingKind===turn.bindingKind&&(current.goalId!==turn.goalId||current.goalRevision!==turn.goalRevision||current.specialistRole!==turn.specialistRole))throw new Error(`turn event ${event.seq} changes execution binding`);if(bindsGoal){const goal=this.#getGoal(turn.goalId);if(thread.agent!=="ceo"||!goal||goal.parentId!==null||goal.owner!=="ceo"||goal.phase!=="active"||goal.revision!==turn.goalRevision)throw new Error(`turn event ${event.seq} has an invalid Human-to-Goal binding`);}if(turn.status==="in_progress"){if(event.type!=="turn.in_progress"||turn.attempt!==current.attempt&&turn.attempt!==current.attempt+1)throw new Error(`turn event ${event.seq} has an invalid retry transition`);}else{if(event.type!==`turn.${turn.status}`||turn.attempt!==current.attempt)throw new Error(`turn event ${event.seq} has an invalid terminal transition`);if(turn.status==="completed"&&turn.bindingKind==="goal")this.#assertReplayGoalCompletion(turn,event);}}
  #assertReplayGoalCompletion(turn:Extract<TurnSnapshot,{bindingKind:"goal"}>,event:EventRecord):void{const items=this.turnItems(turn.id);const responses=items.filter((item)=>item.type==="assistant_message"&&item.status==="completed"&&typeof (item.data as {text?:unknown}).text==="string"&&Boolean((item.data as {text:string}).text.trim()));const handoffItem=items.findLast((item)=>item.type==="handoff"&&item.status==="completed");const responseRow=this.db.prepare("SELECT data FROM events WHERE stream_id=? AND type='response.committed' AND seq<? ORDER BY seq DESC LIMIT 1").get(`turn:${turn.id}`,event.seq) as {data:string}|undefined;const handoffRow=this.db.prepare("SELECT data FROM events WHERE stream_id=? AND type='handoff.recorded' AND seq<? ORDER BY seq DESC LIMIT 1").get(`turn:${turn.id}`,event.seq) as {data:string}|undefined;if(!responseRow||!handoffRow||!handoffItem)throw new Error(`turn event ${event.seq} completes a Goal without response and Handoff`);const response=JSON.parse(responseRow.data) as {messageItemId?:unknown;text?:unknown};const handoff=JSON.parse(handoffRow.data) as unknown as Handoff;const responseItem=responses.find((item)=>item.id===response.messageItemId);if(!responseItem||(responseItem.data as {text?:unknown}).text!==response.text||!isDeepStrictEqual(handoffItem.data,handoff))throw new Error(`turn event ${event.seq} has mismatched response or Handoff facts`);assertHandoff(handoff);const record=this.workRecord(turn.goalId);if(handoff.goalId!==turn.goalId||handoff.goalRevision!==turn.goalRevision||record?.recordRevision!==handoff.recordRevision)throw new Error(`turn event ${event.seq} has a Handoff for another Goal or Work Record`);for(const seq of handoff.evidence)if(seq>=event.seq||!this.db.prepare("SELECT 1 FROM events WHERE seq=?").get(seq))throw new Error(`turn event ${event.seq} has invalid Handoff evidence`);}
  #assertReplayWake(wake:WakeSnapshot,event:EventRecord):void{this.#assertExecutionBinding(wake,true);const current=this.wake(wake.id);if(!current){if(event.type!=="wake.enqueued"||wake.status!=="queued"||wake.attempt!==0||wake.claimedAt||wake.consumedAt||wake.turnId)throw new Error(`wake event ${event.seq} has an invalid initial state`);if(wake.bindingKind==="goal"){const goal=this.#getGoal(wake.goalId);if(!goal||goal.phase!=="active"||goal.owner!==wake.agent)throw new Error(`wake event ${event.seq} has an invalid Goal binding`);}return;}if(current.bindingKind!==wake.bindingKind||current.agent!==wake.agent||current.goalId!==wake.goalId||current.specialistRole!==wake.specialistRole||current.triggerRef!==wake.triggerRef||current.enqueuedSeq!==wake.enqueuedSeq)throw new Error(`wake event ${event.seq} changes Wake identity`);if(current.status==="consumed"||current.status==="cancelled")throw new Error(`wake event ${event.seq} changes a terminal Wake`);if(wake.status==="claimed"){if(event.type!=="wake.claimed"||current.status!=="queued"||wake.attempt!==current.attempt+1||!wake.claimedAt||wake.consumedAt!==null||wake.turnId!==null)throw new Error(`wake event ${event.seq} has an invalid claim transition`);}else if(wake.status==="queued"){if(event.type!=="wake.released"||current.status!=="claimed"||wake.attempt!==current.attempt||wake.claimedAt!==null||wake.consumedAt!==null||wake.turnId!==null)throw new Error(`wake event ${event.seq} has an invalid release transition`);}else if(wake.status==="cancelled"){if(event.type!=="wake.cancelled"||wake.attempt!==current.attempt||wake.claimedAt!==current.claimedAt||wake.consumedAt===null||wake.turnId!==null)throw new Error(`wake event ${event.seq} has an invalid cancel transition`);}else{if(event.type!=="wake.consumed"||current.status!=="claimed"||wake.attempt!==current.attempt||wake.claimedAt!==current.claimedAt||wake.consumedAt===null||!wake.turnId)throw new Error(`wake event ${event.seq} has an invalid consume transition`);const turn=this.turn(wake.turnId);const thread=turn?this.thread(turn.threadId):null;if(!turn||thread?.agent!==wake.agent||!sameWakeTurnBinding(wake,turn))throw new Error(`wake event ${event.seq} does not match its Turn`);}}
  #assertCurrentAutomaticTurn(turn:TurnSnapshot,agent:string,seq?:number):void{if(turn.bindingKind==="human"){if(agent!=="ceo")throw new Error(seq?`turn event ${seq} has invalid Human ownership`:"Human Wake may start only the CEO Turn");return;}if(turn.bindingKind==="goal"){const goal=this.#getGoal(turn.goalId);if(!goal||goal.phase!=="active"||goal.owner!==agent||goal.revision!==turn.goalRevision)throw new Error(seq?`turn event ${seq} has a stale Goal binding`:"Wake Turn must bind the current active Goal revision owned by its Agent");}}
...
function mapSchedule(r: Row): ScheduleSnapshot { return {id:String(r.id),bindingKind:String(r.binding_kind),agent:String(r.agent),specialistRole:r.specialist_role===null?null:String(r.specialist_role),goalId:r.goal_id===null?null:String(r.goal_id),nextWakeAt:String(r.next_wake_at),reason:String(r.reason),setBy:String(r.set_by),status:String(r.status),resolvedAt:r.resolved_at===null?null:String(r.resolved_at)} as ScheduleSnapshot; }
function mapWake(r: Row): WakeSnapshot { return {id:String(r.id),bindingKind:String(r.binding_kind),agent:String(r.agent),specialistRole:r.specialist_role===null?null:String(r.specialist_role),goalId:r.goal_id===null?null:String(r.goal_id),triggerRef:String(r.trigger_ref),status:String(r.status),attempt:Number(r.attempt),enqueuedSeq:Number(r.enqueued_seq),claimedAt:r.claimed_at===null?null:String(r.claimed_at),consumedAt:r.consumed_at===null?null:String(r.consumed_at),turnId:r.turn_id===null?null:String(r.turn_id)} as WakeSnapshot; }
function sameWakeTurnBinding(wake:WakeSnapshot,turn:TurnSnapshot):boolean{if(wake.bindingKind!==turn.bindingKind)return false;if(wake.bindingKind==="human")return turn.bindingKind==="human"&&turn.source==="human";if(wake.bindingKind==="goal")return turn.bindingKind==="goal"&&wake.goalId===turn.goalId;return turn.bindingKind==="specialist"&&wake.specialistRole===turn.specialistRole&&turn.source==="system";}
