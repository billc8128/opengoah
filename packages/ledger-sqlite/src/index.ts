...
export class SqliteLedger implements Ledger {
  ...
  #assertCurrentAutomaticTurn(turn:TurnSnapshot,thread:ThreadSnapshot,seq?:number):void{if(turn.triggerKind!=="wake")throw new Error(seq?`turn event ${seq} is not Wake-triggered`:"automatic Turn must be Wake-triggered");if(turn.goalId!==null){const goal=this.#getGoal(turn.goalId);if(!goal||goal.phase!=="active"||goal.owner!==thread.agent||goal.revision!==turn.goalRevision||thread.role==="ceo"&&goal.parentId!==null||thread.role==="child"&&goal.parentId===null)throw new Error(seq?`turn event ${seq} has a stale Goal commitment`:"Wake Turn must commit to the current active Goal revision owned by its Agent");return;}if(thread.role!=="verifier"&&thread.role!=="audit")throw new Error(seq?`turn event ${seq} has an uncommitted Goal-owning Agent`:"only Specialists may start an uncommitted Wake Turn");}
