...
export class SqliteLedger implements Ledger {
  ...
  commitHandoff(commit: HandoffCommit): EventRecord {
    return this.#transaction(() => {
      ...
      this.#insertEvent({streamId,ts:commit.ts,actor:commit.agent,type:"message.assistant.completed",data:{message:{role:"assistant",content:[{type:"text",text:commit.output.response.content}],stopReason:"stop"}}});
      this.#admitTurnItem(commit.responseItem,commit.agent,streamId);
