...
import {
  ...
  type HandoffCommit,
  type Handoff,
  type HumanTurnAdmissionRequest,
  type HumanTurnAdmissionResult,
  type JsonValue,
  type Ledger,
  type MailSnapshot,
...
CREATE TABLE IF NOT EXISTS mailbox (
  id TEXT PRIMARY KEY,
  route_kind TEXT NOT NULL CHECK(route_kind IN ('goal','ceo_inbox','specialist_inbox')),
  ...
  read_at TEXT,
  CHECK((route_kind='goal' AND goal_id IS NOT NULL AND specialist_role IS NULL) OR (route_kind='ceo_inbox' AND to_agent='ceo' AND from_agent IN ('verifier','audit','supervisor') AND goal_id IS NULL AND specialist_role IS NULL) OR (route_kind='specialist_inbox' AND goal_id IS NULL AND specialist_role IS NOT NULL))
) STRICT;
${indexesAndTriggers}`;
...
export class SqliteLedger implements Ledger {
  ...
  putTurn(turn: TurnSnapshot, actor: string): EventRecord {
    ...
    if(turn.status==="in_progress"&&(!turn.leaseUntil||!turn.leaseToken)||turn.status!=="in_progress"&&(turn.leaseUntil!==null||turn.leaseToken!==null))throw new Error("Turn execution ownership does not match status");
    const thread=this.thread(turn.threadId);if(!thread)throw new Error("Turn Thread does not exist");
    ...
    if (current) {
      ...
      if (current.threadId !== turn.threadId || current.triggerKind !== turn.triggerKind) throw new Error("turn identity cannot change");
      if(current.runnerProfileId!==turn.runnerProfileId)throw new Error("turn Runner Profile cannot change");
      if(current.runnerPid!==turn.runnerPid)throw new Error("Turn Runner process may change only through attach/release APIs");
      if (current.status !== "in_progress") throw new Error("terminal turn cannot change");
      if(current.goalId!==turn.goalId||current.goalRevision!==turn.goalRevision)throw new Error("Turn Goal commitment cannot change through putTurn");
    ...
    } else throw new Error("new Human Turns require atomic admitHumanTurn; automatic Turns require a claimed Wake");
    return this.#project("turns", turn, actor, current ? `turn.${turn.status}` : "turn.started", undefined, undefined, `turn:${turn.id}`);
  }

  admitHumanTurn(request:HumanTurnAdmissionRequest):HumanTurnAdmissionResult{return this.#transaction(()=>{
    const {thread,turn,messageItem}=request;this.#admitThread(thread,"supervisor");
    if(thread.agent!=="ceo"||thread.role!=="ceo"||thread.parentThreadId!==null)throw new Error("Human Turn requires the canonical CEO Thread");
    const messageData=messageItem.data&&typeof messageItem.data==="object"&&!Array.isArray(messageItem.data)?messageItem.data as Record<string,JsonValue>:null;if(!messageData||typeof messageData.text!=="string"||!messageData.text.trim())throw new Error("Human Turn admission requires a readable user message");
    const active=this.activeTurn(thread.id);if(request.replaceTurnId===null&&active)throw new Error("Human Turn admission requires the active Turn to be replaced");if(request.replaceTurnId!==null&&active?.id!==request.replaceTurnId)throw new Error("Human Turn replacement lost its active Turn race");
    let goal:GoalSnapshot|null=null;
    if(request.rootGoal){goal=normalizeGoal(request.rootGoal);assertGoalSnapshot(goal);if(goal.parentId!==null||goal.owner!=="ceo"||goal.phase!=="active"||goal.revision!==0)throw new Error("Human Goal admission requires a new active CEO Root Goal");if(this.#getGoal(goal.id))throw new Error("Goal id is already in use");if(this.goals().some((candidate)=>candidate.parentId===null&&candidate.phase!=="complete"))throw new Error("CEO already has an unfinished Root Goal");this.#assertOwnerSeparation(goal.id,null,goal.owner);this.#recordGoalChange(goal,null,"human",undefined,{operation:"create",reason:"Human established a durable Root Goal",evidence:[],authority:{kind:"human"}});this.#createWorkRecord(goal,"human",`goal:create:${goal.id}`);this.#faultInjector?.("after_human_goal_before_turn");}
    this.#assertTurnShape(turn);if(turn.threadId!==thread.id||turn.triggerKind!=="user_message"||turn.status!=="in_progress"||turn.attempt!==1||turn.endedAt!==null||!turn.leaseUntil||!turn.leaseToken||turn.runnerPid!==null)throw new Error("invalid Human Turn admission");
    if(goal){if(turn.goalId!==goal.id||turn.goalRevision!==goal.revision)throw new Error("Human Goal Turn does not commit to its admitted Root Goal");}else if(turn.goalId!==null){const existing=this.#getGoal(turn.goalId);if(!existing||existing.parentId!==null||existing.owner!=="ceo"||existing.phase!=="active"||existing.revision!==turn.goalRevision)throw new Error("Human Turn commitment is not the current active Root Goal");}
    let replaced:TurnSnapshot|null=null;if(active){replaced=this.#finishTurn(active,"interrupted",{message:"interrupted by Human"},turn.startedAt,"human",[]);this.#faultInjector?.("after_human_replacement_before_turn");}
    this.#recordProjection("turns",turn,"human","turn.started",undefined,turn.startedAt,undefined,`turn:${turn.id}`);
    if(messageItem.turnId!==turn.id||messageItem.ordinal!==1||messageItem.type!=="user_message"||messageItem.status!=="completed"||messageItem.createdAt!==turn.startedAt||messageItem.completedAt!==turn.startedAt)throw new Error("Human Turn admission requires one completed initial user message");this.#admitTurnItem(messageItem,"human");
    return{turn,replacedTurn:replaced,goal};
  });}

  commitTurnToGoal(turnId:string,binding:{goalId:string;goalRevision:number},actor:string):TurnSnapshot{
    if(actor!=="supervisor")throw new Error("only supervisor may commit a Turn to a Goal");
    return this.#transaction(()=>{
      const current=this.#requiredTurn(turnId);if(current.status!=="in_progress")throw new Error("only an active Turn may commit to a Goal");
      if(current.goalId!==null){if(current.goalId===binding.goalId&&current.goalRevision===binding.goalRevision)return current;throw new Error("Turn Goal commitment is immutable");}
      const thread=this.thread(current.threadId);const goal=this.#getGoal(binding.goalId);
      if(!thread||!goal||goal.phase!=="active"||goal.owner!==thread.agent||goal.revision!==binding.goalRevision)throw new Error("Turn commitment must target the current active Goal owned by its Agent");
      if(thread.role==="ceo"&&goal.parentId!==null||thread.role==="child"&&goal.parentId===null||thread.role==="verifier"||thread.role==="audit")throw new Error("Thread role cannot commit to this Goal");
      const next={...current,goalId:goal.id,goalRevision:goal.revision};this.#recordProjection("turns",next,"supervisor","turn.goal_committed",undefined,this.#now(),undefined,`turn:${turnId}`);return next;
    });
  }
  ...
  updateWorkRecord(request: WorkRecordUpdateRequest, actor: string): WorkRecordSnapshot {
    ...
    const turn = this.turn(request.turnId); if (!turn || turn.status !== "in_progress" || turn.goalId !== goal.id || turn.goalRevision !== request.goalRevision) throw new Error("work record update does not match an active committed Turn");
  ...
  commitReassignment(request: ReassignmentRequest, actor: string, wakeId?: string): ReassignmentResult {
    ...
    return this.#transaction(() => {
      ...
      const mail: MailSnapshot[] = [{ id: `reassignment-new-mail:${request.id}`, to: goal.owner, from: actor, level: "decision", ...goalRoute(goal.id), body: { type: "reassignment", reassignmentId: request.id, role: "new_owner", previousOwner:current.owner, brief: request.brief, reason: request.reason, evidence: request.evidence }, readAt: null }];
      const wakeBase: WakeSnapshot | null = goal.phase === "active" ? { id: `reassignment-wake:${request.id}`, ...goalAutomaticTarget(goal.owner,goal.id), triggerRef: `reassignment:${request.id}`, status: "queued", attempt: 0, enqueuedSeq: 0, claimedAt: null, consumedAt: null, turnId: null } : null;
  ...
  completeGoal(request: GoalCompletionRequest, actor: string, wakeId?: string): GoalSnapshot {
    ...
    if (current.parentId !== null) this.#assertChildCompletionRecord(current, request.evidence);
  ...
  consumeSchedule(id:string,wake:WakeSnapshot,now:string):{schedule:ScheduleSnapshot;wake:WakeSnapshot}{const current=this.#requiredSchedule(id);if(current.status!=="pending")throw new Error("only a pending Schedule may be consumed");if(wake.targetKind!==current.targetKind||wake.agent!==current.agent||wake.specialistRole!==current.specialistRole||wake.triggerRef!==`${current.id}@${current.nextWakeAt}`||wake.goalId!==current.goalId)throw new Error("scheduled Wake does not match Schedule");if(wake.status!=="queued"||wake.attempt!==0||wake.claimedAt||wake.consumedAt||wake.turnId)throw new Error("scheduled Wake must be pristine");return this.#transaction(()=>{const exact=this.wakeByTrigger(wake.agent,wake.triggerRef);let stored:WakeSnapshot;if(exact){if(exact.status!=="queued"||exact.targetKind!==wake.targetKind||exact.specialistRole!==wake.specialistRole||exact.goalId!==wake.goalId)throw new Error("scheduled Wake trigger is already terminal or bound differently");stored=exact;}else{const queued=this.wakes().find((candidate)=>candidate.agent===wake.agent&&candidate.status==="queued"&&candidate.targetKind===wake.targetKind&&candidate.specialistRole===wake.specialistRole&&candidate.goalId===wake.goalId);if(queued){stored=queued;this.#admitWakeTrigger(queued.id,wake.triggerRef,"supervisor",now);}else stored=this.#admitWake(wake,"supervisor",false).wake;}const schedule={...current,status:"consumed" as const,resolvedAt:now};this.#recordProjection("schedule",schedule,"supervisor","schedule.consumed",undefined,now);return{schedule,wake:stored};});}
  ...
  startTurnFromWake(id:string,turn:TurnSnapshot,now:string):WakeSnapshot{const wake=this.#requiredWake(id);if(wake.status!=="claimed")throw new Error("only a claimed Wake may start a Turn");if(this.turn(turn.id))throw new Error("Wake Turn already exists");this.#assertTurnShape(turn);if(turn.status!=="in_progress"||turn.attempt!==1||!turn.leaseUntil||!turn.leaseToken||turn.triggerKind!=="wake"||turn.runnerPid!==null)throw new Error("Wake must start an owned Turn at attempt one without a Runner process");const thread=this.thread(turn.threadId);if(!thread||thread.agent!==wake.agent)throw new Error("Wake Turn agent does not match");if(!sameWakeTurnTarget(wake,turn,thread))throw new Error("Wake Turn target does not match");return this.#transaction(()=>{if(this.db.prepare("SELECT 1 FROM turns WHERE trigger_kind='user_message' AND status='in_progress' LIMIT 1").get())throw new Error("automatic Wake cannot start while a user Turn is in progress");this.#assertCurrentAutomaticTurn(turn,thread);this.#recordProjection("turns",turn,"supervisor","turn.started",undefined,now,undefined,`turn:${turn.id}`);this.#resolveWakeTriggers(id,now);const next:WakeSnapshot={...wake,status:"consumed",consumedAt:now,turnId:turn.id};this.#recordProjection("wakes",next,"supervisor","wake.consumed",id,now);return next;});}
  ...
  attachTurnProcess(id: string, leaseToken: string, pid: number): TurnSnapshot {
    ...
    if (current.status !== "in_progress" || current.runnerPid!==null || !Number.isInteger(pid) || pid<=0) throw new Error("a positive Runner pid may attach once to an active Turn");
    return this.#transaction(()=>{const next={...current,runnerPid:pid};this.#recordProjection("turns",next,"supervisor","turn.process_attached",undefined,this.#now(),undefined,`turn:${id}`);return next;});
  }
  ...
  finishTurn(id:string,status:"completed"|"failed"|"interrupted",error:JsonValue|null,now:string,actor:string,mailIds:string[]=[]):TurnSnapshot{if(status==="completed")throw new Error("successful Turn completion requires commitTurnResponse or commitHandoff");const current=this.#requiredTurn(id);return this.#transaction(()=>this.#finishTurn(current,status,error,now,actor,mailIds));}

  commitTurnResponse(id:string,responseItemId:string,now:string,actor:string,mailIds:string[]=[]):TurnSnapshot{return this.#transaction(()=>{const current=this.#requiredTurn(id);if(current.goalId!==null)throw new Error("Goal-bound Turn response requires commitHandoff");const responseItem=this.turnItems(id).find((item)=>item.id===responseItemId);const text=responseItem&&responseItem.type==="assistant_message"&&responseItem.status==="completed"?normalizeAssistantText(String((responseItem.data as {text?:unknown}).text??"")):"";if(!responseItem||!text)throw new Error("Turn response does not match a readable Assistant Item");const streamId=`turn:${id}`;this.#insertEvent({streamId,ts:now,actor,type:"response.committed",data:{messageItemId:responseItem.id,text}});return this.#finishTurn(current,"completed",null,now,actor,mailIds);});}

  releaseTurnProcess(id:string,actor:string):TurnSnapshot{if(actor!=="supervisor")throw new Error("only supervisor may release Runner process ownership");return this.#transaction(()=>{const current=this.#requiredTurn(id);if(current.runnerPid===null)throw new Error("Turn has no Runner process ownership to release");const next={...current,runnerPid:null};this.#recordProjection("turns",next,actor,"turn.process_released",undefined,this.#now(),undefined,`turn:${id}`);return next;});}

  putMail(mail: MailSnapshot, actor: string, wakeId?: string): EventRecord { return this.putMails([mail],actor,wakeId)[0]!; }

  putMails(mails:MailSnapshot[],actor:string,wakeId?:string):EventRecord[]{const ids=new Set<string>();const prior=new Map<string,EventRecord>();for(const mail of mails){this.#assertMailRequest(mail,actor);if(ids.has(mail.id))throw new Error("mail batch contains a duplicate id");ids.add(mail.id);const row=this.db.prepare("SELECT * FROM mailbox WHERE id=?").get(mail.id) as Row|undefined;if(!row)continue;const current=mapMail(row);if(current.routeKind!==mail.routeKind||current.to!==mail.to||current.from!==mail.from||current.specialistRole!==mail.specialistRole||current.level!==mail.level||current.goalId!==mail.goalId||!isDeepStrictEqual(current.body,mail.body))throw new Error("mail id was reused with different content");const event=this.db.prepare("SELECT * FROM events WHERE type='mail.put' AND json_extract(data,'$.snapshot.id')=? ORDER BY seq LIMIT 1").get(mail.id) as Row|undefined;if(!event)throw new Error("mail projection has no source event");prior.set(mail.id,mapEvent(event));}return this.#transaction(()=>mails.map((mail)=>prior.get(mail.id)??this.#recordProjection("mailbox",mail,actor,"mail.put",wakeId)));}
  ...
  commitHandoff(commit: HandoffCommit): EventRecord {
    return this.#transaction(() => {
      ...
      if(turn.goalId===null)throw new Error("only a committed Turn may commit Handoff");const goal=this.#getGoal(turn.goalId);if(!goal||goal.owner!==commit.agent||goal.phase!=="active"||goal.revision!==turn.goalRevision)throw new Error("Handoff Turn Goal commitment is stale");const record=this.workRecord(goal.id);if(!record||record.updatedInTurn!==turn.id||record.goalRevision!==goal.revision||record.recordRevision<1)throw new Error("committed Turn must update its Work Record before Handoff");assertHandoff(commit.output.handoff);if(commit.output.handoff.goalId!==goal.id||commit.output.handoff.goalRevision!==goal.revision||commit.output.handoff.recordRevision!==record.recordRevision)throw new Error("Goal Handoff does not match the current Goal and Work Record");this.#assertEvidenceExists(commit.output.handoff.evidence);
      const responseItem=this.turnItems(turn.id).find((item)=>item.id===commit.responseItemId);if(!responseItem||responseItem.type!=="assistant_message"||responseItem.status!=="completed"||typeof (responseItem.data as {text?:unknown}).text!=="string"||!(responseItem.data as {text:string}).text.trim())throw new Error("Handoff response does not match a readable Turn Item");const ordinal=this.turnItems(turn.id).length+1;if(commit.item.turnId!==turn.id||commit.item.type!=="handoff"||commit.item.status!=="completed"||commit.item.completedAt!==commit.ts||commit.item.createdAt!==commit.ts||commit.item.ordinal!==ordinal||!isDeepStrictEqual(commit.item.data,commit.output.handoff))throw new Error("handoff Item does not match active TurnOutput");
      ...
      this.#insertEvent({streamId,ts:commit.ts,actor:commit.agent,type:"response.committed",data:{messageItemId:responseItem.id,text:(responseItem.data as {text:string}).text}});
      ...
      this.#insertEvent({streamId,ts:commit.ts,actor:"supervisor",type:"transcript.completed",data:{}});this.#recordProjection("turns",{...turn,status:"completed",error:null,endedAt:commit.ts,leaseUntil:null,leaseToken:null},"supervisor","turn.completed",undefined,commit.ts,undefined,streamId);
      return event;
    });
  }
  ...
  #assertGoalProvenance(snapshot:GoalSnapshot,actor:string,authority:GoalChangeAuthority,sourceTurnId?:string,sourceWakeId?:string):void{if(sourceWakeId&&!sourceTurnId)throw new Error("Goal change source Wake requires its source Turn");const turn=sourceTurnId?this.turn(sourceTurnId):null;if(sourceTurnId&&!turn)throw new Error("Goal change source Turn does not exist");if(turn){if(turn.status!=="in_progress")throw new Error("Goal change source Turn is not active");const thread=this.thread(turn.threadId);if(!thread)throw new Error("Goal change source Turn has no Thread");if(authority.kind==="human"){const legalCommitment=turn.goalId===null||turn.goalId===snapshot.id;if(turn.triggerKind!=="user_message"||thread.role!=="ceo"||!legalCommitment)throw new Error("Goal change source Turn does not carry Human authority");}else if(authority.kind==="parent_goal"){if(thread.agent!==actor||turn.goalId!==authority.goalId||turn.goalRevision!==authority.goalRevision)throw new Error("Goal change source Turn is not committed to the authorizing parent Goal");}else if(thread.agent!==actor)throw new Error("Goal change source Turn actor does not match");}if(sourceWakeId){const wake=this.wake(sourceWakeId);if(!wake)throw new Error("Goal change source Wake does not exist");if(wake.status!=="consumed"||sourceTurnId&&wake.turnId!==sourceTurnId)throw new Error("Goal change source Wake does not match its Turn");if(authority.kind==="parent_goal"&&(wake.targetKind!=="goal"||wake.agent!==actor||wake.goalId!==authority.goalId))throw new Error("Goal change source Wake is not bound to the authorizing parent Goal");}}
  ...
  #admitThread(thread:ThreadSnapshot,actor:string):EventRecord{if(!thread.id.trim()||!thread.agent.trim())throw new Error("thread id and agent are required");if((thread.agent==="ceo")!==(thread.role==="ceo"))throw new Error("exactly the ceo Agent must use the ceo Thread role");const current=this.thread(thread.id);const agentThread=this.threads().find((candidate)=>candidate.agent===thread.agent);if(agentThread&&agentThread.id!==thread.id)throw new Error("agent already owns a Thread");if(current&&(current.agent!==thread.agent||current.role!==thread.role||current.parentThreadId!==thread.parentThreadId||current.createdAt!==thread.createdAt))throw new Error("thread identity cannot change");return this.#recordProjection("threads",thread,actor,"thread.put",undefined,thread.updatedAt,undefined,`thread:${thread.id}`);}
  #finishTurn(current:TurnSnapshot,status:"completed"|"failed"|"interrupted",error:JsonValue|null,now:string,actor:string,mailIds:string[]):TurnSnapshot{if(current.status!=="in_progress")throw new Error("only an active Turn may finish");if(status==="completed"&&current.goalId!==null)throw new Error("committed Turn completion requires commitHandoff");if(status==="completed"&&error!==null||status!=="completed"&&error===null)throw new Error("Turn terminal error does not match status");if(status!=="completed")this.#repairOpenTurnItems(current.id,String((error as {message?:unknown}).message??"Turn interrupted"),now,actor);else if(this.turnItems(current.id).some((item)=>item.status==="in_progress"))throw new Error("successful Turn cannot retain in-progress Items");const streamId=`turn:${current.id}`;const thread=this.thread(current.threadId);const delivered=new Set(mailIds);if(status!=="completed"&&mailIds.length)throw new Error("failed Turn cannot acknowledge Mail");if(status==="completed"&&thread)this.#recordMailReads(this.unreadMail(thread.agent).filter((candidate)=>delivered.has(candidate.id)),now,streamId);this.#insertEvent({streamId,ts:now,actor,type:status==="completed"?"transcript.completed":"transcript.interrupted",data:status==="completed"?{}:{reason:String((error as {message?:unknown}).message??"Turn interrupted")}});const next={...current,status,error,endedAt:now,leaseUntil:null,leaseToken:null};this.#recordProjection("turns",next,actor,`turn.${status}`,undefined,now,undefined,streamId);return next;}
  #admitWake(input:WakeSnapshot,actor:string,allowExisting:boolean):{event:EventRecord;created:boolean;wake:WakeSnapshot}{
    if(actor!=="supervisor")throw new Error("only supervisor may enqueue wakes");
    this.#assertAutomaticTarget(input);
    if(!input.id.trim()||!input.agent.trim()||!input.triggerRef.trim())throw new Error("Wake id, agent, and trigger are required");
    if(input.status!=="queued"||input.attempt!==0||input.claimedAt||input.consumedAt||input.turnId)throw new Error("new wake must be pristine and queued");
    ...
    if(sameId){
      ...
      if(!allowExisting||!sameIdentity)throw new Error("wake id was reused with a different identity");
      const row=this.db.prepare("SELECT * FROM events WHERE type='wake.enqueued' AND json_extract(data,'$.snapshot.id')=? ORDER BY seq LIMIT 1").get(sameId.id) as Row|undefined;
    ...
    if(duplicate){
      ...
      if(!allowExisting||!sameTarget)throw new Error("Wake trigger was reused with a different automatic target");
      const row=this.db.prepare("SELECT * FROM events WHERE type='wake.enqueued' AND json_extract(data,'$.snapshot.id')=? ORDER BY seq LIMIT 1").get(duplicate.id) as Row|undefined;
      if(!row)throw new Error("wake projection has no source event");
      return{event:mapEvent(row),created:false,wake:duplicate};
    }
    if(input.targetKind==="goal"){const goal=this.#getGoal(input.goalId);if(!goal||goal.owner!==input.agent||goal.phase!=="active")throw new Error("Wake Goal target is inactive or owned by another Agent");}
    const enqueuedSeq=this.#nextEventSeq();const wake={...input,enqueuedSeq};const event=this.#recordProjection("wakes",wake,actor,"wake.enqueued",wake.id,undefined,enqueuedSeq);this.#admitWakeTrigger(wake.id,wake.triggerRef,actor,event.ts);return{event,created:true,wake};
  }
  #admitWakeTrigger(wakeId:string,triggerRef:string,actor:string,at:string):WakeTriggerSnapshot{if(actor!=="supervisor")throw new Error("only supervisor may add Wake triggers");if(!triggerRef.trim())throw new Error("Wake trigger is required");const wake=this.#requiredWake(wakeId);if(wake.status!=="queued"&&wake.status!=="claimed")throw new Error("triggers may only be added before a Wake starts its Turn");const row=this.db.prepare("SELECT * FROM wake_triggers WHERE agent=? AND trigger_ref=?").get(wake.agent,triggerRef) as Row|undefined;if(row){const current=mapWakeTrigger(row);if(current.wakeId!==wake.id)throw new Error("Wake trigger is already owned by another Wake");return current;}const mailId=mailIdFromTrigger(triggerRef);const mail=mailId?this.mailbox().find((candidate)=>candidate.id===mailId):null;if(mail){const matches=mail.routeKind==="goal"?wake.targetKind==="goal"&&wake.goalId===mail.goalId:mail.routeKind==="specialist_inbox"?wake.targetKind==="specialist"&&wake.specialistRole===mail.specialistRole:false;if(!matches)throw new Error("Mail trigger does not match the Wake automatic target");}const source:WakeTriggerSnapshot["source"]=wake.targetKind==="goal"?"goal":"system";const trigger:WakeTriggerSnapshot={wakeId:wake.id,agent:wake.agent,triggerRef,source,status:"pending",addedAt:at,resolvedAt:null};this.#recordProjection("wake_triggers",trigger,"supervisor","wake_trigger.added",wake.id,at);return trigger;}
  ...
  #admitTurnItem(item:TurnItemSnapshot,actor:string,streamId=`turn:${item.turnId}`):EventRecord{if(item.type==="assistant_message"){const data=item.data&&typeof item.data==="object"&&!Array.isArray(item.data)?item.data as Record<string,JsonValue>:null;const text=normalizeAssistantText(String(data?.text??""));if(!data||!text)throw new Error("assistant message requires canonical readable text");item={...item,data:{...data,text}};}if(!item.id.trim()||!item.turnId.trim())throw new Error("turn item id and turn are required");const turn=this.turn(item.turnId);if(!turn)throw new Error("turn item turn does not exist");if(turn.status!=="in_progress")throw new Error("terminal turn cannot accept items");if(item.status==="in_progress"&&item.completedAt!==null||item.status!=="in_progress"&&item.completedAt===null)throw new Error("turn item terminal time does not match status");const current=this.db.prepare("SELECT * FROM turn_items WHERE id=?").get(item.id) as Row|undefined;if(current&&(String(current.turn_id)!==item.turnId||String(current.type)!==item.type||String(current.status)!=="in_progress"))throw new Error("turn item identity or terminal state cannot change");return this.#recordProjection("turn_items",item,actor,current?`item.${item.type}.${item.status}`:`item.${item.type}.started`,undefined,item.completedAt??item.createdAt,undefined,streamId);}
  #assertMailRequest(mail:MailSnapshot,actor:string):void{if(!mail.id.trim()||!mail.to.trim()||!mail.from.trim())throw new Error("mail id, sender, and recipient are required");if(mail.from!==actor&&actor!=="supervisor")throw new Error("mail sender does not match actor");if(mail.readAt!==null)throw new Error("new mail must be unread");if(mail.routeKind==="goal"){if(mail.specialistRole!==null)throw new Error("Goal Mail cannot carry a specialist role");const goal=this.#getGoal(mail.goalId);if(!goal||goal.owner!==mail.to)throw new Error("Mail Goal route is not owned by its recipient");}else if(mail.routeKind==="ceo_inbox"){if(mail.to!=="ceo"||mail.goalId!==null||mail.specialistRole!==null||!["verifier","audit","supervisor"].includes(mail.from))throw new Error("CEO inbox Mail has an invalid sender or recipient");}else if(mail.goalId!==null||mail.specialistRole===null)throw new Error("Specialist inbox Mail has an invalid route");}
  ...
  #normalizeSchedule(value:ScheduleSnapshot,actor:string,binding?:{goalId:string|null}):ScheduleSnapshot{this.#assertAutomaticTarget(value);if(actor!=="supervisor"&&actor!==value.agent)throw new Error("schedule may only be set by its agent or supervisor");if(value.setBy!==actor)throw new Error("schedule setBy does not match actor");const parsed=Date.parse(value.nextWakeAt);if(!value.id.trim()||!value.agent.trim()||!value.reason.trim()||!Number.isFinite(parsed))throw new Error("schedule requires an id, agent, valid time, and reason");const normalized={...value,nextWakeAt:new Date(parsed).toISOString()} as ScheduleSnapshot;if(normalized.status!=="pending"||normalized.resolvedAt!==null)throw new Error("new schedule must be pending");if(binding&&normalized.goalId!==binding.goalId)throw new Error("handoff schedule does not match the Turn Goal commitment");if(normalized.targetKind==="goal"){const goal=this.#getGoal(normalized.goalId);if(!goal||goal.owner!==normalized.agent||goal.phase!=="active")throw new Error("schedule Goal target is inactive or owned by another Agent");}return normalized;}
  #assertAutomaticTarget(value:{targetKind:string;agent:string;goalId:string|null;specialistRole:string|null}):void{if(value.targetKind==="goal"){if(!value.goalId||value.specialistRole!==null)throw new Error("invalid Goal automatic target");return;}if(value.targetKind!=="specialist"||value.goalId!==null||!value.specialistRole||!["verifier","audit"].includes(value.specialistRole))throw new Error("invalid Specialist automatic target");}
  #assertTurnShape(value:{triggerKind:string;goalId:string|null;goalRevision:number|null}):void{if(!["user_message","wake"].includes(value.triggerKind)||(value.goalId===null)!==(value.goalRevision===null)||value.goalRevision!==null&&(!Number.isInteger(value.goalRevision)||value.goalRevision<0))throw new Error("invalid Turn trigger or Goal commitment");}
  #replayProjection(projection:ProjectionName,snapshot:unknown,event:EventRecord):void{if(projection==="turns")this.#assertReplayTurn(snapshot as TurnSnapshot,event);else if(projection==="wakes")this.#assertReplayWake(snapshot as WakeSnapshot,event);this.#applyProjection(projection,snapshot,event.seq);}
  #assertReplayTurn(turn:TurnSnapshot,event:EventRecord):void{
    this.#assertTurnShape(turn);const thread=this.thread(turn.threadId);if(!thread)throw new Error(`turn event ${event.seq} has no Thread`);const current=this.turn(turn.id);
    if(!current){
      if(event.type!=="turn.started"||turn.status!=="in_progress"||turn.attempt!==1||turn.runnerPid!==null)throw new Error(`turn event ${event.seq} has an invalid initial state`);
      if(event.actor==="human"){if(turn.triggerKind!=="user_message"||thread.role!=="ceo")throw new Error(`turn event ${event.seq} has invalid user ownership`);if(turn.goalId){const goal=this.#getGoal(turn.goalId);if(!goal||goal.parentId!==null||goal.owner!==thread.agent||goal.phase!=="active"||goal.revision!==turn.goalRevision)throw new Error(`turn event ${event.seq} has an invalid direct Goal commitment`);}}
      else{if(event.actor!=="supervisor"||!this.wakes().some((wake)=>wake.status==="claimed"&&wake.agent===thread.agent&&sameWakeTurnTarget(wake,turn,thread)))throw new Error(`turn event ${event.seq} bypasses Wake admission`);this.#assertCurrentAutomaticTurn(turn,thread,event.seq);}return;
    }
    if(current.threadId!==turn.threadId||current.triggerKind!==turn.triggerKind||current.startedAt!==turn.startedAt||current.runnerProfileId!==turn.runnerProfileId)throw new Error(`turn event ${event.seq} changes Turn identity`);
    const sameProcessState=sameTurnExceptRunnerPid(current,turn);
    if(event.type==="turn.process_attached"){if(event.actor!=="supervisor"||current.status!=="in_progress"||current.runnerPid!==null||!Number.isInteger(turn.runnerPid)||turn.runnerPid!<=0||!sameProcessState)throw new Error(`turn event ${event.seq} has an invalid Runner process attachment`);return;}
    if(event.type==="turn.process_released"){if(event.actor!=="supervisor"||current.runnerPid===null||turn.runnerPid!==null||!sameProcessState)throw new Error(`turn event ${event.seq} has an invalid Runner process release`);return;}
    if(current.status!=="in_progress")throw new Error(`turn event ${event.seq} changes a terminal Turn`);
    if(current.runnerPid!==turn.runnerPid)throw new Error(`turn event ${event.seq} changes Runner process ownership outside attach/release`);
    const commitsGoal=event.type==="turn.goal_committed"&&event.actor==="supervisor"&&current.goalId===null&&turn.goalId!==null;if(!commitsGoal&&(current.goalId!==turn.goalId||current.goalRevision!==turn.goalRevision))throw new Error(`turn event ${event.seq} changes Goal commitment`);
    if(commitsGoal){const goal=this.#getGoal(turn.goalId!);if(!goal||goal.owner!==thread.agent||goal.phase!=="active"||goal.revision!==turn.goalRevision||thread.role==="ceo"&&goal.parentId!==null||thread.role==="child"&&goal.parentId===null||thread.role==="verifier"||thread.role==="audit")throw new Error(`turn event ${event.seq} has an invalid Goal commitment`);}
    if(turn.status==="in_progress"){if(!commitsGoal&&event.type!=="turn.in_progress"||turn.attempt!==current.attempt&&turn.attempt!==current.attempt+1)throw new Error(`turn event ${event.seq} has an invalid retry transition`);}
    else{if(event.type!==`turn.${turn.status}`||turn.attempt!==current.attempt||turn.runnerPid!==current.runnerPid)throw new Error(`turn event ${event.seq} has an invalid terminal transition`);if(turn.status==="completed"){if(turn.goalId!==null)this.#assertReplayGoalCompletion(turn,event);else this.#assertReplayResponseCompletion(turn,event);}}
  }
  #assertReplayResponseCompletion(turn:TurnSnapshot,event:EventRecord):TurnItemSnapshot{const responseRow=this.db.prepare("SELECT data FROM events WHERE stream_id=? AND type='response.committed' AND seq<? ORDER BY seq DESC LIMIT 1").get(`turn:${turn.id}`,event.seq) as {data:string}|undefined;if(!responseRow)throw new Error(`turn event ${event.seq} completes without a committed response`);const response=JSON.parse(responseRow.data) as {messageItemId?:unknown;text?:unknown};const responseItem=this.turnItems(turn.id).find((item)=>item.id===response.messageItemId&&item.type==="assistant_message"&&item.status==="completed");if(!responseItem||normalizeAssistantText(String((responseItem.data as {text?:unknown}).text??""))!==response.text)throw new Error(`turn event ${event.seq} has a mismatched committed response`);return responseItem;}
  #assertReplayGoalCompletion(turn:TurnSnapshot,event:EventRecord):void{if(turn.goalId===null||turn.goalRevision===null)throw new Error(`turn event ${event.seq} has no Goal commitment`);const items=this.turnItems(turn.id);this.#assertReplayResponseCompletion(turn,event);const handoffItem=items.findLast((item)=>item.type==="handoff"&&item.status==="completed");const handoffRow=this.db.prepare("SELECT data FROM events WHERE stream_id=? AND type='handoff.recorded' AND seq<? ORDER BY seq DESC LIMIT 1").get(`turn:${turn.id}`,event.seq) as {data:string}|undefined;if(!handoffRow||!handoffItem)throw new Error(`turn event ${event.seq} completes a Goal without Handoff`);const handoff=JSON.parse(handoffRow.data) as unknown as Handoff;if(!isDeepStrictEqual(handoffItem.data,handoff))throw new Error(`turn event ${event.seq} has mismatched Handoff facts`);assertHandoff(handoff);const record=this.workRecord(turn.goalId);if(handoff.goalId!==turn.goalId||handoff.goalRevision!==turn.goalRevision||record?.recordRevision!==handoff.recordRevision)throw new Error(`turn event ${event.seq} has a Handoff for another Goal or Work Record`);for(const seq of handoff.evidence)if(seq>=event.seq||!this.db.prepare("SELECT 1 FROM events WHERE seq=?").get(seq))throw new Error(`turn event ${event.seq} has invalid Handoff evidence`);}
  #assertChildCompletionRecord(goal:GoalSnapshot,evidence:number[]):void{const record=this.workRecord(goal.id);if(!record||record.recordRevision<1||record.goalRevision!==goal.revision||record.updatedBy!==goal.owner)throw new Error("child Goal completion requires a current owner-written Work Record");const turn=this.turn(record.updatedInTurn);const owner=turn?this.thread(turn.threadId)?.agent:null;if(!turn||turn.status!=="completed"||turn.goalId!==goal.id||turn.goalRevision!==goal.revision||owner!==goal.owner)throw new Error("child Goal completion requires a completed owner Turn for the current Work Record");const supportsRecord=evidence.some((seq)=>{if(seq===record.lastEventSeq)return true;const row=this.db.prepare("SELECT type,data FROM events WHERE seq=?").get(seq) as {type:string;data:string}|undefined;if(row?.type!=="handoff.recorded")return false;const handoff=JSON.parse(row.data) as Partial<Handoff>;return handoff.goalId===goal.id&&handoff.goalRevision===goal.revision&&handoff.recordRevision===record.recordRevision;});if(!supportsRecord)throw new Error("child Goal completion evidence must cite its current Work Record or matching Handoff");}
  #assertReplayWake(wake:WakeSnapshot,event:EventRecord):void{this.#assertAutomaticTarget(wake);const current=this.wake(wake.id);if(!current){if(event.type!=="wake.enqueued"||wake.status!=="queued"||wake.attempt!==0||wake.claimedAt||wake.consumedAt||wake.turnId)throw new Error(`wake event ${event.seq} has an invalid initial state`);if(wake.targetKind==="goal"){const goal=this.#getGoal(wake.goalId);if(!goal||goal.phase!=="active"||goal.owner!==wake.agent)throw new Error(`wake event ${event.seq} has an invalid Goal target`);}return;}if(current.targetKind!==wake.targetKind||current.agent!==wake.agent||current.goalId!==wake.goalId||current.specialistRole!==wake.specialistRole||current.triggerRef!==wake.triggerRef||current.enqueuedSeq!==wake.enqueuedSeq)throw new Error(`wake event ${event.seq} changes Wake identity`);if(current.status==="consumed"||current.status==="cancelled")throw new Error(`wake event ${event.seq} changes a terminal Wake`);if(wake.status==="claimed"){if(event.type!=="wake.claimed"||current.status!=="queued"||wake.attempt!==current.attempt+1||!wake.claimedAt||wake.consumedAt!==null||wake.turnId!==null)throw new Error(`wake event ${event.seq} has an invalid claim transition`);}else if(wake.status==="queued"){if(event.type!=="wake.released"||current.status!=="claimed"||wake.attempt!==current.attempt||wake.claimedAt!==null||wake.consumedAt!==null||wake.turnId!==null)throw new Error(`wake event ${event.seq} has an invalid release transition`);}else if(wake.status==="cancelled"){if(event.type!=="wake.cancelled"||wake.attempt!==current.attempt||wake.claimedAt!==current.claimedAt||wake.consumedAt===null||wake.turnId!==null)throw new Error(`wake event ${event.seq} has an invalid cancel transition`);}else{if(event.type!=="wake.consumed"||current.status!=="claimed"||wake.attempt!==current.attempt||wake.claimedAt!==current.claimedAt||wake.consumedAt===null||!wake.turnId)throw new Error(`wake event ${event.seq} has an invalid consume transition`);const turn=this.turn(wake.turnId);const thread=turn?this.thread(turn.threadId):null;if(!turn||!thread||thread.agent!==wake.agent||!sameWakeTurnTarget(wake,turn,thread))throw new Error(`wake event ${event.seq} does not match its Turn`);}}
  #assertCurrentAutomaticTurn(turn:TurnSnapshot,thread:ThreadSnapshot,seq?:number):void{if(turn.triggerKind!=="wake")throw new Error(seq?`turn event ${seq} is not Wake-triggered`:"automatic Turn must be Wake-triggered");if(turn.goalId!==null){const goal=this.#getGoal(turn.goalId);if(thread.role!=="ceo"&&thread.role!=="child"||!goal||goal.phase!=="active"||goal.owner!==thread.agent||goal.revision!==turn.goalRevision||thread.role==="ceo"&&goal.parentId!==null||thread.role==="child"&&goal.parentId===null)throw new Error(seq?`turn event ${seq} has an invalid current Goal commitment for its Thread role`:"Wake Turn must commit to the current active Goal revision allowed by its Thread role");return;}if(thread.role!=="verifier"&&thread.role!=="audit")throw new Error(seq?`turn event ${seq} has an uncommitted Goal-owning Agent`:"only Specialists may start an uncommitted Wake Turn");}
...
function mapSchedule(r: Row): ScheduleSnapshot { return {id:String(r.id),targetKind:String(r.target_kind),agent:String(r.agent),specialistRole:r.specialist_role===null?null:String(r.specialist_role),goalId:r.goal_id===null?null:String(r.goal_id),nextWakeAt:String(r.next_wake_at),reason:String(r.reason),setBy:String(r.set_by),status:String(r.status),resolvedAt:r.resolved_at===null?null:String(r.resolved_at)} as ScheduleSnapshot; }
function mapWake(r: Row): WakeSnapshot { return {id:String(r.id),targetKind:String(r.target_kind),agent:String(r.agent),specialistRole:r.specialist_role===null?null:String(r.specialist_role),goalId:r.goal_id===null?null:String(r.goal_id),triggerRef:String(r.trigger_ref),status:String(r.status),attempt:Number(r.attempt),enqueuedSeq:Number(r.enqueued_seq),claimedAt:r.claimed_at===null?null:String(r.claimed_at),consumedAt:r.consumed_at===null?null:String(r.consumed_at),turnId:r.turn_id===null?null:String(r.turn_id)} as WakeSnapshot; }
function sameTurnExceptRunnerPid(left:TurnSnapshot,right:TurnSnapshot):boolean{const{runnerPid:_left,...leftRest}=left;const{runnerPid:_right,...rightRest}=right;return isDeepStrictEqual(leftRest,rightRest);}
function sameWakeTurnTarget(wake:WakeSnapshot,turn:TurnSnapshot,thread:ThreadSnapshot):boolean{if(turn.triggerKind!=="wake")return false;if(wake.targetKind==="goal")return wake.goalId===turn.goalId;return turn.goalId===null&&thread.role===wake.specialistRole;}
...
function mapMail(r: Row): MailSnapshot { return {id:String(r.id),routeKind:String(r.route_kind),to:String(r.to_agent),from:String(r.from_agent),specialistRole:r.specialist_role===null?null:String(r.specialist_role),level:String(r.level),goalId:r.goal_id===null?null:String(r.goal_id),body:JSON.parse(String(r.body)),readAt:r.read_at===null?null:String(r.read_at)} as MailSnapshot; }
