...
export class SqliteLedger implements Ledger {
  ...
  commitReassignment(request: ReassignmentRequest, actor: string, wakeId?: string): ReassignmentResult {
    ...
    return this.#transaction(() => {
      ...
      const wakeBase: WakeSnapshot = { id: `reassignment-wake:${request.id}`, agent: goal.owner, triggerRef: `reassignment:${request.id}`, status: "queued", attempt: 0, enqueuedSeq: 0, claimedAt: null, consumedAt: null, turnId: null,goalId:goal.id };
      ...
      this.#supersedeSchedulesForOwnerGoal(current.owner,current.id,this.#now());
  ...
  commitHandoff(commit: HandoffCommit): EventRecord {
    return this.#transaction(() => {
      ...
      if (normalizedSchedule) {
        this.#admitSchedule(normalizedSchedule,commit.agent,undefined,commit.ts,streamId,{goalId:turn.goalId},false);
      }
  ...
  #supersedeSchedulesForOwnerGoal(agent:string,goalId:string,now:string):ScheduleSnapshot[]{const rows=this.db.prepare("SELECT * FROM schedule WHERE agent=? AND goal_id=? AND status='pending' ORDER BY id").all(agent,goalId) as Row[];return rows.map((row)=>{const current=mapSchedule(row);const next={...current,status:"superseded" as const,resolvedAt:now};this.#recordProjection("schedule",next,"supervisor","schedule.superseded",undefined,now);return next;});}
