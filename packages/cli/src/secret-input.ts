...
export class SecretInput extends Input {
  override render(width: number): string[] {
    ...
    const mask =
      length > available ? `${"*".repeat(Math.max(0, available - 1))}…` : "*".repeat(length);
    const marker = this.focused ? CURSOR_MARKER : "";
    return [
      `> ${mask}${marker}\x1b[7m \x1b[27m${" ".repeat(Math.max(0, available - mask.length - 1))}`,
    ];
  }
}

/** Input.setValue preserves cursor position; setup defaults should instead edit from the end. */
export function setInitialValue(input: Input, value: string): void {
