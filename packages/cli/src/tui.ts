...
class HeaderBar implements Component {
  ...
  render(width: number): string[] {
    return [renderTuiHeader(width, this.runner, this.target, this.version)];
  }
  invalidate(): void {}
}
...
class OrganizationStatusLine implements Component {
  ...
  set(value: OrganizationStatus): void {
    this.value = value;
  }
  render(width: number): string[] {
    return [renderOrganizationStatus(this.value, width)];
  }
  invalidate(): void {}
}
...
export function renderMessagePlaceholder(width: number, focused: boolean): string {
  return truncateToWidth(
    ` ${tuiTheme.accent("›")} ${focused ? CURSOR_MARKER : ""}${tuiTheme.muted("Message Goah…")}`,
    width,
    "",
    true,
  );
...
const markdownTheme: MarkdownTheme = {
  heading: tuiTheme.accent,
  link: tuiTheme.accent,
  linkUrl: tuiTheme.muted,
  code: tuiTheme.warning,
  codeBlock: tuiTheme.subtle,
  codeBlockBorder: tuiTheme.muted,
  quote: tuiTheme.subtle,
  quoteBorder: tuiTheme.accent,
  hr: tuiTheme.muted,
  listBullet: tuiTheme.accent,
  bold: tuiTheme.strong,
  italic: tuiTheme.muted,
  strikethrough: tuiTheme.muted,
  underline: tuiTheme.underline,
  codeBlockIndent: "  ",
};
interface StoredToolActivity extends ToolActivity {
  startedAt?: number;
  elapsedMs?: number;
}
...
export class ConversationView implements Component {
  ...
  constructor(initial: string[]) {
    this.entries = initial.length ? [{ kind: "notice", content: initial.join("\n") }] : [];
  }
  ...
  addText(content: string): void {
    ...
    if (turn && /^(error|failed|goal |stopping|new turn)/.test(plain)) {
      ...
      if (previous?.kind !== "notice" || previous.content !== content)
        this.entries.push({ kind: "notice", content });
    }
    this.trim();
  }
  ...
  addMarkdown(content: string, messageId?: string): void {
    ...
    if (
      ...
      this.entries.some(
        (entry) =>
          entry.kind === "turn" &&
          entry.responses.some((response) => response.messageId === messageId),
      )
    )
      return;
  ...
  appendLiveMarkdown(content: string): void {
    this.currentTurn().liveMarkdown += content;
  }
  ...
  clearLiveMarkdown(): void {
    const turn = this.lastTurn();
    if (turn) turn.liveMarkdown = "";
  }
  updateThinking(activity: ThinkingActivity): void {
    ...
    if (activity.phase === "clear") {
      ...
      delete turn.thinkingStartedAt;
      delete turn.thinkingElapsedMs;
      return;
    }
    turn.thinking = (activity.text || turn.thinking).trim();
    turn.thinkingActive = false;
    if (turn.thinkingStartedAt !== undefined)
      turn.thinkingElapsedMs = Math.max(0, Date.now() - turn.thinkingStartedAt);
  }
  updateTool(activity: ToolActivity): void {
    ...
    if (previous) {
      ...
      if (activity.status !== "running" && previous.startedAt !== undefined)
        previous.elapsedMs = Math.max(0, Date.now() - previous.startedAt);
    } else
      turn.tools.push({
        ...activity,
        ...(activity.status === "running" ? { startedAt: Date.now() } : {}),
      });
    this.trim();
  }
        ...
        for (const tool of entry.tools)
          if (tool.status === "running") {
            ...
            if (tool.startedAt !== undefined)
              tool.elapsedMs = Math.max(0, Date.now() - tool.startedAt);
          }
  }
  toggleDetails(): void {
  ...
  endTransientTurn(): void {
    ...
    this.stopRunningTools();
  }
  private currentTurn(): TurnPresentation {
    const turn = this.lastTurn();
    if (turn) return turn;
    const created = newTurn();
    this.entries.push(created);
    return created;
  }
  private lastTurn(): TurnPresentation | null {
    const entry = this.entries.at(-1);
    return entry?.kind === "turn" ? entry : null;
  }
  private trim(): void {
    if (this.entries.length > 120)
      this.entries.splice(this.entries[0]?.kind === "component" ? 1 : 0, this.entries.length - 120);
  }
  render(width: number): string[] {
    return this.entries.flatMap((entry) =>
      entry.kind === "component"
        ...
        : entry.kind === "notice"
          ...
          : renderTurn(entry, width, this.details),
    );
  }
  invalidate(): void {}
}
...
function renderTurn(turn: TurnPresentation, width: number, details: boolean): string[] {
  ...
  for (const response of turn.responses)
    lines.push(...new Markdown(response.content, 4, 0, markdownTheme).render(width), "");
...
function renderLedgerActivity(turn: TurnPresentation, width: number): string[] {
  ...
  if (thinking) {
    const elapsed = formatDuration(
      turn.thinkingElapsedMs ??
        (turn.thinkingStartedAt === undefined ? undefined : Date.now() - turn.thinkingStartedAt),
    );
    ...
    lines.push(
      `${tuiTheme.muted(label)}${summary ? `  ${tuiTheme.muted(summary)}` : ""}${fill}${action}`,
    );
  }
  for (let index = 0; index < turn.tools.length; index += 1)
    lines.push(
      renderLedgerTool(turn.tools[index]!, index === turn.tools.length - 1 ? "└" : "├", width),
    );
  return lines;
}
...
export class WelcomeLockup implements Component {
  ...
  constructor(snapshot: WelcomeSnapshot, hasHistory: boolean, imageData: string | null = null) {
    this.mark = imageData
      ? new Image(
          imageData,
          "image/png",
          { fallbackColor: tuiTheme.accent },
          { maxWidthCells: 10, maxHeightCells: 5, filename: "Goah" },
        )
    ...
    if (snapshot.team.length)
      lines.push(
        tuiTheme.muted(
          `${snapshot.team.length} Goal Agent${snapshot.team.length === 1 ? "" : "s"} in the organization`,
        ),
      );
  ...
  render(width: number): string[] {
    ...
    if (width < 58) {
      return [
        "",
        ...this.mark.render(Math.min(14, contentWidth)).map((line) => `  ${line}`),
        "",
        ...this.details.render(contentWidth).map((line) => `  ${line}`),
        "",
      ];
    }
...
function terminalLogoData(): string | null {
  ...
  try {
    return readFileSync(new URL("./console/goah-orbital-mark.png", import.meta.url)).toString(
      "base64",
    );
  } catch {
    return null;
...
export class StreamCoordinator {
  ...
  get active(): AbortController | null {
    return this.#active;
  }
  get hasPending(): boolean {
    return this.#pending !== null;
  }
  begin(controller: AbortController): { owns: boolean; previous: AbortController | null } {
    if (this.#pending) throw new Error("Another Turn is waiting to be accepted.");
    const previous = this.#active;
    if (previous) this.#pending = controller;
    else this.#active = controller;
    return { owns: previous === null, previous };
  }
  accept(controller: AbortController): AbortController | null {
    if (this.#pending !== controller) throw new Error("Stream candidate is no longer pending.");
    const previous = this.#active;
    this.#pending = null;
  ...
  reject(controller: AbortController): void {
...
export function organizationStatusFromValue(
  ...
  const team = Array.isArray(record.team)
    ...
    : [];
  const childAgents = team.filter(
    (member) => member.agent !== "ceo" && member.motion !== "retired",
  ).length;
  const nextWakeAt =
    team
      .map((member) => member.nextWakeAt)
      .filter(
        (item): item is string => typeof item === "string" && Number.isFinite(Date.parse(item)),
      )
      .sort()[0] ?? null;
  return {
    goal: typeof root?.objective === "string" ? root.objective : fallbackGoal,
    model: fallbackModel,
    childAgents,
    nextWakeAt,
  };
}
export function renderOrganizationStatus(
  value: OrganizationStatus,
  width: number,
  now = new Date(),
): string {
  const separator = tuiTheme.subtle(" │ ");
  const modelValue = truncateToWidth(
    value.model ?? "—",
    width < 64 ? 10 : width < 96 ? 18 : 28,
    "…",
  );
  if (width < 96) {
    ...
    const goal = value.goal
      ? truncateToWidth(value.goal.replace(/\s+/g, " ").trim(), available, "…")
      : "";
    return truncateToWidth(
      `${tuiTheme.accent(label)}${goal ? ` ${tuiTheme.muted(goal)}` : ""}${tail}`,
      width,
      "",
      true,
    );
...
export function renderTuiHeader(
  ...
  const context =
    rawContext.length > contextWidth
      ? `${rawContext.slice(0, Math.max(1, contextWidth - 1))}…`
      : rawContext;
...
export type TuiInputAction =
  ...
  | "steer"
  | "send";
export interface TuiCommandDefinition extends SlashCommand {
  action: Exclude<TuiInputAction, "unknown" | "empty" | "steer" | "send">;
...
export const TUI_COMMANDS: TuiCommandDefinition[] = [
  {
    name: "goal",
    action: "goal",
    description: "Start or revise durable work",
  ...
  { name: "status", action: "status", description: "Inspect the current workspace" },
  {
    name: "setup",
    action: "setup",
    description: "Configure Goah",
    argumentHint: "[runner|model|auth]",
    acceptsArguments: true,
  },
  {
    name: "records",
    action: "records",
    description: "Browse current work records",
    argumentHint: "[goal]",
    acceptsArguments: true,
  },
  {
    name: "history",
    action: "records",
    description: "Show a Goal's record history",
    argumentHint: "<goal>",
    acceptsArguments: true,
    requiresArgument: true,
  },
  {
    name: "observe",
    action: "goal",
    description: "Set how the active Goal is observed",
    argumentHint: "<method>",
    acceptsArguments: true,
    requiresArgument: true,
  },
...
function commandDefinition(
  text: string,
): { definition: TuiCommandDefinition; hasArguments: boolean } | null {
  const token = text.trim().split(/\s+/, 1)[0] ?? "";
  if (!token.startsWith("/")) return null;
  const name = token.slice(1);
  const definition = TUI_COMMANDS.find(
...
export function commandAwaitingArgument(text: string): string | null {
  const match = commandDefinition(text);
  return match && match.definition.requiresArgument && !match.hasArguments
...
export function createTuiAutocompleteProvider(
  ...
  const commands: SlashCommand[] = TUI_COMMANDS.map(
...
export function renderTuiCommandHelp(): string {
  return [
    tuiTheme.strong("Commands"),
    ...TUI_COMMANDS.map((command) => {
    ...
    "",
    tuiTheme.strong("Keyboard"),
    `  ${"Ctrl+C".padEnd(28)} ${tuiTheme.muted("Clear input · interrupt current Turn · exit when idle")}`,
    `  ${"Ctrl+D".padEnd(28)} ${tuiTheme.muted("Exit when idle")}`,
    "",
  ].join("\n");
}
export function classifyTuiInput(
  value: string,
  busy: boolean,
): { action: TuiInputAction; text: string } {
...
export function classifyTuiControlKey(
  data: string,
  inputText: string,
  busy: boolean,
): TuiControlAction {
...
export function bindTuiTerminationSignals(
  shutdown: () => void,
  source: TuiSignalSource = process,
): () => void {
...
export async function runGoahTui(
  configPath: string,
  stateDir: string,
  ...
  const liveTurns =
    ...
    !Array.isArray(liveSnapshot) &&
    Array.isArray(liveSnapshot.turns)
      ? (liveSnapshot.turns as Array<Record<string, unknown>>)
      : [];
  ...
  const conversationScroll = new ScrollView(transcriptView, {
    ...
    primary: true,
    scrollbar: "auto",
    scrollbarStyle: tuiTheme.muted,
  });
  const organizationView = new OrganizationStatusLine(
    organizationStatusFromValue(
      initialOrganization,
      snapshot.root?.objective ?? null,
      runner.target,
    ),
  );
  ...
  const appendLive = (text: string): void => {
    transcriptView.appendLiveMarkdown(text);
    tui.requestRender();
  };
  const commitLive = (text: string, messageId?: string): void => {
    transcriptView.clearLiveMarkdown();
    if (text) transcriptView.addMarkdown(text, messageId);
    tui.requestRender();
  };
  const pushResponse = (text: string, messageId?: string): void => {
    transcriptView.addMarkdown(text, messageId);
    tui.requestRender();
  };
  const updateTool = (activity: ToolActivity): void => {
    transcriptView.updateTool(activity);
    tui.requestRender();
  };
  const updateThinking = (activity: ThinkingActivity): void => {
    transcriptView.updateThinking(activity);
    tui.requestRender();
  };
  const replaceLive = (text: string, thinking: string, thinkingActive: boolean): void => {
  ...
  const endVisibleTurn = (): void => {
    transcriptView.endTransientTurn();
    tui.requestRender();
  };
  const takeStream = (controller: AbortController): void => {
    streams.accept(controller);
    endVisibleTurn();
  };
  const supersedeStreams = (): void => {
    streams.supersede();
    endVisibleTurn();
  };
  ...
  const send = async (
    message: string,
    showUser = true,
    request: ControlRequest = { op: "interact", message },
  ): Promise<void> => {
    const controller = new AbortController();
    let started: { owns: boolean; previous: AbortController | null };
    try {
      started = streams.begin(controller);
    } catch (error) {
      push(errorLine(error));
      return;
    }
    let ownsStream = started.owns;
    ...
    try {
      ...
      if (!ownsStream) {
        ...
        if (!rejectionShown && !controller.signal.aborted)
          push(errorLine("Turn request ended before it was accepted."));
  ...
  const attachTurn = async (turnId: string, newEntry = false): Promise<void> => {
    ...
    const controller = new AbortController();
    try {
      if (!streams.begin(controller).owns)
    ...
    try {
      ...
      if (streams.isCurrent(controller) && !controller.signal.aborted) push(errorLine(error));
    } finally {
      if (!streams.complete(controller)) return;
      busy.active = false;
      interrupting = false;
  ...
  const steer = async (message: string): Promise<void> => {
    try {
      ...
      if (queuedBySupervisor)
        push(
          `${tuiTheme.warning("new Turn")}  ${tuiTheme.muted("Continuing your message in a fresh Turn.")}`,
        );
    ...
    if (busy.active) {
      ...
      const turns =
        ...
        Array.isArray(snapshot.turns)
          ? (snapshot.turns as Array<Record<string, unknown>>)
          : [];
  ...
  const launchRunnerCommand = async (
    command: "model" | "auth",
    commandArgs: string[] = [],
  ): Promise<void> =>
    withConfigurationScreen(async () => {
      ...
      push(
        (await reloadDaemon(stateDir, configPath))
          ? "Configuration updated — applies to the next Turn."
          : "Configuration saved — restart Goah to apply it.",
      );
    });
  const launchSetup = async (text: string): Promise<void> =>
    withConfigurationScreen(async () => {
      const current = configuredRunnerProfile(configPath);
      ...
      if (section === "runner" || !current) {
        ...
        push(
          (await reloadDaemon(stateDir, configPath))
            ? "Runner profile updated — applies to the next Turn."
            : "Runner profile saved — restart Goah to apply it.",
        );
        return;
      }
  ...
  input.onSubmit = (line) => {
    ...
    if (
      ["send", "steer", "goal"].includes(action) &&
      looksLikeCredential(text) &&
      sensitiveConfirmation !== text
    ) {
      sensitiveConfirmation = text;
      input.setText(text);
    ...
    if (action === "status") {
      void printStatus(stateDir, push).finally(() =>
        refreshOrganizationStatus(stateDir, organizationView, tui, currentModel()),
      );
      return;
    }
    if (action === "records") {
    ...
    if (action === "model") {
      if (text === "/model") void launchRunnerCommand("model");
      else void switchModelCommand(text, configPath, stateDir, push);
      return;
    }
    if (action === "login") {
      void launchRunnerCommand(
        "auth",
        ["login", text.slice("/login".length).trim()].filter(Boolean),
      );
      return;
    }
    if (action === "logout") {
      void launchRunnerCommand(
        "auth",
        ["logout", text.slice("/logout".length).trim()].filter(Boolean),
      );
      return;
    }
    if (action === "setup") {
    ...
    if (action === "goal") {
      void slashGoal(text, stateDir, push, (objective) =>
        send(`/goal ${objective}`, true, { op: "goal.interact", objective }),
      ).finally(() => refreshOrganizationStatus(stateDir, organizationView, tui, currentModel()));
      return;
    }
    if (action === "unknown") {
  ...
  tui.addInputListener((data) => {
    ...
    if (action === "interrupt" && !interrupting) {
      interrupting = true;
      push(
        `${tuiTheme.warning("stopping")}  ${tuiTheme.muted("Current Turn is being interrupted; press Ctrl+C again to exit.")}`,
      );
      void stopCeoWake(stateDir, push);
      return { consume: true };
    }
  ...
  try {
    tui.start();
    if (initialMessage && looksLikeCredential(initialMessage)) {
      sensitiveConfirmation = initialMessage;
...
export function findLiveTurnId(turns: Array<Record<string, unknown>>): string | null {
  const id = [...turns]
    .reverse()
    .find((turn) => turn.status === "in_progress" && turn.triggerKind === "user_message")?.id;
...
async function runNonInteractive(
  ...
  if (initialMessage && looksLikeCredential(initialMessage))
    throw new Error(
      "Credential-like input is not accepted non-interactively. Put the secret in an ignored local file, then tell Goah only its path.",
    );
  await ensureDaemon(configPath, stateDir);
...
export function renderFrame(
  ...
  appendLive: (text: string) => void = push,
  commitLive: (text: string, messageId?: string) => void = push,
  pushResponse: (text: string, messageId?: string) => void = push,
  updateTool: (activity: ToolActivity) => void = (activity) => push(toolActivityLine(activity)),
  updateThinking: (activity: ThinkingActivity) => void = () => {},
  setWakeState: (state: "queued" | "working") => void = () => {},
  clearLive: () => void = () => {},
  replaceLive: (text: string, thinking: string, thinkingActive: boolean) => void = () => {},
): void {
  if (frame.type === "error") {
  ...
  if (frame.type === "result") {
    ...
    const response =
      value.response && typeof value.response === "object" && !Array.isArray(value.response)
        ...
        : {};
    const turn =
      value.turn && typeof value.turn === "object" && !Array.isArray(value.turn)
  ...
  if (record.type === "tool.called") {
    ...
    updateTool({
      kind: "tool",
      callId: String(data.callId ?? "tool"),
      name: String(data.name ?? "tool"),
      detail: toolDetail(data.arguments),
      status: "running",
    });
    ...
    const delta =
      data.delta && typeof data.delta === "object" && !Array.isArray(data.delta)
    ...
    if (typeof data.text === "string" && data.text.trim())
      commitLive(
        data.text.trim(),
        typeof data.messageItemId === "string" ? data.messageItemId : undefined,
      );
  } else if (record.type === "handoff.recorded") {
    if (typeof data.goalId === "string")
      push(
        `${data.outcome === "blocked" ? tuiTheme.error("goal blocked") : tuiTheme.success("goal saved")}  ${tuiTheme.muted(`${String(data.outcome).replaceAll("_", " ")} · record r${String(data.recordRevision)}`)}`,
      );
  } else if (record.type === "transcript.interrupted") {
    updateThinking({ phase: "clear", text: "" });
    clearLive();
    push(
      `${tuiTheme.error("error")}  ${safeError(typeof data.reason === "string" ? data.reason : "Turn failed")}\n`,
    );
  }
}
...
function toolDetail(value: unknown): string {
  ...
  const candidate = [args.path, args.file_path, args.query, args.command, args.goalId].find(
...
function toolActivityLine(activity: ToolActivity): string {
  const state =
    activity.status === "running"
      ? tuiTheme.accent("running")
      : activity.status === "failed"
        ? tuiTheme.error("failed")
        : tuiTheme.success("done");
...
async function printStatus(stateDir: string, push: (line: string) => void): Promise<void> {
  try {
    ...
    push(
      [
        tuiTheme.strong("Status"),
        root
          ? `  ${tuiTheme.active(" GOAL ")}  ${String(root.objective)}  ${tuiTheme.muted(String(root.phase))}`
          : `  ${tuiTheme.muted("No current Goal")}`,
        `  ${tuiTheme.muted(team.length ? team.map((member) => `${String(member.agent)} ${String(member.motion)}${member.lastOutcome ? `/${String(member.lastOutcome)}` : ""}`).join(" · ") : "No Goal Agents")}`,
        "",
      ].join("\n"),
    );
  } catch (error) {
...
async function refreshOrganizationStatus(
  stateDir: string,
  view: OrganizationStatusLine,
  tui: TuiAltScreen,
  model: string,
): Promise<void> {
  try {
...
async function slashGoal(
  ...
  try {
    ...
    const root =
      ...
      (status as Record<string, unknown>).root &&
      typeof (status as Record<string, unknown>).root === "object"
        ? ((status as Record<string, unknown>).root as Record<string, unknown>)
        : undefined;
    if (!root) {
      if (!isGoal) throw new Error("no active root goal");
      if (startGoal) {
        await startGoal(value);
        return;
      }
      ...
      const goal =
        created && typeof created === "object" && !Array.isArray(created)
          ? ((created as Record<string, unknown>).goal as Record<string, unknown> | undefined)
          : undefined;
    ...
    const goal =
      ...
      (updated as Record<string, unknown>).goal &&
      typeof (updated as Record<string, unknown>).goal === "object"
        ? ((updated as Record<string, unknown>).goal as Record<string, unknown>)
        : (updated as Record<string, unknown>);
    push(
      `${tuiTheme.success(isGoal ? "goal updated" : "observation set")}  ${String(goal?.objective ?? value)}\n`,
    );
  } catch (error) {
    push(errorLine(error));
  }
...
function errorLine(error: unknown): string {
  return `${tuiTheme.error("error")}  ${safeError(error instanceof Error ? error.message : String(error))}\n`;
}
function safeError(value: string): string {
  const sanitized = redactSensitiveText(value)
    .replace(
      /environment variable is missing:\s*[^\s(]+/gi,
      "environment variable is missing: [REDACTED]",
    )
    .replace(/(?:sk|key|token)[-_][A-Za-z0-9._-]{12,}/gi, "[REDACTED]");
  const provider = sanitized.match(/^(\d{3}):\s*(\{.*\})$/s);
  if (provider) {
    try {
      const body = JSON.parse(provider[2]!) as { message?: unknown };
      if (typeof body.message === "string") return `Provider ${provider[1]}: ${body.message}`;
    } catch {}
  }
  const named = sanitized.match(
    /^(?:TypeError|RangeError|ReferenceError|SyntaxError|Error):\s*[^\n]+/m,
  )?.[0];
  ...
  return (
    sanitized
      ...
      .map((line) => line.trim())
      .find(
        (line) => line && !line.startsWith("file://") && !line.startsWith("at ") && line !== "^",
      ) ?? "Turn failed"
  );
}
export function sensitiveMessageWarning(): string {
  return `${tuiTheme.warning("credential-like message")}  ${tuiTheme.muted("Press Enter again to send it to the model and Ledger, or put the secret in an ignored local file and mention only its path.")}`;
}
  ...
  return (value as unknown[])
    .map((item) =>
      ...
      typeof (item as Record<string, unknown>).text === "string"
        ? normalizeAssistantText((item as Record<string, unknown>).text as string)
        : "",
    )
    .filter(Boolean)
    .join("\n");
...
function configuredRunnerProfile(configPath: string) {
  ...
  return (
    workspace?.runnerProfiles?.find((profile) => profile.id === "default") ??
    workspace?.runnerProfiles?.[0] ??
