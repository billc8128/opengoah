...
export function consoleMetadataPath(stateDir: string): string {
  return join(stateDir, "console.json");
}
...
export function organizationTrajectory(
  ledger: Ledger,
  options: { beforeSeq?: number; limit?: number; agent?: string; type?: string } = {},
): TrajectoryPage {
  ...
  const candidates = ledger
    ...
    .filter((event) => {
      ...
      const wakeId = event.streamId.startsWith("wake:")
        ? event.streamId.slice("wake:".length)
        : null;
  ...
  return {
    items: page.map((event) => {
      const wakeId = event.streamId.startsWith("wake:")
        ...
        : null;
      return {
        event: redactValue(event) as JsonValue,
        agent: wakeId ? (wakeAgents.get(wakeId) ?? event.actor) : event.actor,
        wakeId,
      };
    }),
...
export async function runWebConsole(
  ...
  if (!["127.0.0.1", "localhost", "::1"].includes(host))
    throw new Error("Goah Console only binds to loopback hosts");
  ...
  const server = createServer((request, response) => {
    route(request, response, supervisor, ledger, token, allowedHost).catch((error: unknown) => {
      ...
      if (response.headersSent)
        response.end(`data: ${JSON.stringify({ ...message, type: "error" })}\n\n`);
  ...
  if (!address || typeof address === "string")
    throw new Error("console server did not bind a TCP port");
...
async function route(
  ...
  if (request.headers.host !== allowedHost) {
    sendJson(response, 403, { error: "console host is not allowed" });
    return;
  }
  const url = new URL(request.url ?? "/", "http://127.0.0.1");
  if (url.pathname.startsWith("/api/")) {
    if (!authorized(request, url, token)) {
      sendJson(response, 403, { error: "console authorization failed" });
      return;
    }
    if (request.method === "GET" && url.pathname === "/api/snapshot") {
      sendJson(response, 200, consoleSnapshot(ledger, supervisor));
      return;
    }
    if (request.method === "GET" && url.pathname === "/api/trajectory") {
      const options: { beforeSeq?: number; limit?: number; agent?: string; type?: string } = {};
      const beforeSeq = positiveInteger(url.searchParams.get("beforeSeq"));
      if (beforeSeq) options.beforeSeq = beforeSeq;
      const limit = positiveInteger(url.searchParams.get("limit"));
      if (limit) options.limit = limit;
      const agent = nonEmpty(url.searchParams.get("agent"));
      if (agent) options.agent = agent;
      const type = nonEmpty(url.searchParams.get("type"));
    ...
    if (request.method === "GET" && url.pathname.startsWith("/api/threads/")) {
      ...
      const turns = ledger.turns(threadId);
      sendJson(
        response,
        200,
        redactValue({
          ...
          turns: turns.map((turn) => ({
            ...turn,
            leaseToken: null,
            items: ledger.turnItems(turn.id),
          })),
        }),
      );
      return;
  ...
  if (relative === "index.html")
    response.setHeader("set-cookie", `goah_console=${token}; Path=/; HttpOnly; SameSite=Strict`);
  ...
  if (!path.startsWith(assetsDirectory) || !existsSync(path) || !statSync(path).isFile()) {
    ...
    if (!existsSync(fallback)) {
      ...
      response.end("Console assets are not built. Run npm run build.");
      return;
    }
...
function authorized(request: IncomingMessage, url: URL, token: string): boolean {
  const supplied =
    request.headers.authorization?.replace(/^Bearer\s+/i, "") ?? url.searchParams.get("token");
  if (supplied === token) return true;
  const cookieToken = request.headers.cookie
    ?.split(";")
    .map((value) => value.trim())
    .find((value) => value.startsWith("goah_console="))
    ?.slice("goah_console=".length);
