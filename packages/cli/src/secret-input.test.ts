...
test("secret input never renders its value", () => {
  const input = new SecretInput(); input.setValue("sk-live-secret"); input.focused = true;
...
test("setup defaults place the cursor at the end", () => {
  const input = new SecretInput(); setInitialValue(input, "ZAI_API_KEY"); input.handleInput("X");
