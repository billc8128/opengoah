...
async function main(): Promise<void> {
  ...
  if (
    rawCommand &&
    ["setup", "model", "auth", "login", "logout"].includes(rawCommand) &&
    args.slice(1).some((value) => value === "--help" || value === "-h")
  ) {
    printConfigurationHelp(rawCommand);
    return;
  }
  if (rawCommand === "update") {
    await runUpdate({
      check: flag("--check"),
      dryRun: flag("--dry-run"),
      ...(option("--version") ? { target: option("--version")! } : {}),
    });
    return;
  }
  const typo =
    rawCommand && !rawCommand.startsWith("-") && !knownCommand(rawCommand)
      ? closestCommand(rawCommand)
      : null;
  if (typo && args.length === 1 && !rawCommand!.includes(" "))
    throw new Error(
      `Unknown command "${rawCommand}". Did you mean "goah ${typo}"? Use "goah goal start --objective …" to start a goal explicitly.`,
    );
  const interactive =
    rawCommand === undefined ||
    rawCommand === "--continue" ||
    Boolean(rawCommand && !rawCommand.startsWith("-") && !knownCommand(rawCommand));
  const command = interactive ? "interactive" : rawCommand!;
  const initialMessage =
    interactive && rawCommand && rawCommand !== "--continue"
      ? args.filter((value) => !value.startsWith("--")).join(" ")
      : null;
  ...
  if (command === "setup") {
    ...
    if (requested && !["runner", "model", "auth"].includes(requested))
      throw new Error("usage: goah setup [runner|model|auth]");
    const section = requested ?? (current ? await chooseSetupSection(current) : "runner");
    if (!section) {
      console.log("Setup cancelled; nothing changed.");
      return;
    }
    ...
    console.log(
      `Profile saved to ~/.goah/profile.json${existsSync(configPath) ? " and this workspace was updated" : " (run \`goah\` in any directory to create a workspace)"}`,
    );
    return;
  }
  if (command === "login" || command === "logout") {
    await runRunnerWizardEarly("auth", [command, ...configArgs], configPath);
    return;
  }
  if ((command === "auth" || command === "model") && configArgs.length === 0) {
    await runRunnerWizardEarly(command, [], configPath);
    return;
  }
  if (command === "auth" || command === "model") {
    await runRunnerCommand(command, configArgs, configPath);
    return;
  }
  if (command === "runner" && ["list", "setup"].includes(args[1] ?? "list")) {
    await runRunnerEarly(configPath);
    return;
  }
  if (command === "init") {
    ...
    writeDefaultConfig(configPath, {
      ...
      ...(option("--context-window-tokens")
        ? { contextWindowTokens: numberOption("--context-window-tokens") }
        : {}),
      ...(option("--max-output-tokens")
        ? { maxOutputTokensPerTurn: numberOption("--max-output-tokens") }
        : {}),
  ...
  const config: GoahConfig =
    command === "interactive"
      ? (existsSync(configPath) || (await bootstrapWorkspace(configPath), true),
        loadConfig(configPath))
      : (() => {
          if (!existsSync(configPath))
            throw new Error(
              `no Goah workspace here (missing goah.config.json); run \`goah\` to create one interactively, or \`goah init\` for flag-based setup`,
            );
          return loadConfig(configPath);
        })();
  if (command === "runner") {
    await runRunnerManagement(config, configPath);
    return;
  }
  if (command === "daemon") {
    await runDaemonCommand(config, configPath);
    return;
  }
  if (command === "doctor") {
  ...
  if (command === "interactive") {
    await runGoahTui(configPath, config.stateDir, initialMessage);
    return;
  }
  if (command !== "start" && (await controlAvailable(config.stateDir))) {
    const request = remoteRequest(command);
    if (request) {
      console.log(JSON.stringify(await requestControl(config.stateDir, request), null, 2));
      return;
    }
  }
  ...
  try {
    ...
    if (command === "start") {
      ...
      const services = [
        ...
        runWebConsole(supervisor, ledger, config.stateDir, controller.signal, {
          onListening: ({ url }) => console.log(`Goah Console: ${url}`),
        }),
      ];
      try {
      ...
      const wake = supervisor.planWake(
        agent,
        new Date().toISOString(),
        option("--reason") ?? "manual wake",
        "supervisor",
      ...
      console.log(
        JSON.stringify(
          supervisor.startGoal(required("--objective"), option("--id") ?? undefined),
          null,
          2,
        ),
      );
      ...
      console.log(
        JSON.stringify(await supervisor.sendToCeo({ message: required("--message") }), null, 2),
      );
      ...
      if (subcommand === "list") console.log(JSON.stringify(listThreads(ledger), null, 2));
        ...
        if (subcommand === "show")
          console.log(JSON.stringify(showThread(ledger, threadId), null, 2));
        else if (subcommand === "replay")
          console.log(JSON.stringify(replayThread(ledger, threadId), null, 2));
        else if (subcommand === "export") {
          const output = option("--output") ?? `${threadId}.thread.json`;
          writeFileSync(
            output,
            `${JSON.stringify(exportThread(ledger, threadId, { raw: flag("--raw") }), null, 2)}\n`,
          );
          console.log(JSON.stringify({ output, redacted: !flag("--raw") }, null, 2));
        } else throw new Error(`unknown thread command: ${subcommand}`);
      }
    } else if (command === "context") {
      if (requiredPositional(1, "context command") !== "show")
        throw new Error("unknown context command");
      console.log(
        JSON.stringify(showTurnContext(ledger, requiredPositional(2, "turn id")), null, 2),
      );
    } else if (command === "events") {
      console.log(
        JSON.stringify(
          streamEvents(ledger, required("--stream"), option("--from") ? numberOption("--from") : 1),
          null,
          2,
        ),
      );
      ...
      const notes = ledger
        .readStream(memoryStream(requiredPositional(1, "agent")))
        .filter((event) => event.type === "memory.appended")
        .slice(-tail);
      console.log(
        JSON.stringify(
          notes.map((event) => ({
            seq: event.seq,
            ts: event.ts,
            note: (event.data as { note?: unknown }).note ?? null,
            wakeId: (event.data as { wakeId?: unknown }).wakeId ?? null,
          })),
          null,
          2,
        ),
      ...
      const observationMethod = option("--observation-method");
      const verificationMethod = option("--verification-method") ?? observationMethod;
      const goal = {
        id,
        parentId,
        objective,
      ...
      if (!parentId && owner !== "ceo")
        throw new Error("Root Goal owner must be ceo; non-CEO Goals require --parent");
      const parent = parentId ? ledger.goal(parentId) : null;
      if (parentId && !parent) throw new Error("parent goal not found");
      supervisor.createGoal(goal, option("--actor") ?? parent?.owner ?? "human");
      const wake = flag("--wake-now")
        ? supervisor.planWake(owner, new Date().toISOString(), `goal:${id}`, "supervisor", {
            goalId: id,
          })
        : null;
      ...
      const observationMethod = option("--observation-method");
      const verificationMethod = option("--verification-method");
      if (option("--owner"))
        throw new Error("Goal ownership changes require atomic CEO reassignment, not goal-update");
      const id = requiredPositional(1, "goal id");
      const current = ledger.goal(id);
      if (!current) throw new Error("goal not found");
      const actor =
        option("--actor") ??
        (current.parentId ? ledger.goal(current.parentId)?.owner : "human") ??
        "human";
      const goal = supervisor.updateGoal(
        id,
        {
          ...(objective ? { objective } : {}),
      ...
      const path = option("--output") ?? join(config.stateDir, "status.html");
      writeFileSync(path, (await import("goah-supervisor")).renderDashboard(ledger));
      console.log(path);
...
async function run(
  supervisor: ReturnType<typeof createRuntime>["supervisor"],
  signal: AbortSignal,
): Promise<void> {
...
function remoteRequest(command: string): ControlRequest | null {
  ...
  if (command === "goal-update") {
    ...
    return {
      op: "goal.update",
      id: requiredPositional(1, "goal id"),
      ...(option("--objective") ? { objective: option("--objective")! } : {}),
      ...(option("--observation-method")
        ? { observationMethod: option("--observation-method")! }
        : {}),
      ...(option("--verification-method")
        ? { verificationMethod: option("--verification-method")! }
        : {}),
    };
  }
  if (command === "goal-pause" || command === "goal-resume")
    return {
      op: "goal.transition",
      id: requiredPositional(1, "goal id"),
      phase: command === "goal-pause" ? "paused" : "active",
    };
  if (command === "goal-complete")
    return {
      op: "goal.complete",
      id: requiredPositional(1, "goal id"),
      reason: required("--reason"),
...
function evidence(): number[] {
  return required("--evidence").split(",").map(Number);
}
function providerOption(value: string): string {
  return value.trim();
}
function mutates(command: string): boolean {
  return [
    "start",
    "run-once",
    "wake",
...
function normalizeArgs(values: string[]): string[] {
  if ((values[0] === "goal" || values[0] === "ceo") && values[1] && !values[1].startsWith("--"))
    return [`${values[0]}-${values[1]}`, ...values.slice(2)];
  return values;
}
function knownCommand(value: string): boolean {
  return [
    "setup",
    "help",
    "version",
...
function firstRecord(value: JsonValue | undefined): Record<string, JsonValue> | null {
...
async function bootstrapWorkspace(configPath: string): Promise<void> {
  ...
  if (!profile) {
    console.log(
      profile
        ? "Your Goah setup is incomplete — resuming setup now."
        : "No Goah profile found — running first-use setup.",
    );
...
function currentRunnerProfile(configPath: string): RunnerProfile | null {
  ...
  return (
    workspace?.runnerProfiles?.find((item) => item.id === "default") ??
    workspace?.runnerProfiles?.[0] ??
    readDefaultRunnerProfile()
  );
}

async function runRunnerWizardEarly(
  command: string,
  commandArgs: string[],
  configPath: string,
): Promise<void> {
...
async function runRunnerCommand(
  ...
  if (!plugin.configurator.runCommand)
    throw new Error(`${profile.runner} does not expose runner commands.`);
  const interaction = stdioInteraction();
  let result: RunnerCommandResult;
  try {
    result = await plugin.configurator.runCommand(
      command,
      commandArgs,
      profile.config,
      interaction,
...
async function runRunnerEarly(configPath: string): Promise<void> {
  ...
  if (action === "list") {
    for (const manifest of (await import("./runner-registry.js")).runnerManifests())
      console.log(`${manifest.id.padEnd(16)} ${manifest.description}`);
    return;
  }
  const profileId = args[2] ?? "default";
  if (!existsSync(configPath) && profileId !== "default")
    throw new Error("Named Runner Profiles require a Goah workspace.");
  const current = existsSync(configPath)
    ? (loadConfig(configPath).runnerProfiles?.find((item) => item.id === profileId) ?? null)
    : readDefaultRunnerProfile();
  const result = await runSetupWizard(current);
  if (!result.profile) {
    console.log("Runner setup cancelled; nothing changed.");
    return;
  }
  const profile = { ...result.profile, id: profileId };
  try {
    if (profileId === "default")
      persistRunnerProfile(profile, existsSync(configPath) ? configPath : null);
...
async function runRunnerManagement(config: GoahConfig, configPath: string): Promise<void> {
  ...
  if (action === "status") {
    for (const profile of config.runnerProfiles ?? []) {
      const checks = await runnerPlugin(profile.runner).configurator.doctor(profile.config, {
        root: process.cwd(),
      });
      console.log(`${profile.id} · ${profile.runner}`);
      for (const check of checks)
        console.log(`  ${checkIcon(check)} ${check.name}: ${check.detail}`);
    }
    return;
  }
  if (action === "doctor") {
  ...
  if (action === "profile") {
    ...
    if (subcommand === "list") {
      for (const profile of config.runnerProfiles ?? []) {
        const agents = (config.profiles ?? [])
          .filter((agent) => agent.runnerProfile === profile.id)
          .map((agent) => agent.agent);
        console.log(
          `${profile.id.padEnd(18)} ${profile.runner.padEnd(12)} agents=${agents.join(",") || "none"}`,
        );
      }
      return;
    }
...
async function diagnoseRunnerProfiles(
  config: GoahConfig,
  configPath = "goah.config.json",
): Promise<Array<{ ok: boolean; name: string; detail: string; severity?: "warning" | "error" }>> {
  const checks: Array<{
    ok: boolean;
    name: string;
    detail: string;
  ...
  for (const profile of config.runnerProfiles ?? []) {
    try {
      for (const check of await runnerPlugin(profile.runner).configurator.doctor(profile.config, {
        root: resolve(configPath, ".."),
      }))
        checks.push({ ...check, name: `runner:${profile.id}:${check.name}` });
    } catch (error) {
      checks.push({
        ok: false,
        name: `runner:${profile.id}`,
...
function checkPasses(check: { ok: boolean; severity?: "warning" | "error" }): boolean {
  return check.ok || check.severity === "warning";
}
function checkIcon(check: { ok: boolean; severity?: "warning" | "error" }): string {
  return check.ok ? "✓" : check.severity === "warning" ? "!" : "✗";
}
...
async function runDaemonCommand(config: GoahConfig, configPath: string): Promise<void> {
  ...
  if (action === "status") {
    ...
    console.log(
      running ? `running${metadata ? ` · pid ${metadata.pid} · ${metadata.url}` : ""}` : "stopped",
    );
    return;
  }
  if (action === "logs") {
  ...
  if (action === "stop" || action === "restart") {
    if (await controlAvailable(config.stateDir))
      await requestControl(config.stateDir, { op: "daemon.stop" });
    const deadline = Date.now() + 5_000;
    while ((await controlAvailable(config.stateDir)) && Date.now() < deadline)
      await new Promise((resolve) => setTimeout(resolve, 50));
    if (action === "stop") {
      console.log("Goah daemon stopped.");
      return;
    }
    await ensureDaemon(configPath, config.stateDir);
...
function stdioInteraction(): RunnerSetupInteraction & { close(): void } {
  ...
  return {
    select: async ({ title, choices }) => {
      const answer = Number(
        await ask(
          `${title}: ${choices.map((choice, index) => `${index + 1}) ${choice.label}`).join("  ")} — select: `,
        ),
      );
      return choices[answer - 1]?.value ?? null;
    },
    input: async ({ prompt, initial }) =>
      (await ask(`${prompt}${initial ? ` (${initial})` : ""}: `)) || initial || null,
