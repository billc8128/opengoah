...
test("recovery state distinguishes escalation, unrelated schedules, and failed retries", () => {
  ...
  const now = new Date().toISOString()
  runtime.ledger.putSchedule({ id: "ordinary", ...goalAutomaticTarget("ceo","root"),nextWakeAt: new Date(Date.parse(now) + 60_000).toISOString(), reason: "recovery:failed-b", setBy: "ceo", status: "pending", resolvedAt: null }, "ceo")
  ...
  runtime.ledger.enqueueWake({ id: "escalation-a", ...goalAutomaticTarget("ceo","root"),triggerRef: "child-retry-exhausted:failed-a", status: "queued", attempt: 0, enqueuedSeq: 0, claimedAt: null, consumedAt: null, turnId: null }, "supervisor")
...
function failGoalWake(runtime: ReturnType<typeof createRuntime>, goalId: string, agent: string, wakeId: string, turnId: string, triggerRef: string): void {
  ...
  runtime.ledger.enqueueWake({ id: wakeId, ...goalAutomaticTarget(agent,goalId),triggerRef, status: "queued", attempt: 0, enqueuedSeq: 0, claimedAt: null, consumedAt: null, turnId: null }, "supervisor")
  assert.equal(runtime.ledger.claimNextWake(now)?.id, wakeId)
  runtime.ledger.startTurnFromWake(wakeId, { id: turnId, threadId: thread.id, triggerKind:"wake", goalId, goalRevision: runtime.ledger.goal(goalId)!.revision, status: "in_progress", attempt: 1, error: null, startedAt: now, endedAt: null, leaseUntil: new Date(Date.parse(now) + 60_000).toISOString(), leaseToken: turnId, runnerPid: null }, now)
