...
export function planNpmUpdate(
  ...
  const globalRoots =
    process.platform === "win32"
      ...
      : [join(globalPrefix, "lib", "node_modules"), join(globalPrefix, "node_modules")];
  if (globalRoots.some((directory) => isInside(installed, directory)))
    return {
      command: npm,
      args: ["install", "--global", `@goah/cli@${target}`],
      mode: "global",
      prefix: globalPrefix,
    };
  ...
  if (index < 0)
    throw new Error(
      "This Goah is running from a source checkout. Build or install @goah/cli instead of self-updating the checkout.",
    );
  const prefix = installed.slice(0, index);
  return {
    command: npm,
    args: ["install", "--prefix", prefix, `@goah/cli@${target}`],
    mode: "prefix",
    prefix,
  };
}
...
export async function runUpdate(
  options: { check?: boolean; dryRun?: boolean; target?: string } = {},
): Promise<void> {
  const current = installedVersion();
  const target: string =
    options.target !== undefined
      ? options.target
      : await capture("npm", ["view", "@goah/cli", "version", "--json"]).then((text) =>
          String(JSON.parse(text)),
        );
  if (current === target) {
    console.log(`Goah ${current} is already up to date.`);
    return;
  }
  const globalPrefix = (await capture("npm", ["prefix", "--global"])).trim();
  const plan = planNpmUpdate(packageRoot(), globalPrefix, target);
  if (options.check) {
    console.log(`Goah ${current} → ${target} is available.`);
    return;
  }
  ...
  if (updated !== target)
    throw new Error(
      `npm completed but the active installation reports ${updated}, expected ${target}`,
    );
  console.log(
    `Updated Goah to ${updated}. The next goah launch will restart the resident daemon automatically.`,
  );
}
