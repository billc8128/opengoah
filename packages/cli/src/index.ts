...
export function loadConfig(path = "goah.config.json"): GoahConfig {
  ...
  const config = JSON.parse(readFileSync(absolute, "utf8")) as GoahConfig & {
    workspace?: string;
    limits?: unknown;
    heartbeatPolicies?: unknown;
    progressPolicies?: unknown;
  };
  ...
  if (config.runner) {
    ...
    config.runner.args = config.runner.args.map((arg) =>
      arg === "$GOAH_PI_WORKER" ? piWorkerPath() : arg,
    );
  }
  if (
    !config.runnerProfiles?.length &&
    config.runner?.env?.GOAH_PI_PROVIDER &&
    config.runner.env.GOAH_PI_MODEL
  ) {
    const env = config.runner.env;
    const provider = env.GOAH_PI_PROVIDER!;
    const model = env.GOAH_PI_MODEL!;
    const keyRef = Object.entries(env).find(
      ([key, value]) => key.endsWith("API_KEY") && value.startsWith("env:"),
    );
    config.runnerProfiles = [
      {
        ...
        config: {
          provider,
          model,
          ...(keyRef ? { apiKeyEnv: keyRef[1].slice(4) } : {}),
          ...(env.GOAH_PI_BASE_URL ? { baseUrl: env.GOAH_PI_BASE_URL } : {}),
        },
      },
    ];
    if (config.profiles)
      config.profiles = config.profiles.map((profile) => ({
        ...profile,
        runnerProfile: profile.runnerProfile ?? "default",
      }));
  }
  return config;
}

export function createRuntime(config: GoahConfig): {
  ledger: SqliteLedger;
  ...
  const runner = config.runnerProfiles?.length
    ? new RunnerRouter(
        new Map(
          config.runnerProfiles.map((profile) => [
            profile.id,
            runnerPlugin(profile.runner).create(profile.config, configRoot(config)),
          ]),
        ),
      )
    : legacyRunner(config);
...
export function defaultConfig(directory: string, options: InitOptions = {}): GoahConfig {
  ...
  const runnerProfile: RunnerProfile = {
    ...
    config: {
      ...
      ...(options.maxOutputTokensPerTurn
        ? { maxOutputTokens: options.maxOutputTokensPerTurn }
        : {}),
    },
  };
  return {
    ...
    runnerProfiles: [runnerProfile],
    profiles: [
      { agent: "ceo", role: "ceo", runnerProfile: "default" },
      { agent: options.agent ?? "worker", role: "child", runnerProfile: "default" },
    ],
  };
}
...
export class SupervisorLock {
  ...
  constructor(stateDir: string) {
    this.path = join(stateDir, "supervisor.lock");
  }
  acquire(): void {
...
export function writeDefaultConfig(
  ...
  writeFileSync(
    absolute,
    `${JSON.stringify(defaultConfig(dirname(absolute), options), null, 2)}\n`,
  );
}

export function profilePath(): string {
  return join(process.env.GOAH_STATE_HOME ?? join(homedir(), ".goah"), "profile.json");
}
...
export function readDefaultRunnerProfile(): RunnerProfile | null {
  ...
  try {
    ...
    if (
      ...
      return raw as unknown as RunnerProfile;
    if (typeof raw.provider === "string" && typeof raw.model === "string") {
      const { provider, model, apiKeyEnv, baseUrl, contextWindowTokens, maxOutputTokensPerTurn } =
        raw;
      return {
        id: "default",
        runner: "pi",
        config: {
          ...
          ...(typeof maxOutputTokensPerTurn === "number"
...
export function updateWorkspaceRunnerProfile(path: string, profile: RunnerProfile): void {
  withProfileLock(() => {
    recoverProfileTransactionUnlocked();
    updateWorkspaceRunnerProfileUnlocked(path, profile);
  });
}
function updateWorkspaceRunnerProfileUnlocked(path: string, profile: RunnerProfile): void {
  const absolute = resolve(path);
  const current = existsSync(absolute)
    ? (JSON.parse(readFileSync(absolute, "utf8")) as GoahConfig)
    : defaultConfig(dirname(absolute));
  current.runnerProfiles = [
    ...(current.runnerProfiles ?? []).filter((item) => item.id !== profile.id),
    profile,
  ];
  current.profiles = (
    current.profiles ?? [
  ...
  ).map((agent) => ({ ...agent, runnerProfile: agent.runnerProfile ?? profile.id }));
...
function profileTransactionPath(): string {
  return join(dirname(profilePath()), "profile-transaction.json");
}
function writeProfileTransaction(snapshots: ProfileSnapshot[]): void {
  ...
  writeFileSync(
    temporary,
    `${JSON.stringify({ version: 1, snapshots: snapshots.map((snapshot) => ({ path: snapshot.path, content: snapshot.content?.toString("base64") ?? null })) })}\n`,
    { mode: 0o600 },
  );
  ...
  for (const snapshot of snapshots) {
    if (snapshot.content === null) rmSync(snapshot.path, { force: true });
      ...
      writeFileSync(
        temporary,
        snapshot.content,
        snapshot.path === profilePath() ? { mode: 0o600 } : undefined,
      );
...
function recoverProfileTransaction(): void {
  if (existsSync(profileTransactionPath())) withProfileLock(recoverProfileTransactionUnlocked);
}
function recoverProfileTransactionUnlocked(): void {
  ...
  const value = JSON.parse(readFileSync(path, "utf8")) as {
    version?: unknown;
    snapshots?: Array<{ path?: unknown; content?: unknown }>;
  };
  ...
  const snapshots = value.snapshots.map((snapshot): ProfileSnapshot => {
    ...
    return {
      path: snapshot.path,
      content: snapshot.content === null ? null : Buffer.from(snapshot.content, "base64"),
    };
  });
...
export function statusSnapshot(ledger: SqliteLedger): object {
  ...
  const wakes = ledger.wakes().map((wake) => {
    ...
    return {
      ...wake,
      tokensUsed,
      abnormalReason: turn?.status === "failed" ? field(turn.error, "message") : null,
    };
  });
  const goals = ledger.goals();
  const handoffs = events
    .filter((event) => event.type === "handoff.recorded")
    .slice(-20)
    .map((event) => ({
      seq: event.seq,
      ts: event.ts,
  ...
  const modelCapabilitiesByAgent = Object.fromEntries(
    ledger.threads().flatMap((thread) => {
      const turnIds = new Set(ledger.turns(thread.id).map((turn) => turn.id));
      const capability = [...events]
        .reverse()
        .find(
          (event) =>
            event.type === "transcript.started" &&
            event.streamId.startsWith("turn:") &&
            turnIds.has(event.streamId.slice("turn:".length)),
        )?.data;
      return capability ? [[thread.agent, capability]] : [];
    }),
  );
  const modelCapabilities =
    modelCapabilitiesByAgent.ceo ??
    [...events].reverse().find((event) => event.type === "transcript.started")?.data ??
    null;
  return {
    ...
    goals,
    wakes,
    wakeTriggers: wakes.flatMap((wake) => ledger.wakeTriggers(wake.id)),
    schedules: ledger.schedules(),
    modelCapabilities,
    modelCapabilitiesByAgent,
...
function defaultModel(provider: string): string {
  ...
  throw new Error(
    `--model is required for ${provider}; interactive setup discovers models through the selected Runner.`,
  );
}
...
function absolutePath(base: string, value: string): string {
...
export function configRoot(config: GoahConfig): string {
  return configRoots.get(config) ?? process.cwd();
}
export {
  exportThread,
  listThreads,
