...
export function listThreads(ledger: SqliteLedger): ThreadListItem[] {
  return ledger
    .threads()
    .map((thread) => summarizeThread(ledger, thread.id))
...
export function showThread(ledger: SqliteLedger, threadId: string): ThreadDetail {
  ...
  return {
    thread: summarizeThread(ledger, threadId),
    turns: ledger.turns(threadId).map((turn) => {
      const events = ledger.readStream(`turn:${turn.id}`);
      return {
        ...turn,
        leaseToken: null,
        items: ledger.turnItems(turn.id),
        transcript: events.some((event) => event.type === "transcript.started")
          ? replayTranscript(events)
          : null,
      };
    }),
  };
...
export function exportThread(
  ledger: SqliteLedger,
  threadId: string,
  options: { raw?: boolean; now?: string } = {},
): ThreadExport {
  ...
  const value: ThreadExport = {
    ...
    events: ledger
      .turns(threadId)
      .flatMap((turn) => ledger.readStream(`turn:${turn.id}`))
...
function summarizeThread(ledger: SqliteLedger, threadId: string): ThreadListItem {
  ...
  const items = turns.flatMap((turn) => ledger.turnItems(turn.id));
  return {
    threadId,
    agent: thread.agent,
    status: turns.some((turn) => turn.status === "in_progress") ? "in_progress" : "idle",
