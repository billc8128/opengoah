...
export class SearchableSelect implements Component, Focusable {
  ...
  constructor(
    ...
    readonly options: SearchableSelectOptions = {},
  ) {
    this.#filtered = items;
  ...
  render(width: number): string[] {
    ...
    const status = this.query
      ? `${this.resultCount}/${this.items.length} matches · Esc clears`
      : `${this.items.length} options · type to search`;
    return [
      `  ${label}  ${this.theme.scrollInfo(status)}`,
  ...
  #createList(items: SelectItem[]): SelectList {
    const list = new SelectList(items, this.maxVisible, {
      ...
      noMatch: () =>
        this.theme.noMatch(
          `  ${this.options.emptyLabel ?? "No matches"}${this.query ? ` for “${this.query}”` : ""} · Esc clears`,
        ),
    });
