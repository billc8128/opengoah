...
function completedResult(
  request: RunRequest,
  text: string,
  id = `cli-test:assistant:${++completedMessageSequence}`,
...
function invoke(directory: string, ...args: string[]): string {
  return execFileSync(process.execPath, [cli, ...args], {
    cwd: directory,
    encoding: "utf8",
    env: { ...process.env, GOAH_STATE_HOME: testStateRoot },
  });
}
...
test("CLI initializes versioned config, resolves secret references, and enforces singleton lock", () => {
  ...
  assert.equal(
    initialized.profiles.some(
      (profile: { agent: string; role: string }) =>
  ...
  assert.equal(
    raw.profiles.every(
      (profile: { runnerProfile?: string }) => profile.runnerProfile === "default",
    ),
    true,
  );
  const diagnosed = diagnoseConfig(loadConfig(join(directory, "goah.config.json")));
  assert.equal(
    diagnosed.checks.some(
      (check: { name: string; ok: boolean }) => check.name === "runner" && check.ok,
    ),
    true,
  );
  const lock = new SupervisorLock(join(directory, ".goah"));
  lock.acquire();
  assert.throws(() => new SupervisorLock(join(directory, ".goah")).acquire(), /already running/);
  lock.release();
  const next = new SupervisorLock(join(directory, ".goah"));
  next.acquire();
  next.release();
});
...
test("profile persistence rolls the default back when the workspace write fails", () => {
  ...
  const directory = mkdtempSync(join(tmpdir(), "goah-profile-rollback-"));
  process.env.GOAH_STATE_HOME = join(directory, "state");
  try {
    assert.throws(() =>
      persistRunnerProfile(
        { id: "default", runner: "pi", config: { provider: "faux", model: "faux-goah" } },
        join(directory, "missing", "goah.json"),
      ),
    );
    assert.equal(existsSync(profilePath()), false);
  } finally {
    if (previous === undefined) delete process.env.GOAH_STATE_HOME;
    else process.env.GOAH_STATE_HOME = previous;
  }
});

test("profile persistence recovers an interrupted two-file transaction", () => {
  const previous = process.env.GOAH_STATE_HOME;
  const directory = mkdtempSync(join(tmpdir(), "goah-profile-recovery-"));
  const state = join(directory, "state");
  const workspace = join(directory, "goah.config.json");
  ...
  try {
    ...
    writeFileSync(
      profilePath(),
      `${JSON.stringify({ id: "default", runner: "pi", config: { provider: "faux", model: "partial" } })}\n`,
    );
    writeFileSync(workspace, oldWorkspace);
    writeFileSync(
      join(state, "profile-transaction.json"),
      `${JSON.stringify({
        version: 1,
        snapshots: [
          { path: profilePath(), content: Buffer.from(oldProfile).toString("base64") },
          { path: workspace, content: Buffer.from(oldWorkspace).toString("base64") },
        ],
      })}\n`,
    );
...
test("profile persistence refuses concurrent writers", () => {
  const previous = process.env.GOAH_STATE_HOME;
  const directory = mkdtempSync(join(tmpdir(), "goah-profile-lock-"));
  process.env.GOAH_STATE_HOME = join(directory, "state");
  try {
...
test("interactive stream follows redelivery wakes through recovery", async () => {
  ...
  const runner: Runner = {
    ...
    prepare: (request) => {
      ...
      const finalMessageId = "recovered";
      return {
        pid: null,
        begin: () => {
          if (attempt > 1) {
            request.emit({
              ...
              data: {
                ...
                delta: { type: "text_delta", delta: "recovered response" },
              },
            });
            request.emit({
              type: "message.assistant.completed",
        ...
        result: Promise.resolve(
          attempt === 1
            ? { outcome: "abnormal", reason: "temporary provider failure" }
            : { outcome: "completed", finalMessageId },
        ),
  ...
  const supervisor = new Supervisor(ledger, runner, clock, {
    turnRetryPolicy: { maxAttempts: 3, baseDelayMs: 0 },
  });
  const framesPromise = (async () => {
    const frames = [];
    for await (const frame of interactFrames("hello", supervisor, ledger)) frames.push(frame);
    return frames;
  })();
  ...
  assert.equal(
    frames.some(
      (frame) =>
        frame.type === "event" &&
        (frame.event as { type?: string }).type === "message.assistant.completed",
    ),
    true,
  );
...
test("interactive stream terminates when a Human turn completes with a canonical Assistant Item", async () => {
  ...
  const runner: Runner = {
    ...
    prepare: (request) => ({
      pid: null,
      begin: () => undefined,
      result: Promise.resolve(completedResult(request, "normal answer")),
      terminate: async () => undefined,
    }),
    terminateProcess: async () => undefined,
  };
  ...
  const framesPromise = (async () => {
    const frames = [];
    for await (const frame of interactFrames("create a goal", supervisor, ledger))
      frames.push(frame);
    return frames;
  })();
  const frames = await framesPromise;
  assert.equal(
    frames.some(
      (frame) =>
        frame.type === "event" &&
        (frame.event as { type?: unknown; data?: unknown }).type === "message.assistant.completed",
    ),
    true,
  );
...
test("an already-aborted control stream returns without opening a socket", async () => {
  const controller = new AbortController();
  controller.abort();
  await streamControl("/missing-goah-state", { op: "ping" }, () => undefined, controller.signal);
});

test("an internal empty assistant message does not suppress the canonical final Item", async () => {
  const clock: Clock = { now: () => new Date("2026-08-25T00:00:00.000Z") };
  const ledger = new SqliteLedger(":memory:", { clock });
  const runner: Runner = {
    isolation: "process",
    prepare: (request) => ({
      ...
      result: Promise.resolve({ outcome: "completed" as const, finalMessageId: "final" }),
      terminate: async () => undefined,
    }),
    terminateProcess: async () => undefined,
  };
  ...
  assert.equal(
    ledger
      .turnItems(
        (frames.find((frame) => frame.type === "accepted") as { type: "accepted"; turnId: string })
          .turnId,
      )
...
test("canonical streamed assistant text is not repeated in the terminal result", async () => {
  const clock: Clock = { now: () => new Date("2026-08-25T00:00:00.000Z") };
  const ledger = new SqliteLedger(":memory:", { clock });
  const runner: Runner = {
    isolation: "process",
    prepare: (request) => ({
      ...
      result: Promise.resolve({ outcome: "completed" as const, finalMessageId: "hello" }),
      terminate: async () => undefined,
    }),
    terminateProcess: async () => undefined,
  };
  ...
  const frames = [];
  for await (const frame of interactFrames("hello", supervisor, ledger)) frames.push(frame);
  const result = frames.findLast((frame) => frame.type === "result") as {
    type: "result";
    value: { response?: { content?: string } };
  };
  ...
  assert.equal(
    frames.filter(
      (frame) =>
        frame.type === "event" &&
        (frame.event as { type?: unknown }).type === "message.assistant.completed",
    ).length,
    1,
  );
  ledger.close();
...
test("Turn presentation includes Handoff and transcript terminal events", () => {
  assert.equal(isTurnPresentationEvent("handoff.recorded"), true);
  assert.equal(isTurnPresentationEvent("transcript.interrupted"), true);
  assert.equal(isTurnPresentationEvent("rpc.goal.get"), false);
});

test("welcome snapshot restores ordinary Human conversation", async () => {
  const state = mkdtempSync(join(tmpdir(), "goah-welcome-conversation-"));
  const clock: Clock = { now: () => new Date("2026-08-25T00:00:00.000Z") };
  ...
  const runner: Runner = {
    ...
    prepare: (request) => ({
      pid: null,
      begin: () => undefined,
      result: Promise.resolve(completedResult(request, "restored answer")),
      terminate: async () => undefined,
    }),
    terminateProcess: async () => undefined,
  };
  const supervisor = new Supervisor(ledger, runner, clock);
  const accepted = await supervisor.startHumanTurn("remember this question");
  await supervisor.waitForTurn(accepted.turnId);
  const now = clock.now().toISOString();
  const ceo = ledger.threads().find((thread) => thread.agent === "ceo")!;
  ledger.putThread(
    {
      id: "child-thread",
      agent: "worker",
      role: "verifier",
      parentThreadId: ceo.id,
      createdAt: now,
      updatedAt: now,
    },
    "supervisor",
  );
  putSyntheticTurn(ledger, "worker", "child-thread", "child-turn", now);
  ledger.putTurnItem(
    {
      id: "child-message",
      turnId: "child-turn",
      ...
      type: "user_message",
      status: "completed",
      data: { text: "internal worker prompt" },
      createdAt: now,
      completedAt: now,
    },
    "worker",
  ...
  ledger.putTurnItem(
    {
      ...
      status: "completed",
      data: { text: "internal worker answer" },
      createdAt: now,
      completedAt: now,
    },
    "worker",
  );
  ledger.commitTurnResponse("child-turn", "child-answer", now, "supervisor");
  ledger.putGoal(
    {
      id: "root",
      parentId: null,
  ...
  ledger.startTurnFromWake(wake.id, automaticTurn, now);
  ...
  const record = ledger.updateWorkRecord(
    {
      goalId: "root",
      goalRevision: 0,
      expectedRevision: 0,
      content: "# Current State\n\nWaiting for Human input.\n",
      reason: "record question",
      evidence: [evidence.seq],
      turnId: automaticTurn.id,
    },
    "ceo",
  );
  ledger.putTurnItem(
    {
      ...
      type: "assistant_message",
      status: "completed",
      data: { text: "Please choose the deployment timing." },
      createdAt: now,
      completedAt: now,
    },
    "ceo",
  ...
  ledger.commitHandoff({
    agent: "ceo",
    turnId: automaticTurn.id,
    sourceWakeId: wake.id,
    mailIds: [],
    ts: now,
    output: { handoff },
    responseItemId: "automatic-answer",
    item: {
...
test("welcome snapshot excludes every Item from a failed Human Turn", () => {
  ...
  ledger.commitTurnToGoal("t", { goalId: "g", goalRevision: 0 }, "supervisor");
  ledger.putTurnItem(
    {
      id: "provisional",
      turnId: "t",
...
test("version-one Pi config migrates in memory to an opaque Runner Profile", () => {
  ...
  assert.deepEqual(migrated.runnerProfiles, [
    { id: "default", runner: "pi", config: { provider: "faux", model: "faux-goah" } },
  ]);
...
test("CLI runs the install-to-first-handoff path with the faux provider", () => {
  ...
  assert.match(
    doctor.checks.find((item: { name: string }) => item.name === "runner").detail,
    /default:pi/,
  );
  invoke(
    directory,
  ...
  const turnId = run.wake.turnId;
  const status = JSON.parse(invoke(directory, "status"));
  assert.equal(
    status.goals.some((goal: { id: string }) => goal.id === "first"),
    true,
  );
  ...
  const detail = JSON.parse(
    invoke(directory, "thread", "show", "--config", "goah.config.json", threadId),
  );
  ...
  assert.equal(
    events.some((event: { type: string }) => event.type === "transcript.completed"),
    true,
  );
  const exportedPath = join(directory, "thread.json");
  const exported = JSON.parse(
    invoke(directory, "thread", "export", threadId, "--output", exportedPath),
  );
...
test("CLI rejects an unsupported legacy Ark provider", () => {
  ...
  const result = spawnSync(process.execPath, [cli, "run-once"], {
    cwd: directory,
    encoding: "utf8",
    env: { ...process.env, GOAH_STATE_HOME: testStateRoot },
  });
...
test("non-interactive CLI refuses credential-like messages before creating a Turn", () => {
  const directory = repository();
  invoke(directory, "init", "--provider", "faux");
  const state = join(tmpdir(), `goah-sensitive-${process.pid}-${Date.now()}`);
  const result = spawnSync(process.execPath, [cli, "API password: very-secret-value"], {
    cwd: directory,
    encoding: "utf8",
    env: { ...process.env, GOAH_STATE_HOME: state },
  });
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /not accepted non-interactively/);
  assert.doesNotMatch(result.stderr, /very-secret-value/);
});
...
test("thread export redaction preserves structure while removing common secrets and home paths", () => {
  const redacted = redactValue({
    ...
    lease_token: "snake-secret",
    nested: {
      text: `Bearer abcdef /Users/example key-abcdefghijklmnop API password:very-secret-value ${process.env.HOME}`,
    },
  }) as { token: string; leaseToken: string; lease_token: string; nested: { text: string } };
  assert.equal(redacted.token, "[REDACTED]");
  assert.equal(redacted.leaseToken, "[REDACTED]");
  assert.equal(redacted.lease_token, "[REDACTED]");
...
test("status reports model capabilities per Agent and keeps CEO as the primary summary", () => {
  const clock: Clock = { now: () => new Date("2030-01-01T00:00:00.000Z") };
  const ledger = new SqliteLedger(":memory:", { clock });
  const now = clock.now().toISOString();
  ledger.putThread(
  ...
  for (const [agent, threadId, model] of [
    ["ceo", "ceo-thread", "ceo-model"],
    ["worker", "worker-thread", "worker-model"],
  ] as const) {
    const id = `${agent}-turn`;
    putSyntheticTurn(ledger, agent, threadId, id, now);
    ...
    const responseId = `${id}:answer`;
    ledger.putTurnItem(
      {
        id: responseId,
        turnId: id,
        ordinal: ledger.turnItems(id).length + 1,
        type: "assistant_message",
        status: "completed",
        data: { text: "done" },
        createdAt: now,
    ...
    ledger.commitTurnResponse(id, responseId, now, "supervisor");
  }
  const status = statusSnapshot(ledger) as {
    modelCapabilities: { model: string };
    modelCapabilitiesByAgent: Record<string, { model: string }>;
  };
  assert.equal(status.modelCapabilities.model, "ceo-model");
  assert.deepEqual(
    Object.fromEntries(
      Object.entries(status.modelCapabilitiesByAgent).map(([agent, value]) => [agent, value.model]),
    ),
    { ceo: "ceo-model", worker: "worker-model" },
  );
  ledger.close();
});
...
test("CLI exposes the complete goal lifecycle with revisioned transitions", () => {
  ...
  invoke(
    ...
    "Initial objective",
    "--observation-method",
    "Accept a fresh evidence-backed handoff.",
  );
  ...
  const updated = JSON.parse(
    invoke(
      ...
      "Updated objective",
      "--observation-method",
      "Inspect a fresh handoff for the updated objective.",
      "--verification-method",
      "Accept the fresh evidence-backed handoff.",
    ),
  );
  ...
  const completed = JSON.parse(
    invoke(
      ...
      "lifecycle",
      "--reason",
      "fresh handoff satisfies the observation method",
      "--evidence",
      String(evidenceSeq),
    ),
  );
  ...
  assert.match(
    invokeFailure(directory, "goal-update", "lifecycle"),
    /requires objective, observation method, or verification method/,
  );
  assert.match(
    invokeFailure(directory, "goal-update", "lifecycle", "--owner", "other"),
    /atomic CEO reassignment/,
  );
});
...
test("CLI runs a local operations goal without Git", () => {
  ...
  assert.equal(doctor.ok, true);
  assert.match(
    doctor.checks.find((item: { name: string }) => item.name === "root").detail,
    /runner-owned local execution/,
  );
  invoke(
    directory,
    "goal-create",
    ...
    "ceo",
    "--objective",
    "Coordinate storefront operations",
  );
  invoke(
    directory,
    "goal-create",
...
test("CLI exposes one-objective CEO entry and coalesces human corrections", () => {
  ...
  const started = JSON.parse(
    invoke(directory, "goal", "start", "--id", "company", "--objective", "Launch a company"),
  );
  assert.equal(started.goal.owner, "ceo");
  assert.equal(started.wake.agent, "ceo");
  const sent = JSON.parse(
    invoke(directory, "ceo", "send", "--message", "Prioritize low inventory risk"),
  );
  ...
  assert.equal(
    status.team.find((member: { agent: string }) => member.agent === "ceo").motion,
    "queued",
  );
});
...
test("CLI revises and confirms a root through the resident Supervisor control socket", async () => {
  ...
  try {
    ...
    if (process.platform !== "win32")
      assert.equal(statSync(controlEndpoint(config.stateDir)).mode & 0o777, 0o600);
    const oversized = await new Promise<string>((resolveResponse, reject) => {
      const socket = createConnection(controlEndpoint(config.stateDir));
      let response = "";
      socket.on("connect", () => socket.write("x".repeat(CONTROL_LINE_LIMIT + 1)));
      socket.on("data", (chunk) => {
        response += chunk.toString();
      });
      socket.on("error", reject);
      socket.on("close", () => resolveResponse(response));
    });
    assert.match(oversized, /exceeded 1 MB/);
    assert.equal(await controlAvailable(config.stateDir), true);
    const started = JSON.parse(
      invoke(directory, "goal", "start", "--id", "live", "--objective", "Grow revenue"),
    );
    ...
    const revised = JSON.parse(
      invoke(directory, "goal-update", "live", "--objective", "Grow retained net revenue"),
    );
    assert.equal(revised.observationMethod, null);
    assert.equal(
      JSON.parse(invoke(directory, "ceo", "status")).roots[0].objective,
      "Grow retained net revenue",
    );
    assert.match(
      invoke(directory, "Review the revised goal and propose a new observation method"),
      /Hello from Goah/,
    );
  } finally {
    daemon.kill("SIGTERM");
...
test("failed detached daemon startup is reaped", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goah-daemon-fail-"));
  const stateDir = join(directory, "state");
  const pidFile = join(directory, "pid");
  await assert.rejects(
    ensureDaemon(pidFile, stateDir, {
      entryPath: daemonFixture,
...
function putSyntheticTurn(
  ...
  const common = {
    ...
    startedAt: now,
    endedAt: null,
    leaseUntil: new Date(Date.parse(now) + 600_000).toISOString(),
    leaseToken: id,
    runnerPid: null,
  };
  if (agent === "ceo") {
    const turn = { ...common, triggerKind: "user_message" as const };
  ...
  if (ledger.claimNextWake(now)?.id !== wakeId)
    throw new Error("test failed to claim synthetic Wake");
  ledger.startTurnFromWake(wakeId, { ...common, triggerKind: "wake" }, now);
}
