...
import { specialistAutomaticTarget,type Clock, type Runner } from "goah-ledger-contract";
...
test("interactive stream terminates when a Human turn completes", async () => {
  ...
  const runner: Runner = { isolation: "process", prepare: () => ({ pid: null, begin: () => undefined, result: Promise.resolve({ outcome: "completed", response: { content: "normal answer" } }), terminate: async () => undefined }), terminateProcess: async () => undefined };
...
test("welcome snapshot excludes an uncommitted Goal response",()=>{const state=mkdtempSync(join(tmpdir(),"goah-welcome-provisional-"));const clock:Clock={now:()=>new Date("2030-01-01T00:00:00.000Z")};const ledger=new SqliteLedger(join(state,"ledger.sqlite"),{clock});const now=clock.now().toISOString();ledger.putThread({id:"ceo",agent:"ceo",role:"ceo",parentThreadId:null,createdAt:now,updatedAt:now},"supervisor");ledger.putGoal({id:"g",parentId:null,objective:"g",observationMethod:null,verificationMethod:null,owner:"ceo",phase:"active",revision:0},"human");ledger.putTurn({id:"t",threadId:"ceo",triggerKind:"user_message",goalId:null,goalRevision:null,status:"in_progress",attempt:1,error:null,startedAt:now,endedAt:null,leaseUntil:"2030-01-01T00:10:00.000Z",leaseToken:"lease",runnerPid:null},"human");ledger.commitTurnToGoal("t",{goalId:"g",goalRevision:0},"supervisor");ledger.putTurnItem({id:"provisional",turnId:"t",ordinal:1,type:"assistant_message",status:"completed",data:{text:"not committed"},createdAt:now,completedAt:now},"ceo");ledger.finishTurn("t","failed",{message:"Handoff rejected"},now,"supervisor");ledger.close();assert.deepEqual(welcomeSnapshot(state,{runner:"pi",target:"test/model"}).conversation,[]);});
...
function putSyntheticTurn(ledger:SqliteLedger,agent:string,threadId:string,id:string,now:string):void{const common={id,threadId,goalId:null,goalRevision:null,status:"in_progress" as const,attempt:1,error:null,startedAt:now,endedAt:null,leaseUntil:new Date(Date.parse(now)+600_000).toISOString(),leaseToken:id,runnerPid:null};if(agent==="ceo"){ledger.putTurn({...common,triggerKind:"user_message"},"human");return;}const wakeId=`wake:${id}`;ledger.enqueueWake({id:wakeId,...specialistAutomaticTarget(agent,"verifier"),triggerRef:`test:${id}`,status:"queued",attempt:0,enqueuedSeq:0,claimedAt:null,consumedAt:null,turnId:null},"supervisor");if(ledger.claimNextWake(now)?.id!==wakeId)throw new Error("test failed to claim synthetic Wake");ledger.startTurnFromWake(wakeId,{...common,triggerKind:"wake"},now);}
