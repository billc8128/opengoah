...
export interface RunnerCommandWizardResult {
...
export function renderSetupHeader(
  ...
  const meter = progress
    ? `${"━".repeat(progress.current)}${"─".repeat(Math.max(0, progress.total - progress.current))}  ${progress.current}/${progress.total}`
    : "";
  return [
    `${tuiTheme.brand(" GOAH ")}${tuiTheme.rail(` ${surface}${meter ? `  ${meter}` : ""} `)}`,
    "",
    `  ${tuiTheme.strong(title)}`,
    description ? `  ${tuiTheme.muted(description)}` : "",
    "",
  ].join("\n");
}
...
export async function runSetupWizard(
  current: RunnerProfile | null = readDefaultRunnerProfile(),
): Promise<WizardResult> {
  if (!process.stdin.isTTY || !process.stdout.isTTY)
    throw new Error(
      "goah setup requires an interactive terminal; use `goah init --provider ID --model ID` for non-interactive setup.",
    );
  ...
  return withTuiInteraction(async (interaction) => {
    while (true) {
      ...
      try {
        ...
        const runner = await interaction.select({
          title: "Choose a runner",
          description: "A runner owns execution, models, and authentication.",
          progress: { current: 1, total: 5 },
          choices: manifests.map((item) => ({
            value: item.id,
            label: item.name,
        ...
        let config = transaction
          ? transaction.config
          : await configurator.setup(currentConfig, interaction);
        if (config === null) {
          await transaction?.rollback();
          return { profile: null };
        }
        const summary = configurator.summarize?.(config) ?? summarize(config);
        const confirmation = await interaction.select({
          title: "Review",
        ...
        if (confirmation === "save" && transaction) config = await transaction.commit();
        else if (confirmation !== "save") await transaction?.rollback();
        if (confirmation === "save" && config === null)
          throw new Error("Runner setup transaction produced no configuration");
        return {
          profile:
            confirmation === "save" ? { id: current?.id ?? "default", runner, config } : null,
          ...(confirmation === "save" && transaction ? { rollback: transaction.rollback } : {}),
        };
      } catch (error) {
        await transaction?.rollback();
...
async function withTuiInteraction<T>(
  flow: (interaction: RunnerSetupInteraction) => Promise<T>,
  surface = "SETUP",
): Promise<T> {
  const tui = new TuiAltScreen(new ProcessTerminal(), true);
  const header = new Text("");
  let slot: Component | null = null;
  const replace = (component: Component): void => {
  ...
  const setHeader = (
    ...
    header.setText(
      renderSetupHeader(title, description, surface === "SETUP" ? progress : undefined, surface),
    );
  };
  const interaction: RunnerSetupInteraction = {
    select: ({ title, description, choices, progress }) =>
      new Promise((resolve) => {
        setHeader(title, description, progress);
        const list =
          choices.length >= 8
            ? new SearchableSelect(choices as SelectItem[], 9, theme, {
                searchLabel: `Search ${title.toLowerCase()}`,
                emptyLabel: `No matching ${title.toLowerCase()}`,
              })
    ...
    input: ({ title, description, prompt, initial, secret, progress }) =>
      new Promise((resolve) => {
    ...
    notify: (message) => {
      setHeader("Authentication", message);
      tui.requestRender();
    },
    openUrl,
  };
...
export async function runRunnerCommandWizard(
  current: RunnerProfile,
  command: string,
  args: string[] = [],
): Promise<RunnerCommandWizardResult> {
  const plugin = runnerPlugin(current.runner);
  if (!plugin.configurator.runCommand)
    throw new Error(`${current.runner} does not expose runner commands.`);
  if (!process.stdin.isTTY || !process.stdout.isTTY)
    throw new Error(
      command === "model"
        ? "goah model requires an interactive terminal; use `goah model list` or `goah model use PROVIDER/MODEL`."
        : "goah auth requires an interactive terminal; use `goah auth status|list|login|logout PROVIDER`.",
    );
  const run = async (interaction: RunnerSetupInteraction): Promise<RunnerCommandWizardResult> => {
    ...
    return {
      profile: result.config === undefined ? current : { ...current, config: result.config },
...
export async function chooseSetupSection(current: RunnerProfile): Promise<SetupSection | null> {
  ...
  const summary =
    runnerPlugin(current.runner).configurator.summarize?.(current.config) ??
    summarize(current.config);
  return withTuiInteraction(
    (interaction) =>
      interaction.select({
        title: "Goah settings",
        description: summary.map((item) => `${item.label.padEnd(12)} ${item.value}`).join("\n "),
...
function summarize(config: JsonValue): Array<{ label: string; value: string }> {
  if (!config || typeof config !== "object" || Array.isArray(config))
    return [{ label: "Configuration", value: String(config) }];
  return Object.entries(config)
    .filter(([key]) => !/key|token|secret|password|authFile/i.test(key))
