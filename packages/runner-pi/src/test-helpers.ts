import { specialistTarget, type RunRequest } from "goah-ledger-contract";

/** Minimal RunRequest for ProcessRunner tests: trace emits are dropped. */
export function wakeContext(): RunRequest {
  const sourceWake = { id: "test-wake", ...specialistTarget("worker","verifier"), triggerRef: "test", status: "queued" as const, attempt: 0, enqueuedSeq: 0, claimedAt:null,consumedAt:null,turnId:null };
  return {
