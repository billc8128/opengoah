...
test("Pi configurator owns provider, model, and auth choices", async () => {
  ...
  const config = (await piRunnerConfigurator().setup(null, interaction)) as {
    provider: string;
    model: string;
  };
...
test("model command uses a scoped picker without replaying authentication for the current provider", async () => {
  ...
  const authFile = join(mkdtempSync(join(tmpdir(), "goah-model-env-auth-")), "auth.json");
  const interaction: RunnerSetupInteraction = {
    select: async ({ title, choices }) =>
      title.includes("provider")
        ? "openai"
        : title.includes("model")
          ? choices.find((choice) => choice.value === "gpt-5.5")!.value
          : null,
    input: async () => null,
    notify: () => undefined,
  };
  try {
    const result = await piRunnerConfigurator().runCommand!(
      "model",
      [],
      {
        provider: "openai",
...
test("model picker preserves a custom credential store and never launches authentication", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goah-model-auth-store-"));
  const authFile = join(directory, "custom-auth.json");
  await new JsonCredentialStore(authFile).modify("openai", async () => ({
    type: "api_key",
    key: "existing-key",
  }));
  const interaction: RunnerSetupInteraction = {
    select: async ({ title, choices }) => {
      ...
      if (title === "Choose a model")
        return choices.find((choice) => choice.value === "gpt-5.5")!.value;
  ...
  const result = await piRunnerConfigurator().runCommand!(
    "model",
    [],
    { provider: "faux", model: "faux-goah", authFile, authMode: "local" },
    interaction,
  );
...
test("switching to a provider without credentials requires authentication before saving", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goah-model-requires-auth-"));
  const authFile = join(directory, "auth.json");
  const interaction: RunnerSetupInteraction = {
    select: async ({ title, choices }) => {
      if (title === "Choose a provider") return "cloudflare-workers-ai";
      if (title === "Choose a model") return choices[0]!.value;
      if (title === "Authentication") return "key";
      return null;
    },
    input: async ({ secret }) => (secret ? "cloudflare-test-key" : null),
    notify: () => undefined,
  };
  const result = await piRunnerConfigurator().runCommand!(
    "model",
  ...
  assert.equal((result.config as { provider: string }).provider, "cloudflare-workers-ai");
  assert.equal((result.config as { authMode: string }).authMode, "stored-key");
  assert.deepEqual(await new JsonCredentialStore(authFile).read("cloudflare-workers-ai"), {
    type: "api_key",
    key: "cloudflare-test-key",
  });
});

test("model switch rejects an environment reference that is not actually set", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goah-model-missing-env-"));
  const authFile = join(directory, "auth.json");
  delete process.env.GOAH_TEST_MISSING_API_KEY;
  const interaction: RunnerSetupInteraction = {
    select: async ({ title, choices }) => {
      if (title === "Choose a provider") return "openai";
      if (title === "Choose a model")
        return choices.find((choice) => choice.value === "gpt-5.5")!.value;
      if (title === "Authentication") return "env";
      return null;
    },
    input: async ({ title }) =>
      title === "Environment variable" ? "GOAH_TEST_MISSING_API_KEY" : null,
    notify: () => undefined,
  };
  await assert.rejects(
    piRunnerConfigurator().runCommand!(
      "model",
      [],
      { provider: "faux", model: "faux-goah", authFile, authMode: "local" },
      interaction,
...
test("auth login command supports API keys without running model setup", async () => {
  ...
  try {
    ...
    const result = await piRunnerConfigurator().runCommand!(
      "auth",
      ["login", "zai"],
      { provider: "zai", model: "glm-5.3", authMode: "unconfigured" },
      interaction,
    );
    assert.equal((result.config as { authMode: string }).authMode, "stored-key");
    assert.deepEqual(await new JsonCredentialStore(join(state, "auth.json")).read("zai"), {
      type: "api_key",
      key: "command-test-key",
    });
  } finally {
...
test("scoped auth refuses a concurrent update to the same provider", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goah-auth-provider-cas-"));
  const authFile = join(directory, "auth.json");
  ...
  const command = piRunnerConfigurator().runCommand!(
    "auth",
    ["login", "zai"],
    { provider: "zai", model: "glm", authFile },
    {
      select: async () => "key",
      input: async () => {
        inputReady.resolve();
  ...
  await store.modify("zai", async () => ({ type: "api_key", key: "concurrent" }));
...
test("explicit model use accepts faux and custom provider ids", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goah-explicit-model-auth-"));
  const authFile = join(directory, "auth.json");
  await new JsonCredentialStore(authFile).modify("team-gateway", async () => ({
    type: "api_key",
    key: "gateway-key",
  }));
  const interaction: RunnerSetupInteraction = {
  ...
  const faux = await piRunnerConfigurator().runCommand!(
    "model",
    ["use", "faux/faux-goah"],
    {
  ...
  const custom = await piRunnerConfigurator().runCommand!(
    "model",
    ["use", "team-gateway/fast-v2"],
    {
      provider: "team-gateway",
      model: "fast-v1",
      baseUrl: "https://models.example.test/v1",
...
test("logout updates stored auth state and is honest about environment auth", async () => {
  const previous = process.env.GOAH_STATE_HOME;
  const state = mkdtempSync(join(tmpdir(), "goah-command-logout-"));
  process.env.GOAH_STATE_HOME = state;
  try {
    ...
    await store.modify("retired-gateway", async () => ({
      type: "api_key",
      key: "gateway-private",
    }));
    const interaction: RunnerSetupInteraction = {
    ...
    const stored = await piRunnerConfigurator().runCommand!(
      "auth",
      ["logout", "zai"],
      { provider: "zai", model: "glm", authMode: "stored-key" },
      interaction,
    );
    assert.equal((stored.config as { authMode: string }).authMode, "unconfigured");
    assert.equal(await store.read("zai"), undefined);
    const env = await piRunnerConfigurator().runCommand!(
      "auth",
      ["logout", "zai"],
      { provider: "zai", model: "glm", authMode: "environment", apiKeyEnv: "ZAI_API_KEY" },
      interaction,
    );
    assert.equal(env.config, undefined);
    assert.match(env.output[0]!, /unset it in your shell/);
    const switched = await piRunnerConfigurator().runCommand!(
      "model",
      ["use", "openai/gpt-5.5"],
      { provider: "faux", model: "faux-goah", authMode: "local" },
      interaction,
    );
    assert.equal((switched.config as { authMode: string }).authMode, "stored-key");
    const listed = await piRunnerConfigurator().runCommand!(
      "auth",
      ["list"],
      { provider: "zai", model: "glm", authMode: "unconfigured" },
      interaction,
    );
    assert.match(listed.output.join("\n"), /retired-gateway/);
    await piRunnerConfigurator().runCommand!(
      "auth",
      ["logout", "retired-gateway"],
      { provider: "zai", model: "glm", authMode: "unconfigured" },
      interaction,
    );
  ...
  const interaction: RunnerSetupInteraction = {
    select: async ({ title, choices }) => {
      if (title === "Authentication") {
        authChoices = choices.map((choice) => choice.value);
        return "back";
      }
      return null;
    },
...
test("faux remains a local provider in authentication flows", async () => {
  ...
  const login = await piRunnerConfigurator().runCommand!(
    "auth",
    ["login", "faux"],
    { provider: "faux", model: "faux-goah", authMode: "local" },
    interaction,
  ...
  const logout = await piRunnerConfigurator().runCommand!(
    "auth",
    ["logout", "faux"],
    { provider: "faux", model: "faux-goah", authMode: "local" },
    interaction,
...
test("setup stores a pasted API key privately through a masked interaction", async () => {
  ...
  try {
        ...
        title.includes("provider")
          ...
          : title.includes("model")
            ...
            : title === "Authentication"
              ...
              : null,
      input: async ({ secret }) => {
        secretPrompt = Boolean(secret);
        return secret ? "private-test-key" : null;
      },
      notify: () => undefined,
    };
    const config = (await piRunnerConfigurator().setup(null, interaction)) as {
    ...
    assert.equal(config.apiKeyEnv, undefined);
    const stored = (await new JsonCredentialStore(join(state, "auth.json")).read("zai")) as {
      type: string;
      key?: string;
    };
...
test("setup transaction rolls credential changes back on cancel", async () => {
  const previous = process.env.GOAH_STATE_HOME;
  const state = mkdtempSync(join(tmpdir(), "goah-setup-rollback-"));
  process.env.GOAH_STATE_HOME = state;
  try {
    const interaction: RunnerSetupInteraction = {
      select: async ({ title, choices }) =>
    ...
    const configurator = piRunnerConfigurator();
    const transaction = await configurator.beginSetup!(null, interaction);
    ...
    assert.equal(
      ((await new JsonCredentialStore(join(state, "auth.json")).read("zai")) as { key: string })
        .key,
      "temporary-key",
    );
...
test("setup transaction refuses to overwrite concurrent credential changes", async () => {
  const previous = process.env.GOAH_STATE_HOME;
  const state = mkdtempSync(join(tmpdir(), "goah-setup-conflict-"));
  process.env.GOAH_STATE_HOME = state;
  try {
    const interaction: RunnerSetupInteraction = {
      select: async ({ title, choices }) =>
    ...
    const realStore = new JsonCredentialStore(join(state, "auth.json"));
    await realStore.modify("openai", async () => ({ type: "api_key", key: "concurrent-key" }));
...
test("Pi ProcessRunner keeps the full credential store outside the worker environment", () => {
  const runner = createPiProcessRunner(
    { provider: "openai", model: "gpt-5.5", apiKeyEnv: "PRIVATE_OPENAI_KEY" },
    process.cwd(),
  );
...
test("Pi doctor reports native host tool access", async () => {
  const checks = await piRunnerConfigurator().doctor({
    provider: "faux",
    model: "faux-goah",
    authMode: "local",
  });
  assert.equal(
    checks.find((check) => check.name === "tool-access")!.detail,
    "Pi native tools · host user permissions",
  );
});

test("Pi custom endpoints are runner-local overlays", () => {
  const configured = createPiModel("team-gateway", "fast-model", {
    GOAH_PI_BASE_URL: "https://models.example.test/v1",
    GOAH_PI_API: "openai-responses",
    GOAH_PI_CONTEXT_WINDOW_TOKENS: "64000",
    GOAH_PI_AUTH_FILE: "",
  });
...
test("credential store serializes read-modify-write across instances", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goah-auth-lock-"));
  const path = join(directory, "auth.json");
  const first = new JsonCredentialStore(path);
  const second = new JsonCredentialStore(path);
  await Promise.all([
...
test("credential store reclaims only a lock owned by a dead process", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goah-auth-stale-lock-"));
  const path = join(directory, "auth.json");
  const lock = `${path}.lock`;
  mkdirSync(lock);
  writeFileSync(join(lock, "owner.json"), JSON.stringify({ pid: 999_999_999, token: "dead" }));
  const store = new JsonCredentialStore(path);
...
test("credential store does not overwrite malformed data", async () => {
  const directory = mkdtempSync(join(tmpdir(), "goah-auth-malformed-"));
  const path = join(directory, "auth.json");
  writeFileSync(path, "not-json");
  const store = new JsonCredentialStore(path);
  await assert.rejects(
    store.modify("one", async () => ({ type: "api_key", key: "new" })),
    /JSON/,
  );
