...
export async function runVerificationWorker(): Promise<void> {
  ...
  for await (const line of lines) {
    const request = JSON.parse(line) as { operation: "verify_turn" | "blind_audit"; input: unknown };
    ...
    const tool: AgentTool<any> = {
      ...
      parameters: Type.Object({ findings: Type.Array(Type.Object({ id: Type.String(), body: Type.Any(), evidence: Type.Array(Type.Number()), riskWeight: Type.Number() })) }),
