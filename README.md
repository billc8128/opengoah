...
New to the model? [`docs/concepts.md`](./docs/concepts.md) is a one-screen tour: the Goal → Wake → Turn → Work Record + Handoff cycle, plus one-line definitions of every core noun.
...
The most important implemented capabilities today:

- **Event kernel** — stream-aware append-only SQLite ledger with durable Thread, Turn, Item, Goal, Work Record, Schedule, Wake, and Mailbox projections, all rebuildable by replay
- **Resumable transcripts** — versioned Turn vocabulary with replay, content-addressed exact request snapshots, normalized user/assistant/tool events, and interrupted-tool repair
- **Goal execution** — FIFO Wake scheduling with per-agent claim exclusion and durable Schedule terminal states; committed Turns must update the Work Record before an accepted compact Handoff
- **Interactive CEO** — the `goah` shell with durable Goal observation methods, a ledger-derived team roster, atomic delegation/reassignment, and the event-sourced Work Record filesystem
- **Verification** — transcript verifier and global audit interfaces with atomic Mail delivery of findings

The full capability checklist, remaining operational-evidence gaps, and the fault canary live in [`docs/status.md`](./docs/status.md).
...
- `request.component` stores each exact model-visible prompt/context/message/toolset value once per Turn; `request.prepared` records only hashes plus behavior configuration. Provider API keys, authorization headers, abort handles, and transport-private objects remain excluded.
...
- Child completion is mechanically fenced on a current owner-written Work Record from a completed Turn, cited directly or through its matching Handoff.
...
- Credential-like TUI messages require a second Enter and non-interactive submission is refused. Confirmed secrets still go to the model provider and Ledger; prefer an ignored local file and tell the Agent only its path.
